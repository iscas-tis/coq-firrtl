...
\author{David N. Jansen \\[0.3ex] based on discussion with Xiaomu Shi, Keyin Wang and Zhilin Wu}
...
\item	a set of connection trees.
	The root of the tree is marked with an input connector.
	Every leaf in the tree marked with an output connector of a data type that is equivalent with the input connector of the root.
	Every internal node has two children (a \texttt{true} and a \texttt{false} child)
	and is additionally marked with an output connector of data type $\mathsf{UInt\langle 1\rangle}$;
	the latter is called the condition.
	Depending on the condition, data from one of the two children is selected to be forwarded to the parent.
\end{itemize}
\end{definition}
...
The circuit semantics of a HiFIRRTL module is defined to be a set of module graphs.
...
\item	other components and connection trees are defined inductively depending on $S$:
	...
	\item	If $S = (s_1, s_2, \ldots, s_{n+1})$,
		...
		\end{itemize}
	\end{itemize}
\item	If some components have unspecified widths, there may be multiple module graphs in the set.
	Every such module graph has to satisfy what the specification calls the ``legality of its incoming connections'',
	but we prefer to call it ``avoidance of data loss''.
	the inferred width of the input connector at the root of every connection tree is $\geq$ the maximum width of the output connectors of the leaves.
	(Primitive operations, multiplexers and similar components have internal rules
	on what the output width is, given the input widths.
	These internal rules can be found in the specification.)
\end{itemize}
...
or if it contains combinatorial cycles.)
...
Possible solution: we allow a special subaccess component
(with many input connectors: a $\mathsf{UInt}$ selector and a list of input connectors that all have the same data type)
that implements the subaccess
until it is replaced by a chain of multiplexers.
