...
We formalize the ``InferWidths'' pass by defining a Coq function {\tt infer\_width}, which iterates over the statements in $ss$ and updates $ce$, so that the bit widths of all the components can be determined. 
%
  \begin{gather}
    \inferiii[]
...
Moreover, {\tt infer\_width} uses a Coq function {\tt max\_width}, which compares the widths of two types and returns the type of larger width.
