...
the \prettyll{infer\_width} function will update the $wm$ by

  \begin{gather}
    \inferii[]
    {\seqq{ce \vdash infer\_width(declare\;v, wm) \rightsquigarrow wm'}}
    {\seqq{ce \vdash width(t_v) = 0}} {\seqq{wm':= wm \uplus {v : t_v}}}.
    \end{gather}
...
named \prettyll{max\_width} compares the potential widths and outputs
the maximum one. For one statement which connects an expression $e$
to a component $v$, the \prettyll{infer\_width} function will update $wm$ by

  \begin{gather}
    \inferiii[]
    {\seqq{ce \vdash infer\_width(v<=e, wm) \rightsquigarrow wm'}}
    {\seqq{wm \vdash v : t_v}}
    {\seqq{ce \vdash t_e:= type\_of\_expr(e)}}
    {\seqq{wm':= wm \uplus {v : max\_width(t_v,t_e)}}}.
    \end{gather}

After applying \prettyll{infer\_width} on all the statement in $ss$,
the $wm$ contains the inferred width for every implicit defined
component. By using \prettyll{map2} function t merge the $ce$ and $wm$
with the same entry, the component environment $ce$ now contains the
result of the width inferring.

Instead of applying \prettyll{infer\_width} directly on $ce$, the map
