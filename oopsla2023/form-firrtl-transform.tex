...
\subsection{Formalization of InferTypes}
...
We formalize the ``InferTypes'' pass by defining a Coq function {\tt infer\_type}, which iterates over the statements in $ss$ and updates $ce$. 
%For each component defined in $ss$, the {\tt infer\_type} function will update $ce$
%accordingly. 
For instance, the following rule specifies that when {\tt infer\_type} is applied to a wire statement \prettyll{wire} $x: t$, it updates $ce$ by assigning $(t, Wire)$ to $x$ (where $Wire$ denotes the kind of $x$), resulting into the component environment $ce'$,
...
%it is added to component environment by
  \begin{gather}
    ...
    \end{gather}
Moreover, {\tt infer\_type} behaves similarly on \prettyll{reg}, \prettyll{input} and \prettyll{output} port statements. Finally, on a node statement \prettyll{node } $x = e$, {\tt infer\_type} first calculates the type of $e$, denoted by $t_e$, then assigns $(t_e, Node)$ to $x$, resulting into $ce'$,
% 
...
\begin{gather}
    ...
    \end{gather}
Note that {\tt infer\_type} will not update $ce$ when it is applied to connect statements. Nevertheless, in this case, it checks whether the two types of both sides of the connect statement are equivalent. By type equivalence, we mean that the structures of the types are the same, but their bit widths may be missing or different. For instance, the types \prettyll{UInt}, \prettyll{UInt<1>}, and \prettyll{UInt<2>} are seen as equivalent. 
If the two types are equivalent, then $ce$ is kept, otherwise, an error (indicated by $\bot$) will be issued. 
\begin{gather}
    \inferiii[]
...
\begin{gather}
    ...
    \end{gather}
After {\tt infer\_type} has been applied to all the statements in $ss$, if the bit width of a component is still missing, then the zero width will be added for the component in $ce$. 
%After applying the Infer Types pass, types with an
%implicitly defined width are assigned with 0 in $ce$.

\subsection{Formalization of InferWidths}
%The bit widths of some components may still be missing after applying {\tt infer\_type} to all the statements in $ss$.
We formalize the ``InferWidths'' pass by defining a Coq function {\tt infer\_width}, which iterates over the statements in $ss$ and updates $ce$, so that the bit widths of all the components with undefined bit widths can be determined. 
...
The rationale behind {\tt infer\_width} is stated as follows: After applying {\tt infer\_type} to all the statements in $ss$, for each component $x$,
\begin{itemize}
\item if the bit width of $x$ is nonzero, that is, a bit width is available in its declaration statement, then {\tt infer\_width} will \emph{not} change the bit width of $x$,
%
\item otherwise, {\tt infer\_width} chooses the maximum width of the expressions that are connected to $x$. 
%\item The last connection semantics is not considered at this stage,
%  all connect statements are taken into account.
\end{itemize}
Note that the last-connected semantics is ignored here. The behavior of {\tt infer\_width} on a connect statement is defined by the following rule, 
%
  \begin{gather}
    \inferiii[]
...
Moreover, {\tt infer\_width} uses a Coq function {\tt max\_width}, which, given two aggregate types, compares for each field their widths in the aggregate types, and chooses the larger width.
Finally, {\tt infer\_width} processes the partial connect statements similarly. 
...
\subsection{Formalization of InferResets}
We formalize the ``InferResets'' pass by defining a Coq function {\tt infer\_reset}, which iterates over the statements in $ss$ and updates $ce$, so that the concrete reset types of all abstract reset signals can be inferred. 
%Note that after applying {\tt infer\_type} to all the statements in $ss$, if the bit width of a component is nonzero, that is, a bit width is available in its declaration statement, then its bit width will \emph{not} be changed by  {\tt infer\_width}.
The rationale behind {\tt infer\_reset} is stated as follows: After applying {\tt infer\_type} to all the statements in $ss$, for each component $x$ of  type \prettyll{Reset},
\begin{itemize}
\item if only components of  type \prettyll{AsyncReset} are connected to $x$, then {\tt infer\_reset} updates the type of $x$ to \prettyll{AsyncReset}, 
%
\item if $x$ \prettyll{is invalid} is a statement in $ss$ or only components of type \prettyll{UInt<1>} are connected to $x$, then {\tt infer\_reset} updates the type of $x$ to \prettyll{UInt<1>}, 
%
\item if two components of type  \prettyll{AsyncReset} and \prettyll{UInt<1>} respectively are both connected to $x$, then  {\tt infer\_reset} reports an error.
\end{itemize}
The behavior of {\tt infer\_reset} on a connect statement is defined by the following two rules, 
  \begin{gather}
...
%    {\seqq{ce'= \bot}}.
    \end{gather}
Note that {\tt infer\_reset} uses the predicate {\tt conflict} to decide that two reset types are conflicting, that is, one is  \prettyll{AsyncReset} and the other is \prettyll{UInt<1>}.
...
\subsection{Formalization of ExpandConnects}
...
The rationale behind {\tt expand\_expr} and {\tt expand\_cnct} is to break aggregate types in expressions and connect statements recursively into their fields. Note that {\tt expand\_cnct} expands both connect and partial connect statements. The function {\tt expand\_expr} and {\tt expand\_cnct} modify $ss$, instead of $ce$.
%\begin{itemize}
...
The function {\tt expand\_expr} expands the \prettyll{mux} and 
 \prettyll{validif} expressions by the following two rules, 
  ...
  \label{expand-validif}
    ...
    \end{gather}
The rule~(\ref{expand-mux}) uses a function {\tt ls\_mux} that combines $els_t$ and $els_f$ with the same condition $e_c$, resulting into a list of ground-type expressions $\mbox{\prettyll{mux}}(e_c,els_{t, 1}, els_{f, 1}), \cdots, \mbox{\prettyll{mux}}(e_c, els_{t, m}, els_{f, m})$, where  $els_{t,i}$ and $els_{f, i}$ are the $i$-th element of $els_t$ and $els_f$ respectively. The function {\tt ls\_validif} in rule~(\ref{expand-validif}) is defined similarly. 
...
Equipped with the {\tt expand\_expr} function, {\tt expand\_cnct} works as follows: Given a connect statement $(x <= e)$, it applies {\tt expand\_expr} to $x$ and $e$ respectively, producing two lists of expressions of ground types, then constructs a list of connect statements by combining the elements in the two lists. The behavior of {\tt expand\_cnct} on a connect statement is defined by the following rule, 
...
%be generated pairwise from the two by
  ...
  \label{expand-cnct}
    ...
    \end{gather}
The function {\tt expand\_cnct} uses a function {\tt ls\_cnct} that combines the two lists $els_l$ and $els_r$ into a list $ss'$ of connect statements between ground types, that is, the statements $els_{l, 1} <= els_{r, 1}, \cdots, els_{l, m} <= els_{r, m}$, where $m$ is the length of the two lists, and $els_{l, i}, els_{r, i}$ are the $i$-th element of $els_l$ and $els_r$ respectively.
...
The function {\tt expand\_cnct} handles the \prettyll{is invalid} statements in the same way as connect statements. It produces from a \prettyll{is invalid} statement
 a list of \prettyll{is invalid} statements for its fields.
...
\subsection{Formalization of ExpandWhens}
...
\subsection{Formalization of LowerTypes}
