...
We formalize the ``InferTypes'' pass in Coq by defining a Coq function {\tt infer\_type} to update $ce$.
%For each component defined in $ss$, the {\tt infer\_type} function will update $ce$
%accordingly. 
For instance, the following rule specifies that when {\tt infer\_type} is applied to a wire declaration \prettyll{wire} $x: t$, it updates $ce$ by assigning $(t, Wire)$ to $x$ (where $Wire$ denotes the kind of $x$), resulting into the component environment $ce'$,
...
%it is added to component environment by
  \begin{gather}
    ...
    \end{gather}
Moreover, {\tt infer\_type} behaves similarly on \prettyll{reg}, \prettyll{input} and \prettyll{output} port declarations. Finally, on a node declaration \prettyll{node } $x = e$, {\tt infer\_type} first calculates the type of $e$, denoted by $t_e$, then assigns $(t_e, Node)$ to $x$, resulting into $ce'$,
% 
