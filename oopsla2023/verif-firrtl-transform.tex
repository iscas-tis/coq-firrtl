...
Since {\tt expand\_cnct} only has effects for the connect statement,
we can state that
...
Here we introduce an additional function {\tt dec\_type} that can decompose the aggregate types
...
If we use the {\tt dec\_type} function on the aggregate type of the expression aimed to be expanded, it
returns the ground types that are identical to the ones produced by {\tt expand\_expr}.
...
Now it is able to apply Lemma \ref{dec-type} with assigning $t_1$ and $t_2$ with ${\tt typeof}\,v$ and ${\tt typeof}\,e$
to complete the proof.

Secondly, we state that types of the experssion obtained by ExpandConnects pass
are all ground types.
\begin{lemma}
$\forall ce ^{in}\,ss^{in},\,ce^{in}\models AllGround ({\tt expand\_cnct}\,ss^{in})$
\end{lemma}

The lemma states the predicate $AllGround$ holds for the generated
statment of {\tt expand\_cnct}.  It is rather easy to be proved by
reducing the problem to prove the {\tt expand\_expr} produce the same
type as {\tt dec\_type}, and then to prove that the decomposing types by
