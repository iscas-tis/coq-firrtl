...
for each individual comparison of the types on the two side of a connection, the type equivalence
predicate $TypeEquiv$ holds.
If there are $n$ statements $s_1,s_2,\dots ,s_n$ in $ss$,
...
then the type of $v$ and the type of $e$ are type equivalent.
...
Secondly, we state that types of the experssions obtained by ExpandConnects pass
...
statment of {\tt expand\_cnct}.
Similarly, the prove goal will be reduced to prove that the $AllGround$ hold for the expanded expressions.
It is rather easy to use the Lemma \ref{expand-expr-dec-type} we had for {\tt dec\_type}.
To related the types of the expanded experssions are identical to decompose the type of the original experssion.
By induction on the aggregate type constructors, it is able to prove that decomposing types by
the {\tt dec\_type} will always return ground types.
