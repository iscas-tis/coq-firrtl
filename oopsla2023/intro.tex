...
%
High-level synthesis is performed on FIRRTL by a sequence of compilation passes  (aka lowering transformations) and optimizations, resulting into a most restricted form that is close to RTL Verilog, called low FIRRTL (abbreviated as LoFIRRTL). For clarity, let us call FIRRTL with high-level constructs as high FIRRTL (abbreviated as HiFIRRTL).
...
and modules so that Chisel programs can easily be translated into FIRRTL.
>>>>>>> 934abe497d6ca01c6dd3c1295716e2cae42a80bd
%
...
%
Nevertheless, partially due to the fact that Chisel and FIRRTL are relatively new and the ecosystem around them is still evolving, formalization and verification of FIRRTL IR and its lowering transformations are largely missing, with an exception \cite{HarrisonHA21}. In~\cite{HarrisonHA21}, a coinductive denotational semantics was proposed for LoFIRRTL, based on a formalism called device calculus. Nevertheless, while Harrison et al. made progress in the formalization of LoFIRRTL in~\cite{HarrisonHA21}, a framework where  programs in HiFIRRTL and its lowering transformations can be formally reasoned about is still elusive. Specifically, the establishment of such a framework would require 
%\begin{itemize}
%\item 
...
\begin{itemize}
\item Define the executable operational semantics for LoFIRRTL. The operational semantics is based on a concept of pseudo-single-static-assignment form of LoFIRRTL programs. 
%
...
%
\item Introduce a concept of circuit diagrams  and formalize the syntax of HiFIRRTL and its seven lowering transformations, namely, ``InferTypes'', ``InferWidths'', ``InferResets'', ``ExpandConnects'', ``ExpandWhens'', ``LowerTypes'', and ``RemoveResets''.
%
\item Specify and verify the functional correctness of two most involved lowering transformations, namely, ``ExpandConnects'' and ``ExpandWhens''.
\end{itemize}

The rest of this paper is organized as follows. Section~\ref{sec-firrtl} gives an informal introduction to FIRRTL and its lowering transformations. Section~\ref{sec-form-low-firrtl} is devoted to the  operational semantics of LoFIRRTL as well as its validation against Treadle. Section~\ref{sec-hi-firrtl-transform} formalizes HiFIRRTL and its lowering transformations. Section~\ref{sec-spec-verif-expand} specifies and verifies ``ExpandConnects'' and ``ExpandWhens''. Section~\ref{sec-conc} concludes this paper. 
