...
%
\item Introduce a concept of circuit diagrams  and formalize the syntax of HiFIRRTL and its six lowering transformations \fu{in Coq}, namely, ``InferTypes'', ``InferWidths'', ``InferResets'', ``ExpandConnects'', ``ExpandWhens'', ``LowerTypes'', and ``RemoveResets''.
Those 
%
...
\item Specify and verify the functional correctness of the two most intricate and impactful lowering transformations in Coq, namely ``ExpandConnects'' and ``ExpandWhens''. They involve significant changes to the original HiFIRRTL program by creating new connections and thus may impact on the functionality of the circuit. The verification ensures the functional correctness that these transformations prsereve the behaviors of the original programs by taking the type equivalence and the (conditional) last conncet semantics into account.
\end{itemize}
