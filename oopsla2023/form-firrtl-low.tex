...
The rules for the operational semantics of statements are presented in Table~\ref{tab:lofirrtl-sem}, where $te[t/v]$ denotes the function that is obtained from $te$ by assigning $t$ to $v$, similarly for $s[0^{wd(t)}/v]$, and so on. Moreover, the rules rely on $te(e)$ and $s(e)$, which are extensions of the functions $te$ and $s$ to expressions and inductively defined in the sequel. 
%
