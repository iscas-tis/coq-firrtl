...
We denote by $\gtypes$ denote the set of ground types $(\{\mbox{\prettyll{UInt}}, \mbox{\prettyll{SInt}}\} \times \Nat) \cup \{\mbox{\prettyll{Clock}},
...
Formally, the \emph{dependency graph} $G_A$ of the module $A$ is a tuple $(N_A, E_A)$, where $N_A$ is the set $\cpnts(A)$ of components,
...
We develop a verified topological sorting algorithm for checking if a dependency graph $G_A$ is loop-free or not.
...
%
\item the concatenation $\pi_1\|\pi_2$ of sequence  $\pi_1$ of input ports and the sequence $\pi_2$ of heads of the generalized connect statements in the module $A$ forms
...
the generalized connect statement  \prettyll{node a = add(io_in, UInt<32>("h1"))} for \prettyll{a} follows
the generalized connect statement \prettyll{io_out <= tail(a, 1)} for \prettyll{io\_out}.
...
the module $B$ for some $y$ (similar to class inheritance graph in object-oriented programming languages), is acyclic.
...
which denotes an instance of module the $B$ with the identifier $id$, i.e., \prettyll{inst id of B} is a statement in the circuit $A$.
...
where $x.$ is added to avoid name conflict.
\end{itemize}
