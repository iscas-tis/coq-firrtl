...
\subsection{FIRRTL Lowering Transformations}
...
\paragraph{ExpandWhens} The ExpandWhens pass expands the when statements and removes the last connect semantics. For instance, the ExpandWhens pass transforms the following program 
\begin{lstlisting}
module MyModule :
  input in: UInt<1>[2]
  input n: UInt<1>
...
\paragraph{LowerTypes} The LowerTypes pass flattens bundles and vectors. Nevertheless, LowerTypes does not change the implicit bundles, e.g. module instances. After the application of LowerTypes, a module instance is still a bundle of depth 1 (instance -> port). 
