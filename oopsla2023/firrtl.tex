...
%For this reason, 
FIRRTL has first-class support for high-level constructs such as vector types, bundle types, conditional statements, connects, partial connects, and modules. The syntax of FIRRTL is presented in Figure. 
...
\begin{array}{l l l}
%\mbox{\bf Circuit} & 
c & ::= & {\tt circuit}\ id \{m \mid em\}\\
%
%\mbox{\bf Module} & 
m & ::= & {\tt module} \ id \{p\} \{s\}\\
%
%\mbox{\bf External Module} & 
em & ::= & {\tt extmodule}\ id \{p\} [{\tt defname =}\ id] \{{\tt parameter}\ id = (string \mid int)\}\\
%
%\mbox{\bf Port} & 
p & ::= & {\tt input}\ id: t \mid {\tt output}\ id: t \\
%
 %\mbox{\bf Statment} & 
 s & ::= & {\tt wire}\ id:t  \mid {\tt reg}\ id: t, e [{\tt (with:\{reset=>(} e , e )\})] \\
 %& 
  &   &  \mid
%  \mid memory \mid 
  {\tt inst}\ id\ {\tt of}\ id \mid {\tt node}\ id = id  \mid r\ \mbox{<=}\ e  \mid r\ \mbox{<-}\ e \mid r\ {\tt is\ invalid} \\ 
%&  
&   &\mid {\tt when}\ e: \{s\} [{\tt else}: \{s\}] \mid {\tt skip}\\ 
%
%\mbox{\bf Memory} & 
...
%
%\mbox{\bf Expression} & 
e & ::= & {\tt UInt} [w] (int) \mid {\tt SInt} [w] (int) \mid r \mid {\tt mux}(e, e, e) \mid {\tt validif}(e, e) \mid primop;\\
%
%& 
r & ::= & id \mid r.id \mid r[ int] \mid r [ e ]\\
%
%& 
w & ::= & {\tt <} int {\tt >}\\
%
...
t & ::= & tg \mid ta \\
 %& 
 tg & ::= &  {\tt Clock} \mid {\tt Reset} \mid {\tt AsyncReset} \mid {\tt UInt}[w] \mid {\tt SInt}[w]  \mid {\tt Analog}[w]  \\
 %& 
...
\caption{Simplified Syntax of FIRRTL}
