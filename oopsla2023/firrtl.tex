...
circuit MyCircuit:
  module MyModule :
    ...
    when eq(n, UInt(0)) :
      ...
      out.result <= in[2]
\end{lstlisting}
A FIRRTL \emph{circuit} comprises modules. A \emph{module} includes ports and  a sequence of statements. A \emph{port}  is either an input port or an output port, and has an identifier as well as a type. A \emph{statement} is either a declaration of wire, register, instance, node, or a full connection (<=) or partial connection (<-), or a conditional (when) statement. A \emph{type} can be either a \emph{ground type}, e.g. \begin{lstinline}!UInt<2>!\end{lstinline}, or an \emph{aggregate type}, which is either a \emph{vector type}, e.g. \begin{lstinline}!UInt[3]!\end{lstinline}, or a \emph{bundle type} e.g 
%
\begin{lstinline}
! {valid: UInt<1>, ready: UInt<1>,  result: UInt<1>} !
\end{lstinline}.
%

\zhilin{stopped here}

FIRRTL has first-class support for high-level constructs such as vector types, bundle types, conditional statements, connects, partial connects, and modules. The syntax of FIRRTL is presented in Figure~\ref{fig-firrtl-synt-simp}. 
