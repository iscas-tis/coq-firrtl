...
%
\item  if $\theta = \mbox{\prettyll{UInt}}$, then $te(div(e_1, e_2)) =\mbox{\prettyll{UInt}} \bwidth{n_1}$ and $s(div(e_1, e_2)) = bv_{\mbox{\prettyll{UInt}} \bwidth{n_1} (s(e_1) \div s(e_2))}$, otherwise, $te(div(e_1, e_2)) = SInt \bwidth{n_1+1}$ and $s(div(e_1, e_2)) = bv_{SInt \bwidth{n_1+1} (s(e_1) \div s(e_2))}$,
%
...
\begin{itemize}
\item if either $\theta = \mbox{\prettyll{UInt}}$, or $\theta=SInt$, $s(e_1) \ge 0$, and $s(e_2) \ge 0$, then \\ $s(cat(e_1, e_2))$ is $bv_{\mbox{\prettyll{UInt}}\bwidth{n_1+n_2}(s(e_1) \times 2^{n_2} + s(e_2))}$,
%
...
%
\item otherwise, if $\theta = \mbox{\prettyll{UInt}}$, then $s(shr(e_1, n))=bv_{\mbox{\prettyll{UInt}}<1>(0)}$, otherwise, $s(shr(e_1, n))=bv_{SInt<1>(0)}$ if $s(e_1) \ge 0$, and $s(shr(e_1, n))=bv_{SInt<1>(1)}$ if $s(e_1) < 0$,
\end{itemize}
%
...
\section{Missing details in Section~\ref{sec-hi-firrtl-transform}}\label{app-hifirrtl}
