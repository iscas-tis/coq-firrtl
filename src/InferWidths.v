...
Section InferWidthP.
  ...
  Fixpoint get_field_name (r : HiFP.href) : N :=
   ...
   Fixpoint add_wmap (p:pvar) t w : wmap :=
   ...
   Fixpoint inferwidth_wmap (s : HiFP.hfstmt) (ce : CEP.env) (w : wmap): wmap :=
     ...
     (*possible bundle*)
     | Snode v e => if HiF.is_deftyp (type_of_hfexprP e ce)
                    then add_wmap v (type_of_hfexprP e ce) w else w
     ...
     | Sinst v1 v2 => if HiF.is_deftyp (type_of_refP (HiFP.eid v2) ce) then add_wmap v1 (type_of_refP (HiFP.eid v2) ce) w else w
     | Sskip
     | Sinvalid _
     | Smem _ _
