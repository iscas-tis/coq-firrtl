...
module MakeFirrtl =
 ...
 struct
  ...
  (** val sfcnct : Coq__1.fexpr -> Coq__1.fexpr -> Coq__2.fstmt **)
  ...
  let upd_typenv_fstmt s te _ =
    ...
    | Sfcnct (f, e2) ->
      (match f with
       | Eref v -> TE.add v (type_of_fexpr e2 te) te
       | _ -> te)
    ...
    ((fun fO fS n -> if n=0 then fO () else fS (n-1))
       ...
       | Sfcnct (f, e2) ->
         (match f with
              ...
              | Eref v1 ->
                ...
                then let v0 =
                     ...
                     (rs, v0)
                       ...
                       read_la
              | _ ->
                eval_fexpr e2 rs s te1 readerls writerls data2etc memmap
                  read_la
            in
            let memmap0 =
              if mylListIn v writerls
                      ...
                      (match TE.find midvar memmap with
                       | Some b ->
                         ...
                         then if eq_op bitseq_eqType (Obj.magic maskval)
                                   (Obj.magic (b1 :: []))
                              then TE.add midvar
                                     (memupd addrval (SV.acc v s) b) memmap
                              else TE.add midvar
                                     (memupd addrval (b0 :: []) b) memmap
                         else memmap
                       | None -> memmap)
                    | None -> memmap)
              else memmap
          ...
          | _ ->
            ((eval_noninst_fstmt st rs s te1 readerls writerls data2etc
               memmap read_la), fterss))
       | _ ->
         ((eval_noninst_fstmt st rs s te1 readerls writerls data2etc memmap
            read_la), fterss))
       iternum)
       ...
       | Some tseq ->
         let newstorder =
           List0.map (fun tv ->
             ...
             | Some tempst -> tempst
             | None ->
               if varIn tv instoutps
               then Sfcnct ((Eref tv), (Eref tv))
               else sskip) tseq
         in
  ...
  (** val well_typed_fstmt : fstmt -> TE.env -> bool **)

  let well_typed_fstmt s te =
    match s with
    | Swire (_, t0) -> leq (Pervasives.succ 0) (sizeof_fgtyp t0)
    | Sreg r ->
      (&&) (leq (Pervasives.succ 0) (sizeof_fgtyp r.coq_type))
        (match r.reset with
         | NRst -> true
         | Rst (e1, e2) ->
           (&&) (well_typed_fexpr e1 te) (well_typed_fexpr e2 te))
  ...
  let is_defined_fstmt s te =
    ...
    | Sfcnct (f, e) ->
      (match f with
       | Eref v -> (&&) (is_defined v te) (is_defined_fexpr e te)
       | _ -> true)
    | _ -> true
  ...
  (** val well_formed_fstmt : fstmt -> TE.env -> bool **)

  let well_formed_fstmt s te =
    (&&) (well_typed_fstmt s te) (is_defined_fstmt s te)
        ...
        (match f with
         | Eref v ->
           let newv =
             ...
             | Eref v1 ->
               if mylListIn v1 finstoutl
               then (match TE.find v1 finstoutm with
                     | Some p ->
                       let (a4, a0) = p in
                       (match TE.find a4 ffmodsmap with
                        | Some mst ->
                                             ...
                                             snd
                                               (fst
                                                 (fold_left
                                                   ...
                                                   let (y, tfterss) = pat in
                                                   let (trs, ts) = y in
                                                   no_mem_eval_fstmt tempst
                                                     trs ts te0 finstoutl0
                                                     finstoutm0 ffmodsmap
                                                     tfterss finstin
                                                     finstinmap0 finstoutmap0
                                                     allfinstoutl
                                                     allfinstoutm
  ...
  let no_mem_eval_fstmts st rs s te finstoutl finstoutm ffmodsmap fterss finstin finstinmap finstoutmap allfinstoutl allfinstoutm allfinstportmap iternum =
    fold_left (fun pat tempst ->
      let (y, tfterss) = pat in
      let (trs, ts) = y in
      no_mem_eval_fstmt tempst trs ts te finstoutl finstoutm ffmodsmap
        tfterss finstin finstinmap finstoutmap allfinstoutl allfinstoutm
        allfinstportmap iternum) st ((rs, s), fterss)
  ...
  let rec no_mem_eval_fstmts_inline st rs s te =
    ...
    | [] -> (rs, s)
    | hd :: tl ->
      let (rs0, s0) = no_mem_eval_noninst_fstmt hd rs s te in
      no_mem_eval_fstmts_inline tl rs0 s0 te
