...
forall (pp : seq HiFP.hfport)
       (vm_old vm_new : CEP.t vertex_type)
       (ct_old ct_new port_conn_map conn_map : CEP.t def_expr)
       (port_scope scope tmap : CEP.t (ftype * HiF.forient)),
    ports_have_fully_inferred_ground_types pp ->
    stmt_has_fully_inferred_ground_types s ->
    (* not sure whether we need the following or another assumption: *)
    tmap_has_fully_inferred_ground_types tmap ->
    CEP.submap port_scope tmap ->
    (* alternatively, one would have:
    (exists temp_tmap, ports_stmts_tmap pp ss = Some temp_tmap /\ CEP.submap temp_tmap tmap) *)
    ExpandPort_fun pp = Some (port_conn_map, port_scope) ->
    ExpandBranch_fun s (Qnil ProdVarOrder.T) port_conn_map port_scope = Some (component_stmt_of s, conn_map, scope) ->
    Sem_frag_stmts vm_old ct_old (Qcat (component_stmt_of s)
                                       (CEP.fold helper_connect conn_map (Qnil ProdVarOrder.T)))
                   vm_new ct_new tmap ->
    Sem_frag_stmt vm_old ct_old s vm_new ct_new tmap.
...
forall (pp : seq HiFP.hfport)
    ...
    (exists temp_tmap, ports_stmts_tmap pp ss = Some temp_tmap /\ CEP.submap temp_tmap tmap) *)
    ExpandPort_fun pp = Some (port_conn_map, port_scope) ->
    ExpandBranch_funs ss (Qnil ProdVarOrder.T) port_conn_map port_scope = Some (component_stmts_of ss, conn_map, scope) ->
