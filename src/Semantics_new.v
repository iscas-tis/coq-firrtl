...
with eval_hfstmt_for_unique_node s v e init_s tmap : 
  ...
  match s with
  | Swhen c ss1 ss2 => forall valc, Sem_HiFP.eval_hfexpr c init_s tmap = Some valc ->
...
Proof.
  clear 
  eval_hfstmts_for_unique_node_helper. move : ss; elim. simpl; intros; done.
  ...
  case Hst : s => [||var reg|||var node_e||ref|cond ss_true ss_false]; subst s.
  + (* skip *) simpl in Heval; inversion Heval; subst rs0 ns0. move : Hevals; apply (IH Hin); try done.
  + (* wire *) simpl in Heval; inversion Heval; subst rs0 ns0. move : Hevals; apply (IH Hin); try done.
  + (* reg *) simpl in Heval. destruct (PVM.find var init_s); try discriminate.
        inversion Heval; subst rs1 ns1. move : Hevals; apply (IH Hin); try done.
  + (* mem *) simpl in Heval; inversion Heval; subst rs0 ns0. move : Hevals; apply (IH Hin); try done.
  + (* inst *) simpl in Heval; inversion Heval; subst rs0 ns0. move : Hevals; apply (IH Hin); try done.
  + (* node *) admit.
  + (* cnct *) admit.
  + (* invalid *) admit.
  ...
  intro; move : Heval. apply (eval_hfstmts_for_unique_node_helper _ _ _ _ _ H0); try done. admit.
  intro; move : Heval. apply (eval_hfstmts_for_unique_node_helper _ _ _ _ _ H0); try done. admit.
Admitted.
...
Proof.
  ...
  1,2 : move : Heval; apply eval_hfstmts_notinss_findeq. *)
Admitted.
...
Proof. 
  elim. 
  - simpl; intros rs s rs0 s0 Hnotin Heval old_conn_map Hinit conn_map Hexpand_branches.
    inversion Heval; subst rs s; clear Heval. inversion Hexpand_branches; subst conn_map; clear Hexpand_branches. done.
  - intros st sts IH rs s rs0 s0 Hnotin Heval old_conn_map Hinit conn_map Hexpand_branches.
    ...
    move : conn_map Hexpand_branches. apply IH with (rs := rs) (rs0 := rs_temp) (s0 := s_temp); try done. split; move : Hnotin => [Hnotin0 Hnotin1].
    (* not in *)
    ...
    + (* when *)
      ...
      unfold combine_when_connections. rewrite PVM.map2_1.

with eval_hfstmt_ExpandBranches_funs_find_for_comb_helper s v init_s tmap :
  Sem_HiFP.eval_hfstmt s rs0 s0 init_s tmap = Some (rs_temp, s_temp) ->
  forall (old_conn_map : PVM.t def_expr),
    match PVM.find v old_conn_map with
    | Some (D_fexpr e') => PVM.find v s0 = Sem_HiFP.eval_hfexpr e' init_s tmap
    | Some (D_invalidated gt_e') => 
  ...
  match s with
  | Swhen c ss1 ss2 => ExpandBranches_funs ss_true old_conn_map tmap = Some true_conn_map -> 
                      ...
                      | Some (D_invalidated gt_e') =>
                              if sizeof_fgtyp gt_e' < length indeterminate_val
                              then PVM.find v s_temp = Some (take (sizeof_fgtyp gt_e') indeterminate_val)
                              else
                              PVM.find v s_temp =
                              Some (zext (sizeof_fgtyp gt_e' - length indeterminate_val) indeterminate_val)
                      | Some (D_fexpr e') => PVM.find v s_temp = Sem_HiFP.eval_hfexpr e' init_s tmap
                      | None => PVM.find v s_temp = None
                      end
  | _ => True
  end.
...
Proof.
  ...
  - (* combinational part *)
    ...
    * (* node : v的值在 component_stmts_of 中 *) intros bs Hfind1.
      specialize (tmap_find_node_qin Htmap Hcmpnt Hevalss1 Hfind1) as He. destruct He as [e He].
  ...
  - (* sequential part *)
    ...
    * (* register *) 
      ...
      assert (Hin : forall s, Qin s (component_stmts_of ss) -> is_declaration s). admit. 
