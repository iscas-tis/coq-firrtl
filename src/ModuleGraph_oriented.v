...
(* This file contains a simpler version of module graphs as a semantic structure for FIRRTL modules.
   ...
   Additionally this file tests for the correct orientation of components in connections. *)
...
Lemma subdomain_add {T : Type} :
forall (tmap : CEP.t T) (k : ProdVarOrder.T) (val : T),
    subdomain tmap (CEP.add k val tmap).
Proof.
unfold subdomain ; intros.
destruct (k0 == k) eqn: Hk0k.
* rewrite (CEP.Lemmas.find_add_eq Hk0k) //.
* rewrite (CEP.Lemmas.find_add_neq (elimT negP (negbT Hk0k))).
  destruct (CEP.find k0 tmap) ; done.
Qed.

Lemma submap_subdomain {T : Type} :
forall (tmap1 tmap2 : CEP.t T),
    CEP.submap tmap1 tmap2 -> subdomain tmap1 tmap2.
Proof.
unfold subdomain ; intros.
specialize (H k).
destruct (CEP.find k tmap1), (CEP.find k tmap2) ; try by done.
rewrite (H t) //.
Qed.

Lemma Equal_subdomain {T : Type} :
forall (tmap1 tmap2 : CEP.t T),
    CEP.Equal tmap1 tmap2 -> subdomain tmap1 tmap2.
Proof.
intros.
apply (submap_subdomain _ _ (CEP.Lemmas.Equal_submap H)).
Qed.
...
| Fflips v0 flp t ff' =>
    if v != v0 then type_of_ffield v ff' ori
         ...
         | Nflip, _
         | Flipped, HiF.Duplex => Some (t, ori)
         | Flipped, HiF.Source => Some (t, HiF.Sink)
         | Flipped, HiF.Sink => Some (t, HiF.Source)
         | _, _ => None
         end
end.

Fixpoint type_of_ref (ref : HiFP.href) (tmap : CEP.t (ftype * HiF.forient)) : option (ftype * HiF.forient) :=
...
* unfold ports_tmap, ports_names.
  ...
  intro ; contradict H.
  specialize (CEP.empty_1 (elt := ftype * HiF.forient) (a := y)) ; intro.
  specialize (CEP.find_2 (m := CEP.empty (ftype * HiF.forient)) (x := y)) ; intro.
  destruct (CEP.find y (CEP.empty (ftype * HiF.forient))) ; try by done.
  specialize (H p) ; specialize (H0 p Logic.eq_refl) ; by contradiction.
  ...
  + (* Sreg *)
    ...
    - split.
      ...
      * apply CEP.Lemmas.submap_none_add ; first by apply CEP.Lemmas.submap_refl.
        ...
        destruct (CEP.find s scope) ; last by trivial.
        by rewrite -Hfind (H p) //.
    - destruct (type_of_expr h0 scope) as [[[[[|[]]| | | | | |]| |] _]|] ; try by trivial.
      1: destruct (type_of_expr h1 (CEP.add s (newt, HiF.Duplex) scope)) ; last by trivial.
      2: destruct (type_of_expr h1 scope) ; last by trivial.
      1,2: split ;
            first by apply submap_add_add, H.
      1,2: split ;
            first by (apply CEP.Lemmas.submap_none_add ; first (by apply CEP.Lemmas.submap_refl) ;
                      exact Hfind).
      1,2: by apply CEP.Lemmas.submap_none_add ; first (by apply CEP.Lemmas.submap_refl) ;
              specialize (H s) ;
              destruct (CEP.find s scope) ; last (by trivial) ;
              rewrite -Hfind (H p) //.
  + (* Snode *)
...
Lemma stmts_tmap_cat :
   forall (vm : CEP.t vertex_type) (ss1 ss2 : HiFP.hfstmt_seq) (tmap_scope : CEP.t (ftype * HiF.forient) * CEP.t (ftype * HiF.forient)),
...
Lemma stmts_tmap_extend :
forall (vm : CEP.t vertex_type) (ss : HiFP.hfstmt_seq) (old_tmap1 old_tmap2 old_scope : CEP.t (ftype * HiF.forient)),
        CEP.submap old_scope old_tmap1
    ->
        CEP.submap old_tmap1 old_tmap2
    ->
        match stmts_tmap (old_tmap1, old_scope) ss vm, stmts_tmap (old_tmap2, old_scope) ss vm with
        | Some (new_tmap1, new_scope1), Some (new_tmap2, new_scope2) =>
                new_scope1 = new_scope2
            /\
                forall k : CEP.key,
                    match CEP.find k old_tmap1, CEP.find k old_tmap2, CEP.find k new_tmap1, CEP.find k new_tmap2 with
                    | None, None, None, None => True (* nowhere defined *)
                    | None, None, Some e, Some f => e = f (* defined in ss *)
                    | None, Some e, None, Some f => e = f (* in old_tmap2 *)
                    | Some e, Some f, Some g, Some h => e = f /\ e = g /\ e = h (* in old_tmap1 *)
                    | _, _, _, _ => False
                    end
        | None, Some _ => False
        | _, _ => True
        end
with stmt_tmap_extend :
forall (vm : CEP.t vertex_type) (s : HiFP.hfstmt) (old_tmap1 old_tmap2 old_scope : CEP.t (ftype * HiF.forient)),
        CEP.submap old_scope old_tmap1
    ->
        CEP.submap old_tmap1 old_tmap2
    ->
        match stmt_tmap (old_tmap1, old_scope) s vm, stmt_tmap (old_tmap2, old_scope) s vm with
        | Some (new_tmap1, new_scope1), Some (new_tmap2, new_scope2) =>
                new_scope1 = new_scope2
            /\
                forall k : CEP.key,
...
Proof.
* clear stmts_tmap_extend.
  induction ss as [|s ss] ; simpl ; intros old_tmap1 old_tmap2 old_scope Hsc_sub_1 H1_sub_2.
  + (* Qnil *)
    split ; first by reflexivity.
    intro k ; specialize (H1_sub_2 k).
    destruct (CEP.find k old_tmap1) ;
        first by rewrite (H1_sub_2 _ Logic.eq_refl) //.
    destruct (CEP.find k old_tmap2) ; done.
  + (* Qcons *)
    rename stmt_tmap_extend into IHs.
    specialize (IHs vm s old_tmap1 old_tmap2 old_scope Hsc_sub_1 H1_sub_2).
    generalize (stmt_submap vm s old_tmap1 old_scope Hsc_sub_1) ; intro.
    destruct (stmt_tmap (old_tmap1, old_scope) s vm) as [[s_tmap1 s_scope1]|],
             (stmt_tmap (old_tmap2, old_scope) s vm) as [[s_tmap2 s_scope2]|] ;
          try done ;
          last by destruct (stmts_tmap (s_tmap1, s_scope1) ss vm) as [[]|] ; done.
    destruct IHs as [IHs' IHs] ; subst s_scope2.
    assert (CEP.submap s_tmap1 s_tmap2).
          intros k v H1 ; specialize (IHs k).
          rewrite H1 in IHs.
          destruct (CEP.find k old_tmap1) as [o1|],
                   (CEP.find k old_tmap2) as [o2|],
                   (CEP.find k s_tmap2) as [s2|] ; try done.
          - destruct IHs as [_ [H2 H3]] ; subst o1 s2 ; reflexivity.
          - subst s2 ; reflexivity.
    specialize (IHss s_tmap1 s_tmap2 s_scope1 (proj1 H) H0).
    generalize (stmts_submap vm ss s_tmap1 s_scope1 (proj1 H)) ; intro.
    destruct (stmts_tmap (s_tmap1, s_scope1) ss vm) as [[new_tmap1 new_scope1]|],
             (stmts_tmap (s_tmap2, s_scope1) ss vm) as [[new_tmap2 new_scope2]|] ;
          try done.
    destruct IHss as [IHss' IHss] ; subst new_scope2.
    split ; first by reflexivity.
    intro k ; specialize (H1_sub_2 k) ; specialize (IHs k) ; specialize (IHss k).
    destruct (CEP.find k old_tmap1) as [o1|],
             (CEP.find k old_tmap2) as [o2|],
             (CEP.find k s_tmap1) as [s1|],
             (CEP.find k s_tmap2) as [s2|],
             (CEP.find k new_tmap1) as [n1|],
             (CEP.find k new_tmap2) as [n2|] ; try done.
    - 1,3: destruct IHss as [H2 [H3 H4]] ; subst s1 s2 n2 ; done.
    - subst n2 ; exact IHs.
* clear stmt_tmap_extend.
  induction s as [|var ft|var reg|var mem|var1 var2|var expr|ref expr|ref|cond sst ssf] ;
        simpl ; intros old_tmap1 old_tmap2 old_scope Hsc_sub_1 H1_sub_2.
  + (* Sskip *)
    split ; first by reflexivity.
    intro k ; specialize (H1_sub_2 k).
    destruct (CEP.find k old_tmap1) as [o1|],
             (CEP.find k old_tmap2) as [o2|] ; try done.
    - specialize (H1_sub_2 _ Logic.eq_refl) ; injection H1_sub_2 ; clear H1_sub_2 ; intros ; subst o2 ; done.
    - specialize (H1_sub_2 _ Logic.eq_refl) ; discriminate H1_sub_2.
  + (* Swire *)
    generalize (H1_sub_2 var) ; intro Hvar.
    destruct (CEP.find var old_tmap1) eqn: Hvar1 ;
          first by specialize (Hvar _ Logic.eq_refl) ; rewrite Hvar //.
    destruct (code_type_find_vm_widths ft var vm) as [[newt _]|],
             (CEP.find var old_tmap2) eqn: Hvar2 ; try done.
    split ; first by reflexivity.
    intro k.
    destruct (k == var) eqn: Hkvar ;
          first by rewrite (CEP.Lemmas.find_add_eq Hkvar) (CEP.Lemmas.find_add_eq Hkvar)
                           (elimT eqP Hkvar) Hvar1 Hvar2 //.
    apply negbT in Hkvar.
    move /negP : Hkvar => Hkvar.
    rewrite (CEP.Lemmas.find_add_neq Hkvar) (CEP.Lemmas.find_add_neq Hkvar).
    specialize (H1_sub_2 k).
    destruct (CEP.find k old_tmap1) as [o1|],
             (CEP.find k old_tmap2) as [o2|] ; try done.
    - specialize (H1_sub_2 _ Logic.eq_refl) ; injection H1_sub_2 ; clear H1_sub_2 ; intros ; subst o2 ; done.
    - specialize (H1_sub_2 _ Logic.eq_refl) ; discriminate H1_sub_2.
  + (* Sreg *)
    ...
    destruct (CEP.find var old_tmap1) eqn: Hvar1 ;
          first by specialize (Hvar _ Logic.eq_refl) ; rewrite Hvar //.
    destruct (type_of_expr (clock reg) old_scope) ;
          last by destruct (CEP.find var old_tmap2) ; done.
    destruct (code_type_find_vm_widths (type reg) var vm) as [[newt _]|] ;
          last by destruct (CEP.find var old_tmap2) ; done.
    destruct (reset reg) as [|rst_sig rst_val] ; first last.
    - destruct (type_of_expr rst_sig old_scope) as [[[[[|[|]]| | | | | |]| |] _]|] ;
            try (by destruct (CEP.find var old_tmap2) ; done).
      1: destruct (type_of_expr rst_val (CEP.add var (newt, HiF.Duplex) old_scope)) ;
              last by destruct (CEP.find var old_tmap2) ; done.
      2: destruct (type_of_expr rst_val old_scope) ;
              last by destruct (CEP.find var old_tmap2) ; done.
    1-3: destruct (CEP.find var old_tmap2) eqn: Hvar2 ; first by done.
    1-3: split ; first by reflexivity.
    1-3: intro k.
    1-3: destruct (k == var) eqn: Hkvar ;
          first by rewrite (CEP.Lemmas.find_add_eq Hkvar) (CEP.Lemmas.find_add_eq Hkvar)
                           (elimT eqP Hkvar) Hvar1 Hvar2 //.
    1-3: apply negbT in Hkvar.
    1-3: move /negP : Hkvar => Hkvar.
    1-3: rewrite (CEP.Lemmas.find_add_neq Hkvar) (CEP.Lemmas.find_add_neq Hkvar).
    1-3: specialize (H1_sub_2 k).
    1-3: destruct (CEP.find k old_tmap1) as [o1|],
             (CEP.find k old_tmap2) as [o2|] ; try done.
    - 1,3,5: specialize (H1_sub_2 _ Logic.eq_refl) ; injection H1_sub_2 ; clear H1_sub_2 ; intros ; subst o2 ; done.
    - 1-3: specialize (H1_sub_2 _ Logic.eq_refl) ; discriminate H1_sub_2.
  + (* Smem *)
    by trivial.
  + (* Sinst *)
    by trivial.
  + (* Snode *)
    ...
    destruct (CEP.find var old_tmap1) eqn: Hvar1 ;
          first by specialize (Hvar _ Logic.eq_refl) ; rewrite Hvar //.
    destruct (type_of_expr expr old_scope) as [[newt _]|],
             (CEP.find var old_tmap2) eqn: Hvar2 ; try by done.
    split ; first by reflexivity.
    intro k.
    destruct (k == var) eqn: Hkvar ;
          first by rewrite (CEP.Lemmas.find_add_eq Hkvar) (CEP.Lemmas.find_add_eq Hkvar)
    ...
    - specialize (H1_sub_2 _ Logic.eq_refl) ; discriminate H1_sub_2.
  + (* Sfcnct *)
    destruct (type_of_ref ref old_scope) ; last by done.
    destruct (type_of_expr expr old_scope) ; last by done.
    split ; first by reflexivity.
    intro k ; specialize (H1_sub_2 k).
    destruct (CEP.find k old_tmap1) as [o1|],
             (CEP.find k old_tmap2) as [o2|] ; try done.
  ...
  + (* Sinvalid *)
    ...
    - specialize (H1_sub_2 _ Logic.eq_refl) ; discriminate H1_sub_2.
  + (* Shwen *)
    destruct (type_of_expr cond old_scope) ; last by done.
    rename stmts_tmap_extend into IHsst.
    generalize (IHsst vm ssf) ; intro IHssf.
    specialize (IHsst vm sst old_tmap1 old_tmap2 old_scope Hsc_sub_1 H1_sub_2).
    generalize (stmts_submap vm sst old_tmap1 old_scope Hsc_sub_1) ; intro Hsc_sub_tr1.
    destruct (stmts_tmap (old_tmap1, old_scope) sst vm) as [[true_tmap1 true_scope1]|],
             (stmts_tmap (old_tmap2, old_scope) sst vm) as [[true_tmap2 true_scope2]|] ;
          try done ;
          last by destruct (stmts_tmap (true_tmap1, old_scope) ssf vm) as [[]|] ; done.
    destruct IHsst as [IHsst' IHsst] ; subst true_scope2.
    assert (CEP.submap true_tmap1 true_tmap2).
          intros k v H1 ; specialize (IHsst k).
          rewrite H1 in IHsst.
          destruct (CEP.find k old_tmap1) as [o1|],
                   (CEP.find k old_tmap2) as [o2|],
                   (CEP.find k true_tmap2) as [t2|] ; try done.
          - destruct IHsst as [_ [H2 H3]] ; subst o1 t2 ; reflexivity.
          - subst t2 ; reflexivity.
    specialize (IHssf true_tmap1 true_tmap2 old_scope (CEP.Lemmas.submap_trans Hsc_sub_1 (proj1 (proj2 Hsc_sub_tr1))) H).
    generalize (stmts_submap vm ssf true_tmap1 old_scope (CEP.Lemmas.submap_trans Hsc_sub_1 (proj1 (proj2 Hsc_sub_tr1)))) ; intro H1.
    destruct (stmts_tmap (true_tmap1, old_scope) ssf vm) as [[false_tmap1 false_scope1]|],
             (stmts_tmap (true_tmap2, old_scope) ssf vm) as [[false_tmap2 false_scope2]|] ;
          try done.
    destruct IHssf as [IHssf' IHssf] ; subst false_scope2.
    split ; first by reflexivity.
    intro k ; specialize (IHsst k) ; specialize (IHssf k).
    destruct (CEP.find k old_tmap1) as [o1|],
             (CEP.find k old_tmap2) as [o2|],
             (CEP.find k true_tmap1) as [t1|],
             (CEP.find k true_tmap2) as [t2|],
             (CEP.find k false_tmap1) as [f1|],
             (CEP.find k false_tmap2) as [f2|] ; try done.
    - 1,3: destruct IHssf as [H2 [H3 H4]] ; subst t1 t2 f2 ; done.
    - subst f2 ; exact IHsst.
Qed.

(*---------------------------------------------------------------------------*)
(* Properties of type_of_ref and of type_of_expr                             *)

Lemma type_of_ref_submap : forall (ref : HiFP.href) (tmap1 tmap2 : CEP.t (ftype * HiF.forient)),
...
* specialize (H s).
  ...
  specialize (H p Logic.eq_refl).
  exact H.
* destruct (type_of_ref ref tmap1) ; last by trivial.
  rewrite IHref.
  destruct p as [[| |]] ; first (by trivial) ; first by trivial.
  destruct (type_of_ffield v f) ; last by trivial.
  by reflexivity.
* destruct (type_of_ref ref tmap1) ; last by trivial.
  rewrite IHref.
  destruct p as [[| |]] ; first (by trivial) ; last by trivial.
  destruct (n < n0) ; last by trivial.
  by reflexivity.
Qed.

Lemma type_of_expr_submap : forall (expr : HiFP.hfexpr) (tmap1 tmap2 : CEP.t (ftype * HiF.forient)),
   CEP.submap tmap1 tmap2 ->
...
Lemma type_of_expr_add : forall (expr : HiFP.hfexpr) (tmap1 tmap2 : CEP.t (ftype * HiF.forient)) (x : CEP.key) (y : ftype * HiF.forient),
    CEP.submap tmap1 tmap2 ->
    CEP.find x tmap2 = None ->
    type_of_expr expr tmap2 ->
    type_of_expr expr (CEP.add x y tmap1) = type_of_expr expr tmap1
with type_of_ref_add : forall (ref : HiFP.href) (tmap1 tmap2 : CEP.t (ftype * HiF.forient)) (x : CEP.key) (y : ftype * HiF.forient),
    CEP.submap tmap1 tmap2 ->
    CEP.find x tmap2 = None ->
    type_of_ref ref tmap2 ->
    type_of_ref ref (CEP.add x y tmap1) = type_of_ref ref tmap1.
Proof.
* clear type_of_expr_add.
  intros.
  induction expr ; simpl ; simpl in H1.
  + (* Econst *)
    by reflexivity.
  + (* Ecast *)
    destruct u, (type_of_expr expr tmap2) as [[[[| | | | | |]| |]]|] ;
          try done ;
          rewrite (IHexpr Logic.eq_refl) //.
  + (* Eunop *)
    destruct e, (type_of_expr expr tmap2) as [[[[| | | | | |]| |]]|] ;
          try done ;
          rewrite (IHexpr Logic.eq_refl) //.
  + (* Ebinop *)
    destruct e, (type_of_expr expr1 tmap2) as [[[[| | | | | |]| |]]|] ;
          try done ;
          destruct (type_of_expr expr2 tmap2) as [[[[| | | | | |]| |]]|] ;
          try done ;
          rewrite (IHexpr1 Logic.eq_refl) // (IHexpr2 Logic.eq_refl) //.
  + (* Emux *)
    destruct (type_of_expr expr1 tmap2) as [[[[[|[]]| | | | | |]| |]]|] ;
          try done.
    rewrite (IHexpr1 Logic.eq_refl) //.
    destruct (type_of_expr expr2 tmap2) as [[[[| | | | | |]| |]]|] ;
          try done ;
          destruct (type_of_expr expr3 tmap2) as [[[[| | | | | |]| |]]|] ;
          try done ;
          rewrite (IHexpr2 Logic.eq_refl) // (IHexpr3 Logic.eq_refl) //.
  + (* Evalidif *)
    destruct (type_of_expr expr1 tmap2) as [[[[[|[]]| | | | | |]| |]]|] ;
          try done.
    rewrite (IHexpr1 Logic.eq_refl) //.
    destruct (type_of_expr expr2 tmap2) as [[[[| | | | | |]| |]]|] ;
          try done ;
          rewrite (IHexpr2 Logic.eq_refl) //.
  + (* Eref *)
    specialize (type_of_ref_add h tmap1 tmap2 x y H H0).
    destruct (type_of_ref h tmap2) ;
          try done ;
          rewrite type_of_ref_add //.
* clear type_of_ref_add.
  intros.
  induction ref ; simpl ; simpl in H1.
  + (* Eid *)
    destruct (eqVneq s x).
    - rewrite e H0 // in H1.
    - move /negP : i => i.
      rewrite (CEP.Lemmas.find_add_neq i) //.
  + (* Esubfield *)
    destruct (type_of_ref ref tmap2) ; last by discriminate H1.
    rewrite IHref //.
  + (* Esubindex *)
    destruct (type_of_ref ref tmap2) ; last by discriminate H1.
    rewrite IHref //.
...
Lemma stmts_tmap_components :
   forall (vm : CEP.t vertex_type) (ss : HiFP.hfstmt_seq) (tmap scope1 scope2 : CEP.t (ftype * HiF.forient)),
...
with stmt_tmap_components :
   forall (vm : CEP.t vertex_type) (s : HiFP.hfstmt) (tmap scope1 scope2 : CEP.t (ftype * HiF.forient)),
      CEP.submap scope1 tmap ->
      CEP.submap scope2 tmap ->
      CEP.submap scope1 scope2 ->
  ...
  + (* Sreg *)
    ...
    - rewrite H2.
      ...
      * rewrite CEP.Lemmas.find_add_neq ; last by rewrite /HiFirrtl.PVM.SE.eq Hvs //.
        rewrite CEP.Lemmas.find_add_neq ; last by rewrite /HiFirrtl.PVM.SE.eq Hvs //.
        by apply H1.
    - destruct (type_of_expr h0 scope1) as [[[[[|[]]| | | | | |]| |] p]|] eqn: Hte0 ; try by trivial.
      1,2: generalize (type_of_expr_submap h0 scope1 scope2 H1) ;
            intro ; rewrite Hte0 in H3.
      1: destruct (type_of_expr h1 (CEP.add s (newt, HiF.Duplex) scope1)) eqn: Hte1 ; last (by trivial) ;
         generalize (type_of_expr_submap h1 (CEP.add s (newt, HiF.Duplex) scope1) (CEP.add s (newt, HiF.Duplex) scope2)
                                         (submap_add_add scope1 scope2 s (newt, HiF.Duplex) H1)) ;
               intro ; rewrite Hte1 in H4.
      2: destruct (type_of_expr h1 scope1) eqn: Hte1 ; last (by trivial) ;
         generalize (type_of_expr_submap h1 scope1 scope2 H1) ;
               intro ; rewrite Hte1 in H4.
      1,2: rewrite H2 H3 H4.
      1,2: split ; first by reflexivity.
      1,2: intro.
      1,2: destruct (k == s) eqn: Hvs.
      * 1,3: by rewrite CEP.Lemmas.find_add_eq ; last (by rewrite /HiFirrtl.PVM.SE.eq Hvs //) ;
                rewrite CEP.Lemmas.find_add_eq ; last (by rewrite /HiFirrtl.PVM.SE.eq Hvs //) ;
                done.
      * 1,2: by rewrite CEP.Lemmas.find_add_neq ; last (by rewrite /HiFirrtl.PVM.SE.eq Hvs //) ;
                rewrite CEP.Lemmas.find_add_neq ; last (by rewrite /HiFirrtl.PVM.SE.eq Hvs //) ;
                apply H1.
  + (* Snode *)
    destruct (CEP.find s tmap) ; first by trivial.
...
Fixpoint select_field (f : VarOrder.t) (l : seq CEP.key) (ff : ffield) (ori : HiF.forient) : option (seq CEP.key * (ftype * HiF.forient)) :=
...
| Fflips v flp ft ff' =>
    ...
    if size l < elsize then None
              ...
              | Nflip, _
              | Flipped, HiF.Duplex => Some (take elsize l, (ft, ori))
              | Flipped, HiF.Source => Some (take elsize l, (ft, HiF.Sink))
              | Flipped, HiF.Sink => Some (take elsize l, (ft, HiF.Source))
              | Flipped, _ => None
              end
| _ => None
end.
...
forall (v : VarOrder.T) (ff : ffield) (input_list : seq CEP.key) (ori : HiF.forient),
   size_of_fields ff <= size input_list ->
      match select_field v input_list ff ori with
      | Some (output_list, (ft, _)) => size output_list = size_of_ftype ft
      | None => True
      end.
Proof.
...
rewrite H0.
destruct (v != v0).
* specialize (IHff (drop (size_of_ftype f0) input_list) ori).
  destruct (select_field v (drop (size_of_ftype f0) input_list) ff ori) as [[output_list [ff' _]]|] ; last by trivial.
  apply IHff.
  rewrite size_drop leq_subRL ; first by exact H.
  rewrite leqNgt H0 //.
* destruct f, ori ; try done.
  all: rewrite size_take.
  all: destruct (size_of_ftype f0 < size input_list) eqn: H1 ; first by reflexivity.
  all: apply anti_leq.
  all: rewrite leqNgt H1 andTb leqNgt H0 //.
Qed.

Lemma list_rhs_ref_p_ffield :
forall (v : VarOrder.T) (ff : ffield) (ori : HiF.forient) (input_list : seq CEP.key),
   match type_of_ffield v ff ori, select_field v input_list ff ori with
   | Some (type_ff, type_ori), Some (_, (list_ff, list_ori)) => type_ff = list_ff /\ type_ori = list_ori
...
* specialize (IHff ori).
  destruct (type_of_ffield v ff ori) as [[type_ff type_ori]|].
  + destruct (size input_list < size_of_ftype f0) eqn: Hshort.
    - apply ltn_addr.
      rewrite Hshort //.
    - specialize (IHff (drop (size_of_ftype f0) input_list)).
      destruct (select_field v (drop (size_of_ftype f0) input_list) ff ori) as [[t [list_ff list_ori]]|] eqn: Hselect ;
            first by exact IHff.
  ...
  + destruct (size input_list < size_of_ftype f0) eqn: Hshort ;
          first by trivial.
    specialize (IHff (drop (size_of_ftype f0) input_list)).
    destruct (select_field v (drop (size_of_ftype f0) input_list) ff ori) as [[t list_ff]|] eqn: Hselect ;
          done.
* destruct (size input_list < size_of_ftype f0) eqn: Hshort ;
        last by destruct f, ori ; simpl ; done.
  destruct f, ori ; simpl ; try done ;
  apply ltn_addr ;
  rewrite Hshort //.
Qed.

Fixpoint list_lhs_ref_p (ref : HiFP.href) (tmap : CEP.t (ftype * HiF.forient)) : option (seq CEP.key * (ftype * HiF.forient)) :=
...
forall (tmap : CEP.t (ftype * HiF.forient)) (ref : HiFP.href),
match list_lhs_ref_p ref tmap with
| Some (input_list, (ft, _)) => size input_list = size_of_ftype ft
| None => True
end.
Proof.
induction ref ; simpl ; try done.
* destruct (CEP.find s tmap) as [[ft _]|] ; last by trivial.
  rewrite size_mkseq //.
* destruct (list_lhs_ref_p ref tmap) as [[input_list [[| |ff] ori]]|] ; try apply IHref ; try done.
  generalize (select_field_size v ff input_list ori) ; intro.
  destruct (select_field v input_list ff ori) as [[output_list [ft _]]|]; last by trivial.
  apply H, eq_leq.
  rewrite IHref //.
* destruct (list_lhs_ref_p ref tmap) as [[input_list [[|t m|] ori]]|] ; try by trivial.
...
* (* Eid *)
  destruct (CEP.find s tmap) as [[]|] ; by done.
* (* Esubfield *)
  destruct (type_of_ref ref tmap) as [[type_ft type_ori]|] ;
        last by destruct (list_lhs_ref_p ref tmap) as [[]|] ; done.
  generalize (list_lhs_ref_p_size tmap ref) ; intro.
  destruct (list_lhs_ref_p ref tmap) as [[input_list [list_ft list_ori]]|] ; last by done.
  injection IHref ; clear IHref ; intros ; subst type_ft type_ori.
  destruct (list_ft) ; try done.
  generalize (list_rhs_ref_p_ffield v f list_ori input_list) ; intro.
  simpl size_of_ftype in H.
  destruct (type_of_ffield v f list_ori) as [[type_ff type_ori]|] eqn: Hfield ; try done.
  destruct (select_field v input_list f list_ori) as [[_ [list_ft0 list_ori0]]|] ;
        last by rewrite H ltnn // in H0.
  by rewrite (proj1 H0) (proj2 H0) //.
* destruct (type_of_ref ref tmap) as [[type_ft type_ori]|] ;
        last by destruct (list_lhs_ref_p ref tmap) as [[]|] ; done.
  generalize (list_lhs_ref_p_size tmap ref) ; intro.
  destruct (list_lhs_ref_p ref tmap) as [[input_list [list_ft list_ori]]|] ; try done.
  injection IHref ; clear IHref ; intros ; subst type_ft type_ori.
  destruct (list_ft) ; try done.
  simpl size_of_ftype in H.
...
Fixpoint Sem_frag_ports (vm_old : CEP.t vertex_type) (ct_old : CEP.t def_expr) (pp : seq HiFP.hfport) (vm_new : CEP.t vertex_type) (ct_new : CEP.t def_expr) (tmap : CEP.t (ftype * HiF.forient)) : Prop :=
...
destruct a.
1,2: destruct (CEP.find s tmap) as [[f0 _]|] ; last by contradiction H0.
1,2: split.
1-4: intro.
...
destruct a.
1,2: destruct (CEP.find s tmap) as [[f0 _]|] ; last by contradiction H1.
...
* clear connect_subdomain.
  ...
  + (* Gtyp *)
    ...
    1,2: destruct flipped ; move /andP : H => [/andP [_ /eqP H] /orP H4] ; try (by rewrite H //).
    1,2: destruct H4 as [H4|H4] ; move /eqP : H4 => H4 ;
          first by (subst el_src el_tgt ; rewrite H //).
...
* clear connect_preserves_compatibility.
  ...
  + (* Gtyp *)
    ...
    destruct lst_src as [|el_src [|]] ; try by done.
    destruct H3 ; rewrite mem_seq1 in H3 ; move /eqP : H3 => H3 ; rewrite -H3 in H.
    1,2: destruct flipped ; move /andP : H => [/andP [/eqP H4 /eqP H5] /orP H].
    1-4: destruct H as [H|H] ; move /eqP : H => H ;
               try (by subst el_src el_tgt ; rewrite H5 ; split ; done) ;
               try (by rewrite H //) ;
               (by rewrite H5 ; split ; done).
  + (* Atyp *)
    specialize (IHft'_tgt ft'_src).
    move /andP : H => [/eqP H4 /forallP H] ; rewrite -H4 in H0, H2 ; clear n0 H4.
    rewrite eq_refl andTb in H0.
...
Fixpoint Sem_frag_stmt (vm_old : CEP.t vertex_type) (ct_old : CEP.t def_expr) (s : HiFP.hfstmt) (vm_new : CEP.t vertex_type) (ct_new : CEP.t def_expr) (tmap : CEP.t (ftype * HiF.forient)) : Prop :=
   ...
   | Sfcnct ref_tgt (Eref ref_src) => (* allow non-passive types *)
       ...
       /\
          ...
          | Some (lst_tgt, (ft_tgt, HiF.Duplex))
          | Some (lst_tgt, (ft_tgt, HiF.Sink)) =>
              match list_lhs_ref_p ref_src tmap with
              | Some (lst_src, (ft_src, HiF.Duplex))
              | Some (lst_src, (ft_src, HiF.Source)) =>
...
forall (ss : HiFP.hfstmt_seq) (vm_old1 vm_old2 : CEP.t vertex_type) (ct_old1 ct_old2 : CEP.t def_expr) (vm_new1 vm_new2 : CEP.t vertex_type) (ct_new1 ct_new2 : CEP.t def_expr) (tmap1 tmap2 : CEP.t (ftype * HiF.forient)),
...
forall (s : HiFP.hfstmt) (vm_old1 vm_old2 : CEP.t vertex_type) (ct_old1 ct_old2 : CEP.t def_expr) (vm_new1 vm_new2 : CEP.t vertex_type) (ct_new1 ct_new2 : CEP.t def_expr) (tmap1 tmap2 : CEP.t (ftype * HiF.forient)),
    CEP.Equal vm_old1 vm_old2 ->
    CEP.Equal ct_old1 ct_old2 ->
    CEP.Equal vm_new1 vm_new2 ->
    CEP.Equal ct_new1 ct_new2 ->
...
* clear Sem_frag_stmt_compatible.
  ...
  + (* Sfcnct *)
    ...
    1,2: destruct (type_of_expr (Econst ProdVarOrder.T f b) tmap)
         as [[ft_expr _]|] ; last by done.
    3,4: destruct (type_of_expr (Ecast u h0) tmap)
         as [[ft_expr _]|] ; last by done.
    5,6: destruct (type_of_expr (Eprim_unop e h0) tmap)
         as [[ft_expr _]|] ; last by done.
    7,8: destruct (type_of_expr (Eprim_binop e h0_1 h0_2) tmap)
         as [[ft_expr _]|] ; last by done.
    9,10: destruct (type_of_expr (Emux h0_1 h0_2 h0_3) tmap)
          as [[ft_expr _]|] ; last by done.
    11,12: destruct (type_of_expr (Evalidif h0_1 h0_2) tmap)
           as [[ft_expr _]|] ; last by done.
    1-12: destruct H0.
    1-12: apply connect_type_compatible_size in H0.
    ...
    3,4: generalize (list_rhs_expr_p_size ft_expr (Ecast u h0)) ; intro ;
         destruct (list_rhs_expr_p (Ecast u h0) ft_expr) as [expr_list|] ; last by done.
    5,6: generalize (list_rhs_expr_p_size ft_expr (Eprim_unop e h0)) ; intro ;
         destruct (list_rhs_expr_p (Eprim_unop e h0) ft_expr) as [expr_list|] ; last by done.
    7,8: generalize (list_rhs_expr_p_size ft_expr (Eprim_binop e h0_1 h0_2)) ; intro ;
         destruct (list_rhs_expr_p (Eprim_binop e h0_1 h0_2) ft_expr) as [expr_list|] ; last by done.
    9,10: generalize (list_rhs_expr_p_size ft_expr (Emux h0_1 h0_2 h0_3)) ; intro ;
          destruct (list_rhs_expr_p (Emux h0_1 h0_2 h0_3) ft_expr) as [expr_list|] ; last by done.
    11,12: generalize (list_rhs_expr_p_size ft_expr (Evalidif h0_1 h0_2)) ; intro ;
           destruct (list_rhs_expr_p (Evalidif h0_1 h0_2) ft_expr) as [expr_list|] ; last by done.
    1-12: split ; intro ; last (intro k ; specialize (H4 k)).
    1-24: specialize (H k) ; try rewrite -H.
    1-24: destruct (k \in input_list) eqn: Hv;
          last by (destruct H2 as [_ H2] ;
                   specialize (H2 k) ; rewrite Hv in H2 ;
                   rewrite -H2 ; destruct (CEP.find k ct_old) ; done).
    1-24: destruct H2 as [H2 _] ;
          specialize (H2 (index k input_list)).
    1-24: generalize (nth_index k Hv) ; intro Hnth_index ;
          ...
          rewrite H1 H0 -H3 in Hv ;
          apply List.nth_error_Some in Hv.
    1-24: destruct (List.nth_error input_list (index k input_list)) eqn: Hi ; last (by done).
    1-24: destruct (List.nth_error expr_list (index k input_list)) ; last (by done).
    1-24: replace k0 with k in H2
    ...
    1-24: rewrite (proj2 H2) //.
    1-12: destruct (CEP.find k vm_old) as [[]|] ; try (by done).
    1-36: rewrite H4 in H2 ; by contradiction (proj1 H2).
    ...
    destruct (list_lhs_ref_p h tmap) as [[lst_tgt [ft_tgt [| | | |]]]|] eqn: Hhhh ;
          last (by contradiction H0) ;
          try (by destruct (list_lhs_ref_p h0 tmap)
                  as [[_ [_ []]]|]; contradiction H0).
    1,2: generalize (list_lhs_ref_p_size tmap h0) ; intro.
    1,2: generalize (list_lhs_ref_p_type tmap h0) ; intro.
    1,2: destruct (list_lhs_ref_p h0 tmap) as [[lst_src [ft_src []]]|] ; try by contradiction H0.
    1-4: destruct H0 as [H0 [H5 H6]].
    (*apply connect_type_compatible_size in H1.*)
    (*rewrite H1 -H4 in H2. ?? *)
    1-4: split ; intro ; last (intro k ; specialize (H k) ; rewrite -H ; specialize (H7 k)).
    1-8: clear H vm_new.
    1-8: specialize (H6 k).
    1-8: destruct ((k \in lst_tgt) || (k \in lst_src)) eqn: Hv ; rewrite Hv in H6 ;
          last by (rewrite -H6 ; destruct (CEP.find k ct_old) ; done).
    1-8: clear H6 ; move /orP : Hv => Hv.
    ...
    - 1-4: destruct (CEP.find k vm_old) as [[]|] ; try apply H, H7 ;
...
Definition ports_stmt_tmap (pp : seq HiFP.hfport) (s : HiFP.hfstmt) (vm : CEP.t vertex_type) : option (CEP.t (ftype * HiF.forient)) :=
match ports_tmap pp vm with
| Some pmap =>
    match stmt_tmap (pmap, pmap) s vm with
    | Some (tmap, _) => Some tmap
    | None => None
    end
| None => None
