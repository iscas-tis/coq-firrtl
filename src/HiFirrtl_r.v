...
Section HiFirrtl.
  ...
  with hfstmt_seq : Type :=
       | Qnil
       | Qrcons : hfstmt_seq -> hfstmt -> hfstmt_seq.
   ...
   match s with Qnil => default
              | Qrcons tl h => Qhead h tl end.

   Fixpoint Qcat (s1 s2 : hfstmt_seq) : hfstmt_seq :=
   match s2 with Qnil => s1
               | Qrcons h2 t2 => Qrcons (Qcat s1 h2) t2 end.

   Lemma Qcat0s : forall s : hfstmt_seq, Qcat Qnil s = s.
   Proof.
   induction s ; try done.
   rewrite /Qcat -/Qcat IHs ; done.
   Qed.

   Lemma QcatA : forall (u t s : hfstmt_seq), Qcat (Qcat s t) u = Qcat s (Qcat t u).
   induction u ; try done.
   intros ; rewrite /Qcat -/Qcat IHu //.
   Qed.

   Fixpoint Qcons (first : hfstmt) (ss : hfstmt_seq) : hfstmt_seq :=
   match ss with Qnil => Qrcons Qnil first
               | Qrcons head tail => Qrcons (Qcons first head) tail end.

   Lemma Qcat1s : forall (x : hfstmt) (s : hfstmt_seq),
      Qcat (Qcons x Qnil) s = Qcons x s.
   Proof.
   induction s ; try done.
   rewrite /Qcat -/Qcat IHs //.
   Qed.

   Lemma Qcats1 : forall (s : hfstmt_seq) (x : hfstmt),
      Qcat s (Qcons x Qnil) = Qrcons s x.
   Proof.
   done.
   Qed.
   ...
   Section Qfold.
   Context {R : Type}.
   Variable f : R -> hfstmt -> R.

   Fixpoint Qfoldl (default : R) (s : hfstmt_seq) :=
   match s with Qnil => default
              | Qrcons hd tl => f (Qfoldl default hd) tl end.

(* Fixpoint Qfoldr {R : Type} (g : hfstmt -> R -> R) (default : R) (s : hfstmt_seq) :=
   match s with Qnil => default
              | Qcons h tl => g h (Qfoldr g default tl) end. *)
   End Qfold.
...
(* Fixpoint Qcatrev (s1 s2 : hfstmt_seq) : hfstmt_seq := (* calculates the reversal of s1, followed by s2 *)
   ...
   Lemma Qcat_rcons : forall (s t : hfstmt_seq) (x : hfstmt),
      Qcat s (Qcons x t) = Qcat (Qrcons s x) t.
   Proof.
   intros. rewrite -Qcat1s -QcatA Qcats1 //.
   Qed.
...
(*Definition qcons s ss := @Qcons V.T s ss.*)
  ...
  | Resolve_stmts_rcons ss s ce ce' ce'' :
      resolveKinds_stmts ss ce ce' ->
      resolveKinds_stmt s ce' ce'' ->
    resolveKinds_stmts (Qrcons ss s) ce ce''
  .
  ...
  with resolveKinds_stmts_fun (sts : hfstmt_seq) (ce : cenv) : CE.env := (*fold_right resolveKinds_stmt_fun ce sts.*)
    match sts with
    | Qnil => ce
    | Qrcons shd s => resolveKinds_stmt_fun s (resolveKinds_stmts_fun shd ce)
    end.
...
Proof.
  ...
  - intros. apply Resolve_stmts_rcons with (resolveKinds_stmts_fun h ce0).
    exact: (H ce0).
...
(* (*Definition qrcons s ss' := @Qrcons VarOrder.T s ss.*) *)
...
(*   Definition sinst v1 v2 := @Sinst ProdVarOrder.T v1 v2. *)
  ...
  with hfstmt_seq_indexes em ce s : eid_map * CE.env * HiFP.hfstmt_seq :=
         ...
         | Qrcons ss s => let result_ss := hfstmt_seq_indexes em ce ss in
                          let result_s  := hfstmt_indexes (fst (fst result_ss)) (snd (fst result_ss)) s in
                          (fst result_s, Qrcons (snd result_ss) (snd result_s))
         end.
