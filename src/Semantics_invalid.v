...
Inductive lvalue : Type :=
  | Indeterminate : lvalue
  | Determinate : bits -> lvalue.
...
Lemma lvalue_eq_dec (x y : lvalue) : {x = y} + {x <> y}.
Proof.
  destruct x, y; try (right; discriminate); try (left; done).
  destruct (bitseq_eq_dec b b0) as [H|H]; [left; f_equal; exact H | right; injection; auto].
Qed.

Definition lvalue_eqn (x y : lvalue) : bool :=
  match x, y with
  | Indeterminate, Indeterminate => true
  | Determinate bs1, Determinate bs2 => bs1 == bs2
  | _, _ => false
  end.
...
Lemma lvalue_eqP : forall (x y : lvalue), reflect (x = y) (lvalue_eqn x y).
Proof.
  intros x y.
  destruct x as [|n], y as [|m]; simpl; try (right; congruence); try (left; done).
...
Proof.
  - destruct x, y; try (right; discriminate).
    + destruct (lvalue_eq_dec l l0) as [H|H]; [left; f_equal; exact H | right; injection; auto].
...
Fixpoint hvalue_eqn (x y : hvalue) : bool :=
  match x, y with
  | Gval val1, Gval val2 => lvalue_eqn val1 val2
...
Proof.
  ...
  - intros x y.
    ...
    + destruct (lvalue_eqP n m) as [H|H].
        * left. f_equal; assumption.
        * right; congruence. 
...
Fixpoint ftext (ft : ftype) (val : hvalue) : option hvalue :=
  match ft, val with
  | _, Gval Indeterminate => Some (Gval Indeterminate)
  | Gtyp (Fuint w), Gval (Determinate c) => Some (Gval (Determinate (zext (w - size c) c)))
  | Gtyp (Fsint w), Gval (Determinate c) => Some (Gval (Determinate (sext (w - size c) c)))
  | Gtyp _, Gval (Determinate c) => Some (Gval (Determinate (zext (1 - size c) c)))
...
Fixpoint ftext0 (ft : ftype) : hvalue :=
  match ft with
  | Gtyp (Fuint w) 
  | Gtyp (Fsint w) => Gval (Determinate (zeros w))
  | Gtyp _ => Gval (Determinate [::b0])
  | Atyp atyp n => 
...
Fixpoint hvalue_of_ref (r : HiF.href) (s : VM.t hvalue) (tmap: VM.t (ftype * fcomponent)) : option hvalue :=
  ...
  | Esubaccess r e => match eval_hfexpr e s tmap, hvalue_of_ref r s tmap with 
              | Some (Gval (Determinate val)), Some (Aval fv) => let n := to_nat val in
                                  let fix aux fx m := (
                                          match fx, m with
                                          | Acons t fxs, m'.+1 => aux fxs m'
                                          | Acons t _, 0 => Some t 
                                  ...
                                  in aux fv n
              | Some (Gval Indeterminate), Some (Aval fv) => Some (Gval Indeterminate) (* TBD what happens here? *)
              | _, _ => None
              end
  end
...
with eval_hfexpr (exp : HiF.hfexpr) (s : VM.t hvalue) (tmap: VM.t (ftype * fcomponent)) : option hvalue :=
  ...
  | Econst t c => match t with
                  | Fuint w1 => if (size c) > w1 then None else Some (Gval (Determinate (zext (w1 - size c) c)))
                  | Fsint w2 => if (size c) > w2 then None else Some (Gval (Determinate (sext (w2 - size c) c)))
                  | _ => None
                  end
  ...
  | Ecast AsClock e  
  | Ecast AsAsync e => match eval_hfexpr e s tmap with 
                  | Some (Gval (Determinate val)) => Some (Gval (Determinate [::lsb val])) 
                  | Some (Gval Indeterminate) => Some (Gval Indeterminate)
                  | _ => None
                  end
  | Eprim_binop b e1 e2 =>
      match eval_hfexpr e1 s tmap, eval_hfexpr e2 s tmap, type_of_hfexpr e1 tmap, type_of_hfexpr e2 tmap with
      | Some (Gval (Determinate val1)), Some (Gval (Determinate val2)), Some (Gtyp gt1), Some (Gtyp gt2) => 
          let val := LoFirrtl.ebinop_op b gt1 gt2 val1 val2 in Some (Gval (Determinate val))
      | Some (Gval Indeterminate), Some (Gval _), Some (Gtyp gt1), Some (Gtyp gt2) 
      | Some (Gval _), Some (Gval Indeterminate), Some (Gtyp gt1), Some (Gtyp gt2) => Some (Gval Indeterminate)
      | _, _, _, _ => None
      end
  | Eprim_unop u e =>
      match eval_hfexpr e s tmap, type_of_hfexpr e tmap with
      | Some (Gval (Determinate val1)), Some (Gtyp gt1) =>
          let val := LoFirrtl.eunop_op u gt1 val1 in Some (Gval (Determinate val))
      | Some (Gval Indeterminate), Some (Gtyp gt1) => Some (Gval Indeterminate)
      | _, _ => None
      end
  | Emux c e1 e2 => 
      match eval_hfexpr c s tmap, type_of_hfexpr exp tmap, eval_hfexpr e1 s tmap, eval_hfexpr e2 s tmap with
      | Some (Gval (Determinate valc)), Some ft, Some val1, Some val2 => if ~~ (is_zero valc) then ftext ft val1
                                                                                else ftext ft val2
      | Some (Gval Indeterminate), Some ft, Some val1, Some val2 => Some (Gval Indeterminate) (* TBD also, what to be here *)
      | _, _, _, _ => None
      end
  end.
...
Fixpoint find_hvalue_by_offset (val : hvalue) (offset : nat) : option lvalue :=
  match val, offset with
  | Gval (Determinate bv), 0 => Some (Determinate bv)
  | Gval Indeterminate, 0 => Some Indeterminate
  | Aval aval, os.+1 => find_array_value_by_offset aval os
  | Bval bval, os.+1 => find_bundle_value_by_offset bval os
  | _, _ => None
  end 
with find_array_value_by_offset (aval : array_value) (offset : nat) : option lvalue :=
  match aval with
...
with find_bundle_value_by_offset (bval : bundle_value) (offset : nat) : option lvalue := 
  match bval with
...
with update_bundle_value_by_offset (bval : bundle_value) (offset : nat) (new_val : hvalue) : option bundle_value := 
  ...
  Definition g_val0 := Gval (Determinate [::false; false]).
  Definition g_val1 := Gval (Determinate [::true; false]).
  Definition g_val2 := Gval (Determinate [::false; true]).
  Definition g_val3 := Gval (Determinate [::true; true]).
...
Fixpoint eval_hfstmt (st : HiF.hfstmt) (rs ns : VM.t hvalue) (s : VM.t hvalue) (tmap: VM.t (ftype * fcomponent)) : option ((VM.t hvalue) * (VM.t hvalue)) :=
  ...
  | Swhen cond ss_true ss_false => match eval_hfexpr cond s tmap with
                  | Some (Gval (Determinate valc)) => if ~~ (is_zero valc) then eval_hfstmts ss_true rs ns s tmap else eval_hfstmts ss_false rs ns s tmap
                  | Some (Gval Indeterminate) => Some (rs, ns) (* TBD what to do here *)
                  | _ => None
                  end
...
(* Expression evaluation, value *)
Fixpoint eval_hfexpr (exp : HiFP.hfexpr) (s : PVM.t lvalue) (tmap: PVM.t (fgtyp * fcomponent)) : option lvalue :=
  match exp with
  | Econst t c => match t with
                  | Fuint w1 => if (size c) > w1 then None else Some (Determinate (zext (w1 - size c) c))
                  | Fsint w2 => if (size c) > w2 then None else Some (Determinate (sext (w2 - size c) c))
                  | _ => None
                  end
  ...
  | Ecast AsClock e  
  | Ecast AsAsync e => match eval_hfexpr e s tmap with 
                  | Some (Determinate val) => Some (Determinate [::lsb val])
                  | Some Indeterminate => Some Indeterminate
                  | _ => None 
                  end
  | Eprim_binop b e1 e2 =>
      match eval_hfexpr e1 s tmap, eval_hfexpr e2 s tmap, type_of_hfexpr e1 tmap, type_of_hfexpr e2 tmap with
      | Some (Determinate val1), Some (Determinate val2), Some gt1, Some gt2 => 
          let val := LoFirrtl.ebinop_op b gt1 gt2 val1 val2 in Some (Determinate val)
      | Some Indeterminate, Some _, Some gt1, Some gt2 
      | Some _, Some Indeterminate, Some gt1, Some gt2 => Some Indeterminate
      | _, _, _, _ => None
      end
  | Eprim_unop u e =>
      match eval_hfexpr e s tmap, type_of_hfexpr e tmap with
      | Some (Determinate val1), Some gt1 =>
          let val := LoFirrtl.eunop_op u gt1 val1 in Some (Determinate val)
      | Some Indeterminate, Some gt1 => Some Indeterminate
      | _, _ => None
      end
  | Emux c e1 e2 => 
      ...
      | Some (Determinate valc), Some (Fuint w1), Some (Fuint w2), Some (Determinate val1), Some (Determinate val2) => 
                                if ~~ (is_zero valc) then Some (Determinate (zext ((max w1 w2) - w1) val1))
                                else Some (Determinate (zext ((max w1 w2) - w2) val2))
      | Some Indeterminate, Some (Fsint w1), Some (Fsint w2), Some _, Some _ 
      | Some _, Some (Fsint w1), Some (Fsint w2), Some Indeterminate, Some _ 
      | Some _, Some (Fsint w1), Some (Fsint w2), Some _, Some Indeterminate => Some Indeterminate (* TBD also, what to be here *)
      | _, _, _, _, _ => None
      end
  end.

Fixpoint eval_hfstmt (st : HiFP.hfstmt) (rs ns s : PVM.t lvalue) (tmap: PVM.t (fgtyp * fcomponent)) : option ((PVM.t lvalue) * (PVM.t lvalue)) :=
  match st with
  | Snode v e => match eval_hfexpr e s tmap with
  ...
  | Sinvalid (Eid r) => match PVM.find r tmap with
                        | Some (_, Register) => (* 更新rs *) Some (PVM.add r Indeterminate rs, ns)
                        | Some _ => Some (rs, PVM.add r Indeterminate ns)
                        | _ => None
                        end
  | Swhen cond ss_true ss_false => match eval_hfexpr cond s tmap with
                  | Some (Determinate valc) => if ~~ (is_zero valc) then eval_hfstmts ss_true rs ns s tmap else eval_hfstmts ss_false rs ns s tmap
                  | Some Indeterminate => Some (rs, ns) (* TBD *)
                  | _ => None
                  end
  | _ => Some (rs,ns)
  end
with eval_hfstmts (sts : HiFP.hfstmt_seq) (rs ns s : PVM.t lvalue) (tmap: PVM.t (fgtyp * fcomponent)) : option ((PVM.t lvalue) * (PVM.t lvalue)) :=
  match sts with
  | Qnil => Some (rs, ns)
  | Qcons st tl => match eval_hfstmt st rs ns s tmap with
                | Some (rs0, ns0) => eval_hfstmts tl rs0 ns0 s tmap
...
Fixpoint init_dclrs (ss : HiFP.hfstmt_seq) (valmap : PVM.t lvalue) (tmap: PVM.t (fgtyp * fcomponent)) : option (PVM.t lvalue) := 
  match ss with
  | Qnil => Some valmap
...
with init_dclr (s : HiFP.hfstmt) (valmap : PVM.t lvalue) (tmap: PVM.t (fgtyp * fcomponent)) : option (PVM.t lvalue) := 
  match s with
  | Swire v (Gtyp gt) => let w := sizeof_fgtyp gt in Some (PVM.add v (Determinate (zeros w)) valmap)
  | Swire v _ => None
  | Snode v e => match eval_hfexpr e valmap tmap with(* e中被用到的变量应该已被赋初值0,则一定有值 *)
...
(*Fixpoint init_registers (ss : HiFP.hfstmt_seq) (valmap rs : PVM.t lvalue) (tmap: PVM.t (fgtyp * fcomponent)) : PVM.t lvalue := 
  match ss with
  | Qnil => rs
  | Qcons s ss' => init_registers ss' valmap (init_register s valmap rs tmap) tmap
  end
with init_register (s : HiFP.hfstmt) (valmap rs : PVM.t lvalue) (tmap: PVM.t (fgtyp * fcomponent)) : PVM.t lvalue := 
  match s with
  | Sreg v reg => match reset reg with
      | NRst => rs
...
Definition update_values (rs s: PVM.t lvalue) : PVM.t lvalue := 
  PVM.fold (fun key value temps => PVM.add key value temps) rs s.

Fixpoint iterate (n : nat) (func : PVM.t lvalue -> PVM.t lvalue -> PVM.t lvalue -> PVM.t (fgtyp * fcomponent) -> option (PVM.t lvalue * PVM.t lvalue))
  (s : PVM.t lvalue) (tmap: PVM.t (fgtyp * fcomponent)) : option (PVM.t lvalue) :=
  match n with
  | 0 => Some s
  | n'.+1 => match func (PVM.empty lvalue) (PVM.empty lvalue) s tmap with
            | Some (_, ns) => (* everytime we start with an empty map to record the values to be updated in the next iteration *) 
              let s_upd := update_values ns s in
              if PVM.equal (fun val1 val2 => lvalue_eqn val1 val2) s_upd s (* LFP *) then Some s_upd else iterate n' func s_upd tmap
            | _ => None
            end
  end.

Parameter n : nat. (* TBD *)
Definition compute_Sem (c : HiFP.hfcircuit) (inputs reg_init : PVM.t lvalue) : option (PVM.t lvalue * PVM.t lvalue) :=
  match circuit_tmap c, c with
  | Some tmap, Fcircuit _ [::(FInmod _ ps ss)] => 
        let s := update_values reg_init inputs in (* value of inputs and registers should keep during the iteration, wait until the next rising edge comes. *)
        ...
        | Some init_s => match iterate n (eval_hfstmts ss) init_s tmap with
            ...
            | Some s0 => match eval_hfstmts ss (PVM.empty lvalue) (PVM.empty lvalue) s0 tmap with
                (* compute the registers' new value according to the stable state *)
                | Some (rs, _) => Some (s0, rs) 
                | _ => None
                end
...
Parameter flat_valmap : (VM.t hvalue) -> (VM.t (ftype * fcomponent)) -> PVM.t lvalue.
(* TBD *)
...
(* Proves pass expandConnects preserves the semantics *)
  ...
  | Some (sem, regval), Some tmap =>
      ...
      | Some (sem_new, regval_new) => PVM.equal (fun val1 val2 => lvalue_eqn val1 val2) flatten_sem sem_new /\ 
                                      PVM.equal (fun val1 val2 => lvalue_eqn val1 val2) flatten_regval regval_new
...
| Qcons s ss =>
    match ExpandBranch_fun s old_conn_map with
    | Some temp_conn_map =>
        ExpandBranches_funs ss temp_conn_map
    | None => None
    end
end
...
| Sfcnct _ expr => None
| Sinvalid (Eid var) => Some (PVM.add var D_invalidated old_conn_map)
| Sinvalid _ => None
| Swhen cond ss_true ss_false =>
    match ExpandBranches_funs ss_true old_conn_map with
    | Some true_conn_map =>
        match ExpandBranches_funs ss_false old_conn_map with
        | Some false_conn_map =>
...
:=  match m with
    | FInmod v pp ss =>
        match ExpandBranches_funs ss (PVM.empty def_expr) with
            | Some conn_map =>
...
Lemma PVM_equal_iff_find_eq (m1 m2 : PVM.t lvalue) : PVM.equal (fun val1 val2 : bitseq => val1 == val2) m1 m2 <-> (forall v, PVM.find v m1 = PVM.find v m2).
Proof.
Admitted.
...
Lemma PVM_equal_trans (m1 m2 m3: PVM.t lvalue) func : PVM.equal func m1 m2 -> PVM.equal func m2 m3 -> PVM.equal func m1 m3.
Proof.
Admitted.

Lemma PVM_non_equal_trans (m1 m2 m3: PVM.t lvalue) func : ~ PVM.equal func m1 m2 -> PVM.equal func m2 m3 -> ~ PVM.equal func m1 m3.
Proof.
Admitted.

Lemma PVM_equal_comm (m1 m2 : PVM.t lvalue) func : PVM.equal func m1 m2 <-> PVM.equal func m2 m1.
Proof.
Admitted.
...
Definition func_type : Type := PVM.t lvalue -> PVM.t lvalue -> PVM.t lvalue -> PVM.t (fgtyp * fcomponent) -> option (PVM.t lvalue * PVM.t lvalue).
Definition func_type_eq (fun1 fun2 : func_type) (tmap : PVM.t (fgtyp * fcomponent)) : Prop := forall (init_s1 init_s2 : PVM.t lvalue) (s1 s2 rs1 rs2 : PVM.t lvalue),
  PVM.equal (fun val1 val2 : bitseq => val1 == val2) init_s1 init_s2 -> fun1 (PVM.empty lvalue) (PVM.empty lvalue) init_s1 tmap = Some (rs1, s1) -> fun2 (PVM.empty lvalue) (PVM.empty lvalue) init_s2 tmap = Some (rs2, s2) -> 
...
Proof.
  ...
  - (* Case n = S n' *)
    simpl in Hiter1, Hiter2.
    destruct (fun1 (PVM.empty lvalue) (PVM.empty lvalue) init_s1 tmap) as [[rs1 ns1]|] eqn:Hcall1;
      [|discriminate].
    destruct (fun2 (PVM.empty lvalue) (PVM.empty lvalue) init_s2 tmap) as [[rs2 ns2]|] eqn:Hcall2;
      [|discriminate].
...
(*Fixpoint eval_hfstmt_for_v (st : HiFP.hfstmt) (s : PVM.t lvalue) (tmap: PVM.t (fgtyp * fcomponent)) (v : PVM.key) (current_val : option bits) : option bits :=
  match st with
...
with eval_hfstmts_for_v (sts : HiFP.hfstmt_seq) (s : PVM.t lvalue) (tmap: PVM.t (fgtyp * fcomponent)) (v : PVM.key) (current_val : option bits) : option bits :=
  match sts with
  | Qnil => current_val
...
(*Lemma eval_hfstmts_for_node_no_cnct component_stmts connect_stmts init_s tmap v : (* 这里需要一些语义前提，即node不再次被连接，不出现在cnct stmt中 *) 
  (forall e, ~ Qin (Sfcnct (Eid v) e) connect_stmts) -> forall rs s, Sem_HiFP.eval_hfstmts (Qcat component_stmts connect_stmts) (PVM.empty lvalue) (PVM.empty lvalue) init_s tmap = Some (rs, s) ->
  forall rs' s', Sem_HiFP.eval_hfstmts component_stmts (PVM.empty lvalue) (PVM.empty lvalue) init_s tmap = Some (rs', s') -> PVM.find v s' = PVM.find v s.
Proof.
Admitted.*)

Definition unique_node_dclr (ss : HiFP.hfstmt_seq) : Prop :=
  forall v e, Qin (Snode v e) ss -> (forall e', e' <> e -> ~ Qin (Snode v e') ss) /\ (forall e', ~ Qin (Sfcnct (Eid v) e') ss).

Lemma eval_hfstmts_for_unique_node ss v e : (* 在ss induction即可？ *)
  Qin (Snode v e) ss -> unique_node_dclr ss -> 
  forall init_s tmap rs s, Sem_HiFP.eval_hfstmts ss (PVM.empty lvalue) (PVM.empty lvalue) init_s tmap = Some (rs, s) -> PVM.find v s = Sem_HiFP.eval_hfexpr e init_s tmap.
Proof.
Admitted.
...
Lemma eval_hfstmts_for_comb_only_cnct init_s tmap v component_stmts connect_stmts: (* 需要 component_stmts 的一些前提 *)
  match PVM.find v tmap with
  | Some (gt, Out_port) 
  | Some (gt, Wire) => forall rs s, Sem_HiFP.eval_hfstmts (Qcat component_stmts connect_stmts) (PVM.empty lvalue) (PVM.empty lvalue) init_s tmap = Some (rs, s) ->
      forall rs' s', Sem_HiFP.eval_hfstmts connect_stmts (PVM.empty lvalue) (PVM.empty lvalue) init_s tmap = Some (rs', s') -> PVM.find v s' = PVM.find v s
  | _ => True
  end.
Proof.
Admitted.
...
Lemma eval_hfstmts_for_sequ_only_cnct init_s tmap v component_stmts connect_stmts: (* 需要 component_stmts 和 connect_stmts 的一些前提 *)
  match PVM.find v tmap with
  | Some (gt, Register) => forall rs s, Sem_HiFP.eval_hfstmts (Qcat component_stmts connect_stmts) (PVM.empty lvalue) (PVM.empty lvalue) init_s tmap = Some (rs, s) ->
      forall rs' s', Sem_HiFP.eval_hfstmts connect_stmts (PVM.empty lvalue) (PVM.empty lvalue) init_s tmap = Some (rs', s') -> PVM.find v rs' = PVM.find v rs
  | _ => True
  end.
Proof. (* first do this *)
Admitted.
...
Lemma eval_hfstmts_Qcat_some' s1 s2 rs ns s tmap res : Sem_HiFP.eval_hfstmts (Qcat s1 s2) rs ns s tmap = Some res ->
  exists res', Sem_HiFP.eval_hfstmts s2 rs ns s tmap = Some res'.
Proof.
Admitted.
...
Lemma eval_hfstmts_convert_to_connect_stmts_for_comb conn_map init_s tmap rs s v e : 
  match PVM.find v tmap with
  | Some (gt, Out_port) 
  | Some (gt, Wire) => Sem_HiFP.eval_hfstmts (convert_to_connect_stmts conn_map) (PVM.empty lvalue) 
  (PVM.empty lvalue) init_s tmap = Some (rs, s) -> PVM.find v conn_map = Some (D_fexpr e) -> Sem_HiFP.eval_hfexpr e init_s tmap = PVM.find v s
  | _ => True
  end.
Proof. 
...
Lemma eval_hfstmts_convert_to_connect_stmts_for_sequ conn_map init_s tmap rs s v e : 
  match PVM.find v tmap with
  | Some (gt, Register) => Sem_HiFP.eval_hfstmts (convert_to_connect_stmts conn_map) (PVM.empty lvalue) 
  (PVM.empty lvalue) init_s tmap = Some (rs, s) -> PVM.find v conn_map = Some (D_fexpr e) -> Sem_HiFP.eval_hfexpr e init_s tmap = PVM.find v rs
  | _ => True
  end.
Proof. 
...
Lemma eval_hfstmts_ExpandBranches_funs_find_for_comb ss init_s tmap rs s v : 
  match PVM.find v tmap with
  | Some (gt, Out_port) 
  | Some (gt, Wire) => Sem_HiFP.eval_hfstmts ss (PVM.empty lvalue) (PVM.empty lvalue) init_s tmap = Some (rs, s) ->
...
Proof. 
  destruct (PVM.find v tmap) as [[gt cmpnt]|] eqn : Hcmpnt; try done. destruct cmpnt; try done.
  assert (Hhelper : forall (ss : HiFP.hfstmt_seq) (rs s rs0 s0 : PVM.t lvalue),
...
Lemma eval_hfstmts_ExpandBranches_funs_find_for_sequ ss init_s tmap rs s v : 
  match PVM.find v tmap with
  | Some (gt, Register) => Sem_HiFP.eval_hfstmts ss (PVM.empty lvalue) (PVM.empty lvalue) init_s tmap = Some (rs, s) ->
  forall conn_map, ExpandBranches_funs ss (PVM.empty def_expr) = Some conn_map -> forall e, PVM.find v conn_map = Some (D_fexpr e) -> 
...
Lemma eval_hfstmts_find_rs ss init_s tmap rs s : Sem_HiFP.eval_hfstmts ss (PVM.empty lvalue) (PVM.empty lvalue) init_s tmap = Some (rs, s) ->
...
(* Proves pass expandWhens preserves the semantics *)
  forall (c : HiFP.hfcircuit) (inputs reg_init : PVM.t lvalue),
...
Proof.
  ...
  destruct (Sem_HiFP.eval_hfstmts ss (PVM.empty lvalue) (PVM.empty lvalue) s0 tmap) as [[rs ns]|] eqn : Hregval; try discriminate. inversion Hsem; subst s0 rs; clear Hsem.
  ...
  destruct (Sem_HiFP.eval_hfstmts (Qcat (component_stmts_of ss) (convert_to_connect_stmts conn_map))
               (PVM.empty lvalue) (PVM.empty lvalue) s0 tmap) as [[rs ns_new]|] eqn : Hregval_new; try discriminate. inversion Hsem_new; subst s0 rs; clear Hsem_new.
