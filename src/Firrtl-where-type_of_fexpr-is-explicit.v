...
Qed.
  ...
  Fixpoint type_of_fexpr (e : fexpr) (te : TE.env) : fgtyp_explicit :=
    ...
    | Ecast AsUInt e
    | Ecast AsSInt e => let (t1, _) := type_of_fexpr e te in
                        exist not_implicit_width (Fsint (sizeof_fgtyp t1)) I
    | Ecast AsClock e
    | Ecast AsReset e
...
*)
  ...
  Definition upd_typenv_fstmt (s : fstmt) (te : TE.env) (st : vstate) : TE.env :=
    ...
    | Snode v e => let (t2, _) := type_of_fexpr e te in TE.add v t2 te
    ...
    | Sfcnct v e2 => let (t2, _) := type_of_fexpr e2 te in TE.add v t2 te
...
*)
  ...
  Definition no_mem_upd_typenv_fstmt (s : fstmt) (te : TE.env) : TE.env :=
    ...
    | Swire v t  => TE.add v t te
    | Sreg r => TE.add (rid r) (type r) te
    | Snode v e => let (t1, _) := type_of_fexpr e te in TE.add v t1 te
    | Sinst inst => upd_typenv_fports (iports inst) te
    | Sinvalid v => TE.add v (TE.vtyp v te) te
    | _ => te
    end.
...
*)
  ...
  Definition well_typed_fstmt_inline (s : fstmt) (te : TE.env) s1 rs1 : bool :=
    ...
    | Sfcnct v e => let (t1, _) := type_of_fexpr e te in
                    match (TE.vtyp v te) with
                    | Fuint w | Fuint_implicit w => well_typed_fexpr e te && (w >= (sizeof_fgtyp t1))
                    | Fsint w | Fsint_implicit w => well_typed_fexpr e te && (w >= (sizeof_fgtyp t1))
                    | _ => false
                    end
    | Sinst _ => false
    | _ => true
  ...
  Proof.
    ...
    - case Hinit : (SV.acc (rid r) rs1 == [::]).
      ...
      apply SV.conform_Upd with (zeros (sizeof_fgtyp (type r))) s1.
      rewrite size_zeros//.
      destruct (type_of_fexpr e1 (TE.add (rid r) (type r) te)) ; apply SV.Upd_upd.
      done.
      simpl.
      apply SV.conform_Upd with (zeros (sizeof_fgtyp (type r))) rs1.
      rewrite size_zeros//.
      ...
      simpl.
      move : Hcfr1.
      destruct (type_of_fexpr e1 (TE.add (rid r) (type r) te)) ; apply SV.conform_equal.
      exact Haddsame.
      destruct (type_of_fexpr e1 (TE.add (rid r) (type r) te)) as [[n|n|n|n| | |] niw] eqn: Htypee1.
      3,4: clear Htypee1 ; rewrite /not_implicit_width // in niw.
      - (* Fuint n *)
  ...
  Lemma typeof_eval_fexpr e : forall s1 te,
    ...
    size (no_mem_eval_fexpr e s1 te) = sizeof_fgtyp (let (t, _) := type_of_fexpr e te in t).
  Proof.
    induction e.
    - (* const *)
      move => s1 te Hcf.
    ...
    - (* cast *)
      ...
      - (* asuint *)
        ...
        destruct (type_of_fexpr e te) ; exact Hwt.
        exact Hcf.
      - (* assint *)
        move => s1 te Hcf Hwt.
        simpl.
    ...
    move => s1 te Hcf.
      ...
      - move => Hwf.
        ...
        destruct (type_of_fexpr e3 te) as [[n1|n1|n1|n1| | |] niw] eqn: Ht2 ; try discriminate.
        move : Hwt => /andP [Hwt1 Hwt2].
        simpl.
        rewrite Ht1 Ht2.
        simpl.
      ...
      destruct (type_of_fexpr e2 te) as [[n0|n0|n0|n0| | |] niw0] eqn: H2 ; simpl in Hwt.
      3,4: clear H2 ; rewrite /not_implicit_width // in niw0.
  ...
  no_mem_eval_noninst_fstmt (Snode v e) rs1 s1 (let (t1, _) := type_of_fexpr e te in TE.add v t1 te) = no_mem_eval_noninst_fstmt (Snode v e) rs1 s1 te.
  Admitted.
  ...
  Proof.
    ...
    destruct (type_of_fexpr (Econst V.T f b) te) eqn: Htof ;
    apply SV.conform_upd.
    rewrite /well_formed_fstmt_inline /= in Hwf.
    move : Hwf => /andP [/eqP H _].
    simpl.
    rewrite H subnn zext0 sext0.
    rewrite /type_of_fexpr in Htof.
    destruct f ; injection Htof ; intro ; rewrite -H0 ; exact H.
    exact Hcf.
    (* cast *)
    - case u; try by done.
      (* uint *)
      ...
      simpl in IHe.
      destruct (type_of_fexpr e te) eqn: Htof ;
      apply SV.conform_upd.
      simpl.
      rewrite /well_formed_fstmt_inline in Hwf.
      simpl in Hwf.
    ...
    - move => rs1 s1 te v Hcf Hwf.
      ...
      specialize IHe with (rs1:=rs1) (s1:=s1) (te:=te) (v:=v).
      simpl in IHe.
      destruct e; try by done.
      - (* upad *)
        simpl snd ; simpl no_mem_upd_typenv_fstmt.
        destruct (type_of_fexpr e0 te) as [[n0|n0|n0|n0| | |] niw] ; try discriminate.
        - 1,2: assert (Hwf : well_formed_fstmt_inline (Snode v e0) te s1 rs1)
               by (rewrite /well_formed_fstmt_inline ; simpl ; rewrite Hwt Hid //).
          2: destruct (n < sizeof_fgtyp (Fsint n0)) eqn: Hn.
          1: destruct (n < sizeof_fgtyp (Fuint n0)) eqn: Hn.
          1,2,3,4: apply SV.conform_upd ; try exact Hcf.
          1,2,3,4: apply IHe in Hcf ; try exact Hwf.
          1,2,3,4: apply SV.conform_mem with (v:=v) in Hcf ;
                   try (apply SV.Lemmas.mem_add_eq, TE.SE.eq_refl).
          1,2,3,4: rewrite TE.vsize_add_eq // SV.acc_upd_eq // in Hcf.
          1,3: rewrite Hcf in Hn ; rewrite Hn Hcf //.
          - rewrite Hcf in Hn ; rewrite Hn size_zext -Hcf /sizeof_fgtyp subnKC //.
            rewrite -Hcf /sizeof_fgtyp leqNgt ltnS // in Hn.
            apply negbFE ; exact Hn.
          - rewrite Hcf in Hn ; rewrite Hn size_sext -Hcf /sizeof_fgtyp subnKC //.
            rewrite -Hcf /sizeof_fgtyp leqNgt ltnS // in Hn.
            apply negbFE ; exact Hn.
      - (* shl *)
        simpl.
        destruct (type_of_fexpr e0 te) as [[n0|n0|n0|n0| | |] _] ; try discriminate.
        - 1,2: assert (Hwf : well_formed_fstmt_inline (Snode v e0) te s1 rs1)
               by (rewrite /well_formed_fstmt_inline ;
               simpl ;
               rewrite Hwt Hid //).
          1,2: apply SV.conform_upd ; try exact Hcf.
          1,2: apply IHe in Hcf ; try exact Hwf.
          1,2: apply SV.conform_mem with (v:=v) in Hcf ;
               try (apply SV.Lemmas.mem_add_eq, TE.SE.eq_refl).
          1,2: rewrite TE.vsize_add_eq // SV.acc_upd_eq // in Hcf.
          1,2: rewrite size_cat -Hcf size_zeros /sizeof_fgtyp addnC //.
      - (* shr *)
        simpl.
        destruct (type_of_fexpr e0 te) as [[n0|n0|n0|n0| | |] _] ; try discriminate.
        - 1,2: assert (Hwf : well_formed_fstmt_inline (Snode v e0) te s1 rs1)
               by (rewrite /well_formed_fstmt_inline ; simpl ; rewrite Hwt Hid //).
          1,2: destruct (n < n0) eqn: Hn.
          1,2,3,4: apply SV.conform_upd ; try exact Hcf.
          1,2,3,4: apply IHe in Hcf ; try exact Hwf.
          1,2,3,4: apply SV.conform_mem with (v:=v) in Hcf ;
                   try (apply SV.Lemmas.mem_add_eq, TE.SE.eq_refl).
          1,2,3,4: rewrite TE.vsize_add_eq // SV.acc_upd_eq // in Hcf.
          1,3: rewrite -Hcf /sizeof_fgtyp Hn size_high //.
          1,2: rewrite -Hcf /sizeof_fgtyp Hn /size //.
      - (* cvt *)
        simpl.
        destruct (type_of_fexpr e0 te) as [[n0|n0|n0|n0| | |] _] ; try discriminate.
        - 1,2: assert (Hwf : well_formed_fstmt_inline (Snode v e0) te s1 rs1)
      ...
      - (* not *)
        ...
        - 1,2: assert (Hwf : well_formed_fstmt_inline (Snode v e0) te s1 rs1)
          ...
          1,2: apply SV.conform_mem with (v:=v) in Hcf ;
               try (apply SV.Lemmas.mem_add_eq, TE.SE.eq_refl).
          1,2: rewrite TE.vsize_add_eq // SV.acc_upd_eq // in Hcf.
          1,2: rewrite size_invB ; exact Hcf.
      - (* andr, orr, xorr *)
        1,2,3: simpl ;
               destruct (type_of_fexpr e0 te) as [[n0|n0|n0|n0| | |] _] ; try discriminate ;
      ...
      - (* extr *)
        simpl.
        destruct (type_of_fexpr e0 te) as [[n1|n1|n1|n1| | |] _] ; try discriminate ;
        apply SV.conform_upd ; try exact Hcf ;
        rewrite size_extract //.
      - (* head *)
        simpl.
        destruct (type_of_fexpr e0 te) as [[n2|n2|n2|n2| | |] _] ; try discriminate ;
        apply SV.conform_upd ; try exact Hcf ;
        rewrite size_high //.
      - (* tail *)
        simpl.
        ...
        - 1,2: assert (Hwf : well_formed_fstmt_inline (Snode v e0) te s1 rs1)
          ...
          1,2: apply SV.conform_mem with (v:=v) in Hcf ;
               try (apply SV.Lemmas.mem_add_eq, TE.SE.eq_refl).
          1,2: rewrite TE.vsize_add_eq // SV.acc_upd_eq // in Hcf.
          1,2: rewrite size_low /sizeof_fgtyp -Hcf //.
    (* ebinop *)
    - move => rs1 s1 te v Hcf Hwf.
      rewrite /well_formed_fstmt_inline in Hwf.
      simpl in Hwf.
      ...
      simpl in IHe2.
      destruct e1; try by done.
      - (* add *)
        simpl.
        destruct (type_of_fexpr e2 te) as [[n0|n0|n0|n0| | |] niw0] eqn: H2 ; try discriminate.
        - 1,2: destruct (type_of_fexpr e3 te) as [[n1|n1|n1|n1| | |] niw1] eqn: H3 ; try discriminate.
          1,2: move : Hwt => /andP [Hwt2 Hwt3].
          1,2: assert (Hwf2 : well_formed_fstmt_inline (Snode v e2) te s1 rs1)
               by (rewrite /well_formed_fstmt_inline ;
                   simpl ; rewrite Hwt2 Hid2 //).
          1,2: assert (Hwf3 : well_formed_fstmt_inline (Snode v e3) te s1 rs1)
               by (rewrite /well_formed_fstmt_inline ;
                   simpl ;
                   rewrite Hwt3 Hid3 //).
          1,2: apply SV.conform_upd ; try .
