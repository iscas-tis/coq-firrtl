...
let mem_str2N (m:Ast.fmem) map flag rl wl datatoetc = (*addrvar,envar), midvar), maskvar*)
  let reader_str2N (a, b, rls, wls, data2etc) v = 
    ...
    (StringMap.add (m.mid^"."^v^".clk") b3 a3, b3 + 1, List.cons b rls, wls, IntMap.add b (b1,b2,flag-1,-1) data2etc)
    ) in
  let writer_str2N (a, b, rls, wls, data2etc) v = 
    ...
    (StringMap.add (m.mid^"."^v^".clk") b4 a4, b4 + 1, rls, List.cons b wls, IntMap.add b (b1,b2,flag-1,b3) data2etc)
    ) in
...
let modgraph_m modsnum g m = 
  ...
  | Ast.FInmod (mv , _, sl) -> let modnum = StringMap.find mv modsnum in
                                  List.fold_left (modgraph_s modnum modsnum) g sl
  | Ast.FExmod _ -> g
...
let writeinp =  Env.TE.add (Obj.magic 10) [[true;true];[false]] writeinp
*)
...
let () = 
  ...
  let (modsmap,flagls,flagmap,_,_,_,_) = generate_modsmap lowf_ast name2ports in (* memmap *)
  ...
  (* generate inp *)
  let (inp_bitsmap, _, ulst) = Helper.generate_map lowf_ast clks (initmap, [], []) in
  let my2Z k = 
  ...
  (*StringMap.iter (fun key value -> if (List.mem key inp_lst) then (printf "%s:" key; List.iter (printf " %d") value; printf "\n")) inp_map;
  *)
  let mainmap = StringMap.find cv modsmap in
  let (inp_intmap,_,_) = Helper.m_extract_i mainmap inp_bitsmap mainm in
  ...
  let (instin,instout,(_,instoutl)) = findoutport lowf_ast modsmap modsnum insnum in
  ...
  *) 
  ...
  StringMap.iter (fun key value -> (*if (String.equal cv key) then*) ((printf "%s:" key); StringMap.iter (fun key0 value0 -> (printf "\n"; printf "%s:%d" key0 value0)) value)
    ) modsmap;
StringMap.iter (fun key0 value0 -> (printf "%s: %d" key0 value0); printf "\n") modsnum;
StringMap.iter (fun key0 value0 -> (printf "%s: %d" key0 value0); printf "\n") insnum;
                                               
  let modg = modgraph modsnum lowf_ast in
  (*let (_,orderfun) = C.scc modg in
  printf "10: %d\n" (orderfun 10);
  printf "11: %d\n" (orderfun 11);
  printf "12: %d\n" (orderfun 12);*)
  ...
  (* test 排序 *)
  match (Env.TE.find (Obj.magic 12) ffstmtsmap) with 
  | None -> printf "no C";
  | Some stlsc -> 
      ...
      let (newgc, _) =
        List.fold_left (fun pat tempst ->
          ...
          Firrtl.LoFirrtl.buildg [] ffstmtsmap Env.TE.empty Env.TE.empty Env.TE.empty tempg tempm
            tempst) (Firrtl.LoFirrtl.emptyg, Env.TE.empty) stlsc
        in
        List.iter (fun t -> (printf "0->%d\n" (Obj.magic t))) (Firrtl.LoFirrtl.findg (Obj.magic 0) newgc);
        List.iter (fun t -> (printf "1->%d\n" (Obj.magic t))) (Firrtl.LoFirrtl.findg (Obj.magic 1) newgc);
        List.iter (fun t -> (printf "3->%d\n" (Obj.magic t))) (Firrtl.LoFirrtl.findg (Obj.magic 3) newgc);
        if (Firrtl.LoFirrtl.dfs_path newgc 4 (Obj.magic 0) (Obj.magic 2)) then (printf "0daoda2\n"; )
      else printf "budaoda\n";
      if (Firrtl.LoFirrtl.dfs_path newgc 4 (Obj.magic 2) (Obj.magic 0)) then (printf "0daoda2\n"; )
      else printf "budaoda\n";
  (* inst *)
  match (Env.TE.find (Obj.magic 11) ffstmtsmap) with 
  | None -> printf "no B";
  | Some stls -> List.iter p_stmt stls;

  List.iter (fun tst -> match tst with
                        | Firrtl.Sfcnct (_, e2) -> let preset = Firrtl.LoFirrtl.expr2varlist e2 [] in printf_list (List.map Obj.magic preset)
                        | _ -> printf "tbd\n";) stls;

  let inps =
    match Env.TE.find (Obj.magic 11) finstin with
    | Some tstmts -> tstmts
    | None -> []
  in

  let (regls, roots) =
        List.fold_left (fun pat tst ->
          let (ls1, ls2) = pat in Firrtl.LoFirrtl.findreg ls1 ls2 tst) ([], []) stls
      in
  printf "regs:"; printf_list (List.map Obj.magic regls);
  printf "roots:"; printf_list (List.map Obj.magic roots);
  printf "inputs:"; printf_list (List.map Obj.magic inps);
  
  match Env.TE.find (Obj.magic 11) fnewinstportmap with
  | None -> printf "no portsmap\n"
  | Some (inm, outm) -> 

      let (newg, var2stmt) =
        List.fold_left (fun pat tempst ->
          let (tempg, tempm) = pat in
          Firrtl.LoFirrtl.buildg [] ffstmtsmap inm outm (Env.TE.add (Obj.magic 12) newgc Env.TE.empty) tempg tempm
            tempst) (Firrtl.LoFirrtl.emptyg, Env.TE.empty) stls
        in
  List.iter (fun t -> (printf "0->%d\n" (Obj.magic t))) (Firrtl.LoFirrtl.findg (Obj.magic 0) newg);
  List.iter (fun t -> (printf "1->%d\n" (Obj.magic t))) (Firrtl.LoFirrtl.findg (Obj.magic 1) newg);
  List.iter (fun t -> (printf "4->%d\n" (Obj.magic t))) (Firrtl.LoFirrtl.findg (Obj.magic 4) newg);
  List.iter (fun t -> (printf "5->%d\n" (Obj.magic t))) (Firrtl.LoFirrtl.findg (Obj.magic 5) newg);
  List.iter (fun t -> (printf "6->%d\n" (Obj.magic t))) (Firrtl.LoFirrtl.findg (Obj.magic 6) newg);
  List.iter (fun t -> (printf "7->%d\n" (Obj.magic t))) (Firrtl.LoFirrtl.findg (Obj.magic 7) newg);

  match (Env.TE.find (Obj.magic 2) var2stmt) with 
  | None -> printf "no 2 stmt\n";
  | Some tst -> printf "2: "; p_stmt tst;

  match (Env.TE.find (Obj.magic 3) var2stmt) with 
  | None -> printf "no 3 stmt\n";
  | Some tst -> printf "3: "; p_stmt tst;

  match Firrtl.LoFirrtl.topo_sort newg (List.length stls + List.length inps) (List.append (Obj.magic inps) roots) with
  | None -> (*(fingmap, newstmtsmap)*) printf "no\n";
  | Some tseq -> printf "yes\n"; printf_list (List.map Obj.magic tseq);
    let newstorder =
      List0.map (fun tv ->
        match Env.TE.find (Obj.magic tv) var2stmt with
        | Some tempst -> tempst
        | None -> Firrtl.Sskip) tseq
    in
    List.iter p_stmt newstorder;

  let (allg, orderedmap) = Firrtl.LoFirrtl.modname2g (Obj.magic morder) fflagmap ffstmtsmap fnewinstportmap finstin Env.TE.empty Env.TE.empty in
  match (Env.TE.find (Obj.magic 10) ffstmtsmap) with
  | None -> printf "no stmtsmap\n";
  | Some tm -> printf "10stmts: "; List.iter p_stmt tm;
  
  match (Env.TE.find (Obj.magic 10) allg) with
  | None -> printf "no 10g\n";
  | Some g10 -> printf "g10: \n"; 
  List.iter (fun t -> (printf "0->%d\n" (Obj.magic t))) (Firrtl.LoFirrtl.findg (Obj.magic 0) g10);
  List.iter (fun t -> (printf "1->%d\n" (Obj.magic t))) (Firrtl.LoFirrtl.findg (Obj.magic 1) g10);
  List.iter (fun t -> (printf "2->%d\n" (Obj.magic t))) (Firrtl.LoFirrtl.findg (Obj.magic 2) g10);
  List.iter (fun t -> (printf "3->%d\n" (Obj.magic t))) (Firrtl.LoFirrtl.findg (Obj.magic 3) g10);
  List.iter (fun t -> (printf "4->%d\n" (Obj.magic t))) (Firrtl.LoFirrtl.findg (Obj.magic 4) g10);
  List.iter (fun t -> (printf "5->%d\n" (Obj.magic t))) (Firrtl.LoFirrtl.findg (Obj.magic 5) g10);
  List.iter (fun t -> (printf "6->%d\n" (Obj.magic t))) (Firrtl.LoFirrtl.findg (Obj.magic 6) g10);
  List.iter (fun t -> (printf "7->%d\n" (Obj.magic t))) (Firrtl.LoFirrtl.findg (Obj.magic 7) g10);
  List.iter (fun t -> (printf "8->%d\n" (Obj.magic t))) (Firrtl.LoFirrtl.findg (Obj.magic 8) g10);
  List.iter (fun t -> (printf "9->%d\n" (Obj.magic t))) (Firrtl.LoFirrtl.findg (Obj.magic 9) g10);

  match (Env.TE.find (Obj.magic 10) orderedmap) with
  | None -> printf "no orderedmap\n";
  | Some tm0 -> printf "ordered10: "; List.iter p_stmt tm0;

  let stmtsorder = Firrtl.LoFirrtl.reorder (Obj.magic morder) fflagmap ffstmtsmap fnewinstportmap finstin in
  List.iter (fun modname -> printf "%d:\n" modname; match (Env.TE.find (Obj.magic modname) stmtsorder) with
                                                                | None -> printf "mei pai\n";
                                                                | Some stls -> List.iter p_stmt stls;) morder;
