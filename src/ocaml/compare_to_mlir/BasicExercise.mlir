...
firrtl.module @BasicExercise(in %clock_wky: !firrtl.clock, in %reset1: !firrtl.uint<1>, in %io_sw: !firrtl.uint<2>, out %io_led: !firrtl.uint<1>) attributes {convention = #firrtl<convention scalarized>} {
  %io_sw_0 = firrtl.wire {name = "io_sw"} : !firrtl.uint<2>
  %io_led_1 = firrtl.wire {name = "io_led"} : !firrtl.uint<1>
  firrtl.matchingconnect %io_sw_0, %io_sw : !firrtl.uint<2>
  firrtl.matchingconnect %io_led, %io_led_1 : !firrtl.uint<1>
  ...
  firrtl.matchingconnect %io_led_1, %_io_led_T_2 : !firrtl.uint<1>
}
