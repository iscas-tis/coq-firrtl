...
firrtl.module @ShiftRegister(in %clock: !firrtl.clock, in %reset1: !firrtl.uint<1>, in %io_din: !firrtl.uint<1>, out %io_dout: !firrtl.uint<1>, in %io_serIn: !firrtl.uint<1>, out %io_paraOut: !firrtl.uint<4>, in %io_d: !firrtl.uint<4>, in %io_load: !firrtl.uint<1>, out %io_serOut: !firrtl.uint<1>) attributes {convention = #firrtl<convention scalarized>} {
  ...
  firrtl.matchingconnect %io_dout, %io_dout_1 : !firrtl.uint<1>
  firrtl.matchingconnect %io_serIn_2, %io_serIn : !firrtl.uint<1>
  firrtl.matchingconnect %io_paraOut, %io_paraOut_3 : !firrtl.uint<4>
  firrtl.matchingconnect %io_d_4, %io_d : !firrtl.uint<4>
  firrtl.matchingconnect %io_load_5, %io_load : !firrtl.uint<1>
  firrtl.matchingconnect %io_serOut, %io_serOut_6 : !firrtl.uint<1>
  ...
  firrtl.matchingconnect %shiftReg, %_shiftReg_T_1 : !firrtl.uint<4>
  ...
  firrtl.matchingconnect %outReg, %_outReg_T_1 : !firrtl.uint<4>
  ...
  firrtl.connect %loadReg, %8 : !firrtl.uint<4>
  %9 = firrtl.bits %loadReg 0 to 0 : (!firrtl.uint<4>) -> !firrtl.uint<1>
  %serOut = firrtl.node %9 : !firrtl.uint<1>
  firrtl.matchingconnect %io_serOut_6, %serOut : !firrtl.uint<1>
  firrtl.matchingconnect %io_paraOut_3, %outReg : !firrtl.uint<4>
  firrtl.matchingconnect %io_dout_1, %dout : !firrtl.uint<1>
}
