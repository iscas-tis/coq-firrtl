...
firrtl.module @FunctionCounter(in %clock: !firrtl.clock, in %reset1: !firrtl.uint<1>, out %io_cnt: !firrtl.uint<8>, out %io_tick: !firrtl.uint<1>) attributes {convention = #firrtl<convention scalarized>} {
  ...
  firrtl.matchingconnect %count10, %_count10_cntReg_T_3 : !firrtl.uint<8>
  ...
  firrtl.matchingconnect %count99, %_count99_cntReg_T_3 : !firrtl.uint<8>
  ...
  firrtl.matchingconnect %testCounter, %_testCounter_cntReg_T_3 : !firrtl.uint<8>
  %_io_tick_T = firrtl.node %8 : !firrtl.uint<1>
  firrtl.matchingconnect %io_tick_1, %_io_tick_T : !firrtl.uint<1>
  firrtl.matchingconnect %io_cnt_0, %testCounter : !firrtl.uint<8>
}
