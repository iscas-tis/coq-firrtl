...
firrtl.module @Logic(in %clock: !firrtl.clock, in %reset_wky: !firrtl.uint<1>, in %io_a: !firrtl.uint<1>, in %io_b: !firrtl.uint<1>, in %io_c: !firrtl.uint<1>, out %io_out: !firrtl.uint<1>, out %io_cat_wky: !firrtl.uint<16>, out %io_ch: !firrtl.uint<8>, out %io_word: !firrtl.uint<16>, out %io_result: !firrtl.uint<4>) attributes {convention = #firrtl<convention scalarized>} {
  ...
  %io_cat_wky_4 = firrtl.wire {name = "io_cat_wky"} : !firrtl.uint<16>
  ...
  firrtl.matchingconnect %io_a_0, %io_a : !firrtl.uint<1>
  firrtl.matchingconnect %io_b_1, %io_b : !firrtl.uint<1>
  firrtl.matchingconnect %io_c_2, %io_c : !firrtl.uint<1>
  firrtl.matchingconnect %io_out, %io_out_3 : !firrtl.uint<1>
  firrtl.matchingconnect %io_cat_wky, %io_cat_wky_4 : !firrtl.uint<16>
  firrtl.matchingconnect %io_ch, %io_ch_5 : !firrtl.uint<8>
  firrtl.matchingconnect %io_word, %io_word_6 : !firrtl.uint<16>
  firrtl.matchingconnect %io_result, %io_result_7 : !firrtl.uint<4>
  %c65_ui7 = firrtl.constant 65 : !firrtl.uint<7>
  %0 = firrtl.pad %c65_ui7, 8 : (!firrtl.uint<7>) -> !firrtl.uint<8>
  firrtl.matchingconnect %io_ch_5, %0 : !firrtl.uint<8>
  ...
  %add_wky = firrtl.node %8 : !firrtl.uint<1>
  ...
  %sub_wky = firrtl.node %10 : !firrtl.uint<1>
  ...
  firrtl.connect %w, %_w_T : !firrtl.uint<1>
  ...
  firrtl.matchingconnect %io_cat_wky_4, %word : !firrtl.uint<16>
  ...
  firrtl.matchingconnect %split_low, %c1_ui8 : !firrtl.uint<8>
  firrtl.matchingconnect %split_high, %c255_ui8 : !firrtl.uint<8>
  %19 = firrtl.cat %split_high, %split_low : (!firrtl.uint<8>, !firrtl.uint<8>) -> !firrtl.uint<16>
  %_assignWord_T = firrtl.node %19 : !firrtl.uint<16>
  firrtl.matchingconnect %assignWord, %_assignWord_T : !firrtl.uint<16>
  firrtl.matchingconnect %io_word_6, %assignWord : !firrtl.uint<16>
  ...
  firrtl.matchingconnect %vecResult_0, %c1_ui1 : !firrtl.uint<1>
  firrtl.matchingconnect %vecResult_1, %c0_ui1 : !firrtl.uint<1>
  firrtl.matchingconnect %vecResult_2, %c1_ui1 : !firrtl.uint<1>
  firrtl.matchingconnect %vecResult_3, %c0_ui1 : !firrtl.uint<1>
  ...
  firrtl.matchingconnect %io_result_7, %uintResult : !firrtl.uint<4>
}
