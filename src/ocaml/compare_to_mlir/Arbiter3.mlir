...
firrtl.module @Arbiter3(in %clock_wky: !firrtl.clock, in %reset1: !firrtl.uint<1>, in %io_request: !firrtl.uint<3>, out %io_grant: !firrtl.uint<3>) attributes {convention = #firrtl<convention scalarized>} {
  ...
  firrtl.matchingconnect %request_0, %_request_T : !firrtl.uint<1>
  firrtl.matchingconnect %request_1, %_request_T_1 : !firrtl.uint<1>
  firrtl.matchingconnect %request_2, %_request_T_2 : !firrtl.uint<1>
  ...
  firrtl.matchingconnect %notGranted_1, %_notGranted_1_T_1 : !firrtl.uint<1>
  %7 = firrtl.and %request_2, %notGranted_1 : (!firrtl.uint<1>, !firrtl.uint<1>) -> !firrtl.uint<1>
  %_grant_2_T = firrtl.node %7 : !firrtl.uint<1>
  firrtl.matchingconnect %grant_2, %_grant_2_T : !firrtl.uint<1>
  %8 = firrtl.cat %grant_2, %grant_1 : (!firrtl.uint<1>, !firrtl.uint<1>) -> !firrtl.uint<2>
  %io_grant_hi = firrtl.node %8 : !firrtl.uint<2>
  %9 = firrtl.cat %io_grant_hi, %grant_0 : (!firrtl.uint<2>, !firrtl.uint<1>) -> !firrtl.uint<3>
  %_io_grant_T = firrtl.node %9 : !firrtl.uint<3>
  firrtl.matchingconnect %io_grant_1, %_io_grant_T : !firrtl.uint<3>
}
