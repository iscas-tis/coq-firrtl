...
firrtl.module @PopulationCount(in %clock: !firrtl.clock, in %reset_wky: !firrtl.uint<1>, in %io_dinValid: !firrtl.uint<1>, out %io_dinReady: !firrtl.uint<1>, in %io_din: !firrtl.uint<8>, out %io_popCntValid: !firrtl.uint<1>, in %io_popCntReady: !firrtl.uint<1>, out %io_popCnt: !firrtl.uint<4>) attributes {convention = #firrtl<convention scalarized>} {
  ...
  firrtl.matchingconnect %io_dinValid_0, %io_dinValid : !firrtl.uint<1>
  firrtl.matchingconnect %io_dinReady, %io_dinReady_1 : !firrtl.uint<1>
  firrtl.matchingconnect %io_din_2, %io_din : !firrtl.uint<8>
  firrtl.matchingconnect %io_popCntValid, %io_popCntValid_3 : !firrtl.uint<1>
  firrtl.matchingconnect %io_popCntReady_4, %io_popCntReady : !firrtl.uint<1>
  firrtl.matchingconnect %io_popCnt, %io_popCnt_5 : !firrtl.uint<4>
  %fsm_clock, %fsm_reset_wky, %fsm_io_dinValid, %fsm_io_dinReady, %fsm_io_popCntValid, %fsm_io_popCntReady, %fsm_io_load, %fsm_io_done = firrtl.instance fsm @PopCountFSM(in clock: !firrtl.clock, in reset_wky: !firrtl.uint<1>, in io_dinValid: !firrtl.uint<1>, out io_dinReady: !firrtl.uint<1>, out io_popCntValid: !firrtl.uint<1>, in io_popCntReady: !firrtl.uint<1>, out io_load: !firrtl.uint<1>, in io_done: !firrtl.uint<1>)
  ...
  firrtl.matchingconnect %fsm_io_dinValid, %fsm.io_dinValid : !firrtl.uint<1>
  firrtl.matchingconnect %fsm.io_dinReady, %fsm_io_dinReady : !firrtl.uint<1>
  firrtl.matchingconnect %fsm.io_popCntValid, %fsm_io_popCntValid : !firrtl.uint<1>
  firrtl.matchingconnect %fsm_io_popCntReady, %fsm.io_popCntReady : !firrtl.uint<1>
  firrtl.matchingconnect %fsm.io_load, %fsm_io_load : !firrtl.uint<1>
  firrtl.matchingconnect %fsm_io_done, %fsm.io_done : !firrtl.uint<1>
  firrtl.matchingconnect %fsm_clock, %clock : !firrtl.clock
  firrtl.matchingconnect %fsm_reset_wky, %reset_wky : !firrtl.uint<1>
  %data_clock, %data_reset_wky, %data_io_din, %data_io_load, %data_io_popCnt, %data_io_done = firrtl.instance data @PopCountDataPath(in clock: !firrtl.clock, in reset_wky: !firrtl.uint<1>, in io_din: !firrtl.uint<8>, in io_load: !firrtl.uint<1>, out io_popCnt: !firrtl.uint<4>, out io_done: !firrtl.uint<1>)
  ...
  firrtl.matchingconnect %data_io_din, %data.io_din : !firrtl.uint<8>
  firrtl.matchingconnect %data_io_load, %data.io_load : !firrtl.uint<1>
  firrtl.matchingconnect %data.io_popCnt, %data_io_popCnt : !firrtl.uint<4>
  firrtl.matchingconnect %data.io_done, %data_io_done : !firrtl.uint<1>
  firrtl.matchingconnect %data_clock, %clock : !firrtl.clock
  firrtl.matchingconnect %data_reset_wky, %reset_wky : !firrtl.uint<1>
  firrtl.matchingconnect %fsm.io_dinValid, %io_dinValid_0 : !firrtl.uint<1>
  firrtl.matchingconnect %io_dinReady_1, %fsm.io_dinReady : !firrtl.uint<1>
  firrtl.matchingconnect %io_popCntValid_3, %fsm.io_popCntValid : !firrtl.uint<1>
  firrtl.matchingconnect %fsm.io_popCntReady, %io_popCntReady_4 : !firrtl.uint<1>
  firrtl.matchingconnect %data.io_din, %io_din_2 : !firrtl.uint<8>
  firrtl.matchingconnect %io_popCnt_5, %data.io_popCnt : !firrtl.uint<4>
  firrtl.matchingconnect %data.io_load, %fsm.io_load : !firrtl.uint<1>
  firrtl.matchingconnect %fsm.io_done, %data.io_done : !firrtl.uint<1>
}

// -----// IR Dump After ExpandWhens: firrtl-expand-whens //----- //
firrtl.module private @PopCountDataPath(in %clock: !firrtl.clock, in %reset_wky: !firrtl.uint<1>, in %io_din: !firrtl.uint<8>, in %io_load: !firrtl.uint<1>, out %io_popCnt: !firrtl.uint<4>, out %io_done: !firrtl.uint<1>) {
  ...
  firrtl.matchingconnect %io_load_1, %io_load : !firrtl.uint<1>
  firrtl.matchingconnect %io_popCnt, %io_popCnt_2 : !firrtl.uint<4>
  firrtl.matchingconnect %io_done, %io_done_3 : !firrtl.uint<1>
  %c0_ui8 = firrtl.constant 0 : !firrtl.uint<8>
  %dataReg = firrtl.regreset %clock, %reset_wky, %c0_ui8 : !firrtl.clock, !firrtl.uint<1>, !firrtl.uint<8>, !firrtl.uint<8>
  %popCntReg = firrtl.regreset %clock, %reset_wky, %c0_ui8 : !firrtl.clock, !firrtl.uint<1>, !firrtl.uint<8>, !firrtl.uint<8>
  %c0_ui4 = firrtl.constant 0 : !firrtl.uint<4>
  %counterReg = firrtl.regreset %clock, %reset_wky, %c0_ui4 : !firrtl.clock, !firrtl.uint<1>, !firrtl.uint<4>, !firrtl.uint<4>
  ...
  firrtl.connect %dataReg, %11 : !firrtl.uint<8>
  ...
  firrtl.connect %popCntReg, %13 : !firrtl.uint<8>
  %14 = firrtl.mux(%io_load_1, %c8_ui4, %10) : (!firrtl.uint<1>, !firrtl.uint<4>, !firrtl.uint<4>) -> !firrtl.uint<4>
  firrtl.connect %counterReg, %14 : !firrtl.uint<4>
  %15 = firrtl.asUInt %reset_wky : (!firrtl.uint<1>) -> !firrtl.uint<1>
  ...
  firrtl.matchingconnect %io_popCnt_2, %19 : !firrtl.uint<4>
  firrtl.matchingconnect %io_done_3, %done : !firrtl.uint<1>
}

// -----// IR Dump After ExpandWhens: firrtl-expand-whens //----- //
firrtl.module private @PopCountFSM(in %clock: !firrtl.clock, in %reset_wky: !firrtl.uint<1>, in %io_dinValid: !firrtl.uint<1>, out %io_dinReady: !firrtl.uint<1>, out %io_popCntValid: !firrtl.uint<1>, in %io_popCntReady: !firrtl.uint<1>, out %io_load: !firrtl.uint<1>, in %io_done: !firrtl.uint<1>) {
  %io_dinValid_0 = firrtl.wire {name = "io_dinValid"} : !firrtl.uint<1>
  %io_dinReady_1 = firrtl.wire {name = "io_dinReady"} : !firrtl.uint<1>
  ...
  firrtl.matchingconnect %io_dinValid_0, %io_dinValid : !firrtl.uint<1>
  firrtl.matchingconnect %io_dinReady, %io_dinReady_1 : !firrtl.uint<1>
  firrtl.matchingconnect %io_popCntValid, %io_popCntValid_2 : !firrtl.uint<1>
  firrtl.matchingconnect %io_popCntReady_3, %io_popCntReady : !firrtl.uint<1>
  firrtl.matchingconnect %io_load, %io_load_4 : !firrtl.uint<1>
  firrtl.matchingconnect %io_done_5, %io_done : !firrtl.uint<1>
  %c0_ui1 = firrtl.constant 0 : !firrtl.uint<1>
  ...
  firrtl.connect %io_dinReady_1, %5 : !firrtl.uint<1>
  ...
  firrtl.connect %io_load_4, %7 : !firrtl.uint<1>
  ...
  firrtl.connect %io_popCntValid_2, %28 : !firrtl.uint<1>
  ...
  firrtl.connect %stateReg, %34 : !firrtl.uint<2>
}
