...
firrtl.module @RisingMooreFsm(in %clock: !firrtl.clock, in %reset1: !firrtl.uint<1>, in %io_din: !firrtl.uint<1>, out %io_risingEdge: !firrtl.uint<1>) attributes {convention = #firrtl<convention scalarized>} {
  ...
  %8 = firrtl.mux(%io_din_0, %7, %stateReg) : (!firrtl.uint<1>, !firrtl.uint<2>, !firrtl.uint<2>) -> !firrtl.uint<2>
  ...
  %16 = firrtl.and %15, %io_din_0 : (!firrtl.uint<1>, !firrtl.uint<1>) -> !firrtl.uint<1>
  %17 = firrtl.not %io_din_0 : (!firrtl.uint<1>) -> !firrtl.uint<1>
  %18 = firrtl.and %15, %17 : (!firrtl.uint<1>, !firrtl.uint<1>) -> !firrtl.uint<1>
  %19 = firrtl.pad %c0_ui1, 2 : (!firrtl.uint<1>) -> !firrtl.uint<2>
  %20 = firrtl.mux(%io_din_0, %c2_ui2, %19) : (!firrtl.uint<1>, !firrtl.uint<2>, !firrtl.uint<2>) -> !firrtl.uint<2>
  %21 = firrtl.not %14 : (!firrtl.uint<1>) -> !firrtl.uint<1>
  %22 = firrtl.and %9, %21 : (!firrtl.uint<1>, !firrtl.uint<1>) -> !firrtl.uint<1>
  %23 = firrtl.asUInt %c2_ui2 : (!firrtl.uint<2>) -> !firrtl.uint<2>
  %24 = firrtl.node %23 : !firrtl.uint<2>
  %25 = firrtl.node %2 : !firrtl.uint<2>
  %26 = firrtl.eq %24, %25 : (!firrtl.uint<2>, !firrtl.uint<2>) -> !firrtl.uint<1>
  %27 = firrtl.node %26 : !firrtl.uint<1>
  %28 = firrtl.and %22, %27 : (!firrtl.uint<1>, !firrtl.uint<1>) -> !firrtl.uint<1>
  %29 = firrtl.eq %io_din_0, %c0_ui1 : (!firrtl.uint<1>, !firrtl.uint<1>) -> !firrtl.uint<1>
  ...
  %33 = firrtl.mux(%30, %32, %stateReg) : (!firrtl.uint<1>, !firrtl.uint<2>, !firrtl.uint<2>) -> !firrtl.uint<2>
  %34 = firrtl.mux(%27, %33, %stateReg) : (!firrtl.uint<1>, !firrtl.uint<2>, !firrtl.uint<2>) -> !firrtl.uint<2>
  %35 = firrtl.mux(%14, %20, %34) : (!firrtl.uint<1>, !firrtl.uint<2>, !firrtl.uint<2>) -> !firrtl.uint<2>
  %36 = firrtl.mux(%5, %8, %35) : (!firrtl.uint<1>, !firrtl.uint<2>, !firrtl.uint<2>) -> !firrtl.uint<2>
  firrtl.connect %stateReg, %36 : !firrtl.uint<2>
  %37 = firrtl.eq %stateReg, %c1_ui1 : (!firrtl.uint<2>, !firrtl.uint<1>) -> !firrtl.uint<1>
  %_io_risingEdge_T = firrtl.node %37 : !firrtl.uint<1>
  firrtl.matchingconnect %io_risingEdge_1, %_io_risingEdge_T : !firrtl.uint<1>
}
