...
firrtl.module @ReadyValidBuffer(in %clock: !firrtl.clock, in %reset1: !firrtl.uint<1>, out %io_in_ready: !firrtl.uint<1>, in %io_in_valid: !firrtl.uint<1>, in %io_in_bits1: !firrtl.uint<8>, in %io_out_ready: !firrtl.uint<1>, out %io_out_valid: !firrtl.uint<1>, out %io_out_bits1: !firrtl.uint<8>) attributes {convention = #firrtl<convention scalarized>} {
  ...
  firrtl.matchingconnect %io_in_ready, %io_in_ready_0 : !firrtl.uint<1>
  firrtl.matchingconnect %io_in_valid_1, %io_in_valid : !firrtl.uint<1>
  firrtl.matchingconnect %io_in_bits1_2, %io_in_bits1 : !firrtl.uint<8>
  firrtl.matchingconnect %io_out_ready_3, %io_out_ready : !firrtl.uint<1>
  firrtl.matchingconnect %io_out_valid, %io_out_valid_4 : !firrtl.uint<1>
  firrtl.matchingconnect %io_out_bits1, %io_out_bits1_5 : !firrtl.uint<8>
  ...
  firrtl.matchingconnect %io_in_ready_0, %emptyReg : !firrtl.uint<1>
  %0 = firrtl.eq %emptyReg, %c0_ui1 : (!firrtl.uint<1>, !firrtl.uint<1>) -> !firrtl.uint<1>
  %_io_out_valid_T = firrtl.node %0 : !firrtl.uint<1>
  firrtl.matchingconnect %io_out_valid_4, %_io_out_valid_T : !firrtl.uint<1>
  firrtl.matchingconnect %io_out_bits1_5, %dataReg : !firrtl.uint<8>
  ...
  firrtl.connect %dataReg, %3 : !firrtl.uint<8>
  ...
  firrtl.connect %emptyReg, %8 : !firrtl.uint<1>
}
