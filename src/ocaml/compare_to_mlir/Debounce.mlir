...
firrtl.module @Debounce(in %clock: !firrtl.clock, in %reset_wky: !firrtl.uint<1>, in %io_btnU: !firrtl.uint<1>, out %io_led: !firrtl.uint<8>) attributes {convention = #firrtl<convention scalarized>} {
  %io_btnU_0 = firrtl.wire {name = "io_btnU"} : !firrtl.uint<1>
  %io_led_1 = firrtl.wire {name = "io_led"} : !firrtl.uint<8>
  firrtl.matchingconnect %io_btnU_0, %io_btnU : !firrtl.uint<1>
  firrtl.matchingconnect %io_led, %io_led_1 : !firrtl.uint<8>
  %c0_ui1 = firrtl.constant 0 : !firrtl.uint<1>
  %btnSync_REG = firrtl.reg %clock : !firrtl.clock, !firrtl.uint<1>
  firrtl.matchingconnect %btnSync_REG, %io_btnU_0 : !firrtl.uint<1>
  %btnSync = firrtl.reg %clock : !firrtl.clock, !firrtl.uint<1>
  firrtl.matchingconnect %btnSync, %btnSync_REG : !firrtl.uint<1>
  ...
  firrtl.connect %cntReg, %4 : !firrtl.uint<32>
  %5 = firrtl.mux(%tick, %btnSync, %btnDebReg) : (!firrtl.uint<1>, !firrtl.uint<1>, !firrtl.uint<1>) -> !firrtl.uint<1>
  firrtl.connect %btnDebReg, %5 : !firrtl.uint<1>
  %c0_ui3 = firrtl.constant 0 : !firrtl.uint<3>
  %shiftReg = firrtl.regreset %clock, %reset_wky, %c0_ui3 : !firrtl.clock, !firrtl.uint<1>, !firrtl.uint<3>, !firrtl.uint<3>
  ...
  firrtl.connect %shiftReg, %8 : !firrtl.uint<3>
  ...
  firrtl.matchingconnect %risingEdge_REG, %btnClean : !firrtl.uint<1>
  ...
  %reg_wky = firrtl.regreset %clock, %reset_wky, %c0_ui8 : !firrtl.clock, !firrtl.uint<1>, !firrtl.uint<8>, !firrtl.uint<8>
  %19 = firrtl.add %reg_wky, %c1_ui1 : (!firrtl.uint<8>, !firrtl.uint<1>) -> !firrtl.uint<9>
  ...
  %21 = firrtl.mux(%risingEdge, %_reg_T_1, %reg_wky) : (!firrtl.uint<1>, !firrtl.uint<8>, !firrtl.uint<8>) -> !firrtl.uint<8>
  firrtl.connect %reg_wky, %21 : !firrtl.uint<8>
  firrtl.matchingconnect %io_led_1, %reg_wky : !firrtl.uint<8>
}
