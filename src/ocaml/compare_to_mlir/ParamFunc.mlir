...
firrtl.module @ParamFunc(in %clock: !firrtl.clock, in %reset1: !firrtl.uint<1>, in %io_selA: !firrtl.uint<1>, out %io_resA: !firrtl.uint<5>, in %io_selB: !firrtl.uint<1>, out %io_resB_d: !firrtl.uint<10>, out %io_resB_b: !firrtl.uint<1>) attributes {convention = #firrtl<convention scalarized>} {
  ...
  firrtl.matchingconnect %io_selA_0, %io_selA : !firrtl.uint<1>
  firrtl.matchingconnect %io_resA, %io_resA_1 : !firrtl.uint<5>
  firrtl.matchingconnect %io_selB_2, %io_selB : !firrtl.uint<1>
  firrtl.matchingconnect %io_resB_d, %io_resB_d_3 : !firrtl.uint<10>
  firrtl.matchingconnect %io_resB_b, %io_resB_b_4 : !firrtl.uint<1>
  ...
  firrtl.connect %resB_d, %3 : !firrtl.uint<10>
  %4 = firrtl.mux(%io_selB_2, %tVal_b, %fVal_b) : (!firrtl.uint<1>, !firrtl.uint<1>, !firrtl.uint<1>) -> !firrtl.uint<1>
  firrtl.connect %resB_b, %4 : !firrtl.uint<1>
  firrtl.connect %io_resA_1, %resA : !firrtl.uint<5>, !firrtl.uint<4>
  firrtl.matchingconnect %io_resB_d_3, %resB_d : !firrtl.uint<10>
  firrtl.matchingconnect %io_resB_b_4, %resB_b : !firrtl.uint<1>
}
