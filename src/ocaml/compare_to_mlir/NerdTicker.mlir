...
firrtl.module @NerdTicker(in %clock: !firrtl.clock, in %reset1: !firrtl.uint<1>, out %io_tick: !firrtl.uint<1>) attributes {convention = #firrtl<convention scalarized>} {
  %io_tick_0 = firrtl.wire {name = "io_tick"} : !firrtl.uint<1>
  firrtl.matchingconnect %io_tick, %io_tick_0 : !firrtl.uint<1>
  ...
  firrtl.connect %cntReg, %7 : !firrtl.sint<8>
  firrtl.connect %io_tick_0, %6 : !firrtl.uint<1>
}
