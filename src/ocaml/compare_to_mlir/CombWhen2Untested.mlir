...
firrtl.module @CombWhen2Untested(in %clock: !firrtl.clock, in %reset1: !firrtl.uint<1>, in %io_data: !firrtl.uint<4>, out %io_out: !firrtl.uint<1>) attributes {convention = #firrtl<convention scalarized>} {
  ...
  firrtl.matchingconnect %io_data_0, %io_data : !firrtl.uint<4>
  firrtl.matchingconnect %io_out, %io_out_1 : !firrtl.uint<1>
  ...
  firrtl.connect %enoughMoney, %1 : !firrtl.uint<1>
  firrtl.matchingconnect %io_out_1, %enoughMoney : !firrtl.uint<1>
}
