...
firrtl.module @Hello(in %clock: !firrtl.clock, in %reset1: !firrtl.uint<1>, out %io_led: !firrtl.uint<1>) attributes {convention = #firrtl<convention scalarized>} {
  ...
  firrtl.matchingconnect %io_led, %io_led_0 : !firrtl.uint<1>
  ...
  firrtl.connect %cntReg, %5 : !firrtl.uint<32>
  ...
  firrtl.connect %blkReg, %7 : !firrtl.uint<1>
  firrtl.matchingconnect %io_led_0, %blkReg : !firrtl.uint<1>
}
