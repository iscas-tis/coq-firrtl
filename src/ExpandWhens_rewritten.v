...
| Sfcnct _ e => expr_has_fully_inferred_ground_types e
...
forall (ss old_def_ss : HiFP.hfstmt_seq) (vm_old vm_new vm_tmap : CEP.t vertex_type)
    ...
    submap old_scope old_tmap_scope.2 ->
    submap old_tmap_scope.2 old_tmap_scope.1 ->
    stmts_tmap old_tmap_scope ss vm_tmap = Some new_tmap_scope ->
    submap new_tmap_scope.1 tmap ->
...
* clear ExpandBranches_Sem.
  ...
  specialize (ExpandBranch_Sem h old_def_ss vm_old vm' vm_tmap ct_old ct' old_conn_map old_scope tmap old_tmap_scope).
  ...
  specialize (ExpandBranch_Sem (tmap', scope') H H0 H1 H2 Logic.eq_refl) with (2 := H5) (3 := H6).
...
* clear ExpandBranch_Sem.
  ...
  + (* Snode *)
    ...
    generalize (submap_addl old_tmap_scope.1 tmap s) ; intro.
    destruct (CEP.find s old_tmap_scope.1) ; first by discriminate H3.
    ...
    specialize (H7 (Gtyp gt) Logic.eq_refl H4).
  ...
  + (* Swhen *)
    ...
    assert (submap old_scope tmap).
          apply (submap_trans old_tmap_scope.2) ; first by exact H1.
          apply (submap_trans old_tmap_scope.1) ; first by exact H2.
          destruct (stmts_tmap old_tmap_scope ss_true vm_tmap) as [[tmap_true scope_true]|] ; last by discriminate H3.
          generalize (submap_trans scope_true old_tmap_scope.2 tmap_true (proj2 (proj2 H7)) (proj1 H7)) ; intro.
          ...
          apply (submap_trans tmap_true) ; first by apply H7.
          apply (submap_trans tmap_false) ; first by apply H9.
          destruct new_tmap_scope as [new_tmap new_scope].
          injection H3 ; clear H3 ; intros _ H3.
          rewrite H3 //.
    generalize (type_of_expr_submap cond old_scope tmap H8) ; intro.
    destruct (type_of_expr cond old_scope) as [[[[[|[]]| | | | | |]| |] p]|] ; try by done.
    generalize (ExpandBranches_Sem ss_true old_def_ss vm_old) ; intro.
    ...
    assert (submap old_tmap_scope.2 tmap_true)
          by (apply (submap_trans scope_true) ; apply H7).
    specialize (ExpandBranches_Sem ss_false true_def_ss vm_true vm_new vm_tmap ct_old ct_false old_conn_map old_scope tmap (tmap_true, old_tmap_scope.2))
          with (1 := Hfalse) (2 := H) (3 := H1) (4 := H11) (7 := proj1 (proj2 H5)) (8 := H6).
    generalize (stmts_submap vm_tmap ss_false tmap_true old_tmap_scope.2 H11) ; intro.
    destruct (stmts_tmap (tmap_true, old_tmap_scope.2) ss_false vm_tmap) as [[tmap_false scope_false]|] ; last by discriminate H3.
    destruct new_tmap_scope as [new_tmap new_scope].
    injection H3 ; clear H3 ; intros _ H3.
    rewrite -H3 /fst in H4.
    specialize (ExpandBranches_Sem (tmap_false, scope_false) Logic.eq_refl H4).
    destruct (ExpandBranch_funs ss_false true_def_ss old_conn_map old_scope) as [[[false_def_ss false_conn_map] _]|] ; last by done.
    ...
    rewrite (ExpandBranches_Sem y).
    assert (submap tmap_true tmap)
          by (apply (submap_trans tmap_false) ; last (by exact H4) ;
              apply H12).
    specialize (H10 H13).
...
Ultimately we want:
   Given ExpandBranch_funs ss old_def_ss ct_old old_scope = Some (def_ss, new_conn_map, new_scope)
     and Sem_frag_stmts vm_old ct_old (Qcat def_ss (... new_conn_map)) vm_new ct_new tmap,
