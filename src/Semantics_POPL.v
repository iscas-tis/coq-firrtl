...
From firrtl Require Import Env_with_implicit_widths Firrtl_with_implicit_widths HiEnv_with_implicit_widths HiFirrtl_with_implicit_widths.

Set Implicit Arguments.
Unset Strict Implicit.
Import Prenex Implicits.

Module MKS := MapKeySet VarOrder VM VS.

(* sequence order, to create a way to identify all components in instances. *)
(* This module code is copied from Semantics_reorganize.v *)
...
Module SeqVarOrder := MakeSeqOrder VarOrder.
...
Variable mi_top : VarOrder.T.
Variable mml : seq (HiF.hfmodule).

Section Kinds_and_Types.

Axiom No_external_modules : all (fun (m : HiF.hfmodule) => match m with
                                                           | FInmod _ _ _ => true
                                                           | FExmod _ _ _ => false
                                                           end) mml.

(* find the module with a identifier mi *)
Definition find_module (mi : VarOrder.T) : option HiF.hfmodule :=
    match filter (fun m : HiF.hfmodule => match m with
                                          | FInmod mi' _ _
                                          | FExmod mi' _ _ => mi == mi'
                                          end) mml with
    | [:: m] => Some m
...
Lemma find_module_in : forall (mi : VarOrder.T) (m : HiF.hfmodule),
    find_module mi = Some m -> m \in mml.
Proof.
intros.
rewrite /find_module in H.
generalize (filter_subseq (fun m : HiF.hfmodule => match m with
                                                   | FInmod mi' _ _
                                                   | FExmod mi' _ _ => mi == mi'
                                                   end) mml) ; intro.
destruct (filter (fun m : HiF.hfmodule => match m with
                                          | FInmod mi' _ _
                                          | FExmod mi' _ _ => mi == mi'
                                          end) mml) as [|m_mi1 m_mi'] eqn: Hm ;
      rewrite Hm in H, H0 ; first by discriminate.
destruct m_mi' as [|m_mi2 m_mi''] ; last by discriminate.
injection H ; clear H ; intro H ; subst m_mi1.
rewrite sub1seq // in H0.
Qed.

Fixpoint hfstmt_inst_in_mml (s : HiF.hfstmt) : bool :=
    match s with
    | Sinst ii mi => find_module mi != None
    | Swhen _ ssl_true ssl_false => hfstmt_seq_inst_in_mml ssl_true && hfstmt_seq_inst_in_mml ssl_false
    | _ => true
    end
with hfstmt_seq_inst_in_mml (ssl : HiF.hfstmt_seq) : bool :=
    match ssl with
    | Qnil => true
    | Qcons s ssl' => hfstmt_inst_in_mml s && hfstmt_seq_inst_in_mml ssl'
    end.

Axiom Top_module_present : find_module mi_top <> None.

(* bundle type of the ports of a module *)
Fixpoint port_type (ppl : seq HiF.hfport) : ffield :=
    match ppl with
    | [::] => Fnil
    | Finput p t :: ppl' => Fflips p Flipped t (port_type ppl')
    | Foutput p t :: ppl' => Fflips p Nflip t (port_type ppl')
    end.

(* type of a module interface *)
Definition module_type (mi : VarOrder.T) : option ftype :=
    match find_module mi with
    | Some (FInmod _ ppl _) => Some (Btyp (port_type ppl))
    | _ => None
    end.

(* (kind, type) pair of port *)
Fixpoint port_kind (ppl : seq HiF.hfport) : VM.t (fcomponent * ftype) :=
    match ppl with
    | [::] => VM.empty (fcomponent * ftype)
    | Finput p t :: ppl' => VM.add p (In_port, t) (port_kind ppl')
    | Foutput p t :: ppl' => VM.add p (Out_port, t) (port_kind ppl')
    end.

(* type of ref expressions. sc = set of identifiers in scope. ckt = component kind and type of all identifiers in the namespace, including those out of scope. *)
Fixpoint type_of_ref (sc : option VS.t) (ckt : VM.t (fcomponent * ftype)) (r : HiF.href) : option ftype :=
    match r with
    | Eid v => match sc with 
               | Some sc' => if VS.mem v sc' then match VM.find v ckt with
                                                  | Some (_, ft) => Some ft
                                                  | None => None
                                                  end
                                             else None
               | None => match VM.find v ckt with
                         | Some (_, ft) => Some ft
                         | None => None
                         end
               end
    | Esubfield r v => match type_of_ref sc ckt r with
    ...
    | Esubaccess r _
    | Esubindex r _ => match type_of_ref sc ckt r with
...
Definition fgtyp_mux (x y : fgtyp) : option fgtyp :=
    match x, y with
    | Fuint wx, Fuint wy => Some (Fuint (maxn wx wy))
    | Fuint_implicit wx, Fuint_implicit wy
    | Fuint_implicit wx, Fuint wy
    | Fuint wx, Fuint_implicit wy => Some (Fuint_implicit (maxn wx wy))
    | Fsint wx, Fsint wy => Some (Fsint (maxn wx wy))
    | Fsint_implicit wx, Fsint_implicit wy
    | Fsint_implicit wx, Fsint wy
    | Fsint wx, Fsint_implicit wy => Some (Fsint_implicit (maxn wx wy))
    | Fuint_implicit wx, Fuint_implicit_eq1
    | Fuint wx, Fuint_implicit_eq1
    | Fuint_implicit_eq1, Fuint_implicit wx
    | Fuint_implicit_eq1, Fuint wx => Some (Fuint_implicit (maxn wx 1))
    | Fuint_implicit_eq1, Fuint_implicit_eq1 => Some Fuint_implicit_eq1
...
Fixpoint make_width_explicit (t : ftype) : ftype :=
    match t with
    | Gtyp (Fsint_implicit w) => Gtyp (Fsint w)
    | Gtyp (Fuint_implicit w) => Gtyp (Fuint w)
    | Gtyp Fuint_implicit_eq1 => Gtyp (Fuint 1)
    | Gtyp gt => Gtyp gt
    | Atyp t' n => Atyp (make_width_explicit t') n
    | Btyp ff => Btyp (make_ffield_explicit ff)
    end
with make_ffield_explicit (ff : ffield) : ffield :=
    match ff with
    | Fnil => Fnil
    | Fflips v f t ff' => Fflips v f (make_width_explicit t) (make_ffield_explicit ff')
    end.

(* Note that type_of_expr never returns an implicit-width type. *)
Fixpoint type_of_expr (sc : option VS.t) (ckt : VM.t (fcomponent * ftype)) (e : HiF.hfexpr) : option ftype :=
  match e with
  | Econst t bs => Some (Gtyp t)
  | Eref r => match type_of_ref sc ckt r with
              | Some t => Some (make_width_explicit t)
              | None => None
              end
  | Ecast AsUInt e1 => match type_of_expr sc ckt e1 with
                        | Some (Gtyp (Fsint w))
  ...
  | Ecast AsSInt e1 => match type_of_expr sc ckt e1 with
                        | Some (Gtyp (Fsint w))
  ...
  | Ecast AsClock e1 => match type_of_expr sc ckt e1 with
                        | Some (Gtyp _) => Some (Gtyp Fclock)
                        | _ => None
                        end
  | Ecast AsAsync e1 => match type_of_expr sc ckt e1 with
                        | Some (Gtyp _) => Some (Gtyp Fasyncreset)
                        | _ => None
                        end
  | Eprim_unop (Upad n) e1 => match type_of_expr sc ckt e1 with
                              | Some (Gtyp (Fsint w)) => Some (Gtyp (Fsint (maxn w n)))
                              | Some (Gtyp (Fuint w)) => Some (Gtyp (Fuint (maxn w n)))
                              | _ => None
                              end
  | Eprim_unop (Ushl n) e1 => match type_of_expr sc ckt e1 with
                              | Some (Gtyp (Fsint w)) => Some (Gtyp (Fsint (w + n)))
                              | Some (Gtyp (Fuint w)) => Some (Gtyp (Fuint (w + n)))
                              | _ => None
                              end
  | Eprim_unop (Ushr n) e1 => match type_of_expr sc ckt e1 with
                              | Some (Gtyp (Fsint w)) => Some (Gtyp (Fsint (maxn (w - n) 1)))
                              | Some (Gtyp (Fuint w)) => Some (Gtyp (Fuint (w - n)))
                              | _ => None
                              end
  | Eprim_unop Ucvt e1 => match type_of_expr sc ckt e1 with
                          | Some (Gtyp (Fsint w)) => Some (Gtyp (Fsint w))
                          | Some (Gtyp (Fuint w)) => Some (Gtyp (Fsint (w + 1)))
                          | _ => None
                          end
  | Eprim_unop Uneg e1 => match type_of_expr sc ckt e1 with
                          | Some (Gtyp (Fsint w))
                          | Some (Gtyp (Fuint w)) => Some (Gtyp (Fsint (w + 1)))
                          | _ => None
                          end
  | Eprim_unop Unot e1 => match type_of_expr sc ckt e1 with
                          | Some (Gtyp (Fsint w))
                          | Some (Gtyp (Fuint w)) => Some (Gtyp (Fuint w))
                          | _ => None
                          end
  | Eprim_unop (Uextr n1 n2) e1 => match type_of_expr sc ckt e1 with
                                    | Some (Gtyp (Fsint w))
                                    | Some (Gtyp (Fuint w)) =>
  ...
  | Eprim_unop (Uhead n) e1 => match type_of_expr sc ckt e1 with
                                | Some (Gtyp (Fsint w))
                                | Some (Gtyp (Fuint w)) =>
  ...
  | Eprim_unop (Utail n) e1 => match type_of_expr sc ckt e1 with
                                | Some (Gtyp (Fsint w))
                                | Some (Gtyp (Fuint w)) =>
  ...
  | Eprim_unop _ e1 => match type_of_expr sc ckt e1 with
                        | Some (Gtyp (Fsint _))
                        | Some (Gtyp (Fuint _)) => Some (Gtyp (Fuint 1))
                        | _ => None
                        end
  | Eprim_binop (Bcomp _) e1 e2 => match type_of_expr sc ckt e1, type_of_expr sc ckt e2 with
                                    | Some (Gtyp (Fsint _)), Some (Gtyp (Fsint _))
                                    | Some (Gtyp (Fuint _)), Some (Gtyp (Fuint _)) => Some (Gtyp (Fuint 1))
                                    | _, _ => None
                                    end
  | Eprim_binop Badd e1 e2
  | Eprim_binop Bsub e1 e2 => match type_of_expr sc ckt e1, type_of_expr sc ckt e2 with
  ...
  | Eprim_binop Bmul e1 e2 => match type_of_expr sc ckt e1, type_of_expr sc ckt e2 with
                              | Some (Gtyp (Fuint w1)), Some (Gtyp (Fuint w2)) => Some (Gtyp (Fuint (w1 + w2)))
                              | Some (Gtyp (Fsint w1)), Some (Gtyp (Fsint w2)) => Some (Gtyp (Fsint (w1 + w2)))
                              | _, _ => None
                              end
  | Eprim_binop Bdiv e1 e2  => match type_of_expr sc ckt e1, type_of_expr sc ckt e2 with
                                | Some (Gtyp (Fuint w1)), Some (Gtyp (Fuint w2)) => Some (Gtyp (Fuint w1))
                                | Some (Gtyp (Fsint w1)), Some (Gtyp (Fsint w2)) => Some (Gtyp (Fsint (w1 + 1)))
                                | _, _ => None
                                end
  | Eprim_binop Brem e1 e2 => match type_of_expr sc ckt e1, type_of_expr sc ckt e2 with
  ...
  | Eprim_binop Bdshl e1 e2 => match type_of_expr sc ckt e1, type_of_expr sc ckt e2 with
  ...
  | Eprim_binop Bdshr e1 e2 => match type_of_expr sc ckt e1, type_of_expr sc ckt e2 with
                                | Some (Gtyp (Fuint w1)), Some (Gtyp (Fuint w2)) => Some (Gtyp (Fuint w1))
                                | Some (Gtyp (Fsint w1)), Some (Gtyp (Fuint w2)) => Some (Gtyp (Fsint w1))
                                | _, _ => None
                                end
  | Eprim_binop Bcat e1 e2 => match type_of_expr sc ckt e1, type_of_expr sc ckt e2 with
  ...
  | Eprim_binop Bxor e1 e2 => match type_of_expr sc ckt e1, type_of_expr sc ckt e2 with
                              | Some (Gtyp (Fuint w1)), Some (Gtyp (Fuint w2))
                              | Some (Gtyp (Fsint w1)), Some (Gtyp (Fsint w2)) => Some (Gtyp (Fuint (maxn w1 w2)))
                              | _, _ => None
                              end
  | Emux c e1 e2 => match type_of_expr sc ckt c, type_of_expr sc ckt e1, type_of_expr sc ckt e2 with
...
Fixpoint statement_kind (sc : VS.t) (ckt_earlier : VM.t (fcomponent * ftype)) (ssl : HiF.hfstmt_seq) : option (VS.t * VM.t (fcomponent * ftype)) :=
    match ssl with
    | Qnil => Some (sc, ckt_earlier)
    | Qcons Sskip ssl' => statement_kind sc ckt_earlier ssl'
    | Qcons (Swire w t) ssl' => if ~~VM.find w ckt_earlier then statement_kind (VS.add w sc) (VM.add w (Wire, t) ckt_earlier) ssl'
                                                           else None
    | Qcons (Sreg r rt) ssl' => if ~~VM.find r ckt_earlier then statement_kind (VS.add r sc) (VM.add r (Register, type rt) ckt_earlier) ssl'
                                                           else None
    | Qcons (Sinst i mi) ssl' => match VM.find i ckt_earlier, module_type mi with
                                 | None, Some t => statement_kind (VS.add i sc) (VM.add i (Instanceof, t) ckt_earlier) ssl'
                                 | _, _ => None
                                 end
    | Qcons (Snode n e) ssl' => match VM.find n ckt_earlier, type_of_expr (Some sc) ckt_earlier e with
                               | None, Some t => statement_kind (VS.add n sc) (VM.add n (Node, t) ckt_earlier) ssl'
                               | _, _ => None
                               end
    | Qcons (Sfcnct r e) ssl' => match type_of_ref (Some sc) ckt_earlier r, type_of_expr (Some sc) ckt_earlier e with
                                 | Some tr, Some te => if ftype_mux tr te then statement_kind sc ckt_earlier ssl'
                                                                          else None
                                 | _, _ => None
                                 end
    | Qcons (Sinvalid r) ssl' => if type_of_ref (Some sc) ckt_earlier r then statement_kind sc ckt_earlier ssl'
                                                                        else None
    | Qcons (Swhen c ssl_true ssl_false) ssl' => match type_of_expr (Some sc) ckt_earlier c, statement_kind sc ckt_earlier ssl_true with
                                                 | Some (Gtyp (Fuint _)), Some (_, ckt_true) => match statement_kind sc ckt_true ssl_false with
                                                                                                | Some (_, ckt_false) => statement_kind sc ckt_false ssl'
                                                                                                | None => None
                                                                                                end
                                                 | _, _ => None
                                                 end
...
Lemma statement_kind_inst_defined :
    forall (ssl : HiF.hfstmt_seq) (sc : VS.t) (ckt_earlier : VM.t (fcomponent * ftype)),
        statement_kind sc ckt_earlier ssl <> None -> hfstmt_seq_inst_in_mml ssl
with statement_kind_inst_defined_hfstmt :
    forall (s : HiF.hfstmt) (sc : VS.t) (ckt_earlier : VM.t (fcomponent * ftype)),
        statement_kind sc ckt_earlier (Qcons s HiF.qnil) <> None -> hfstmt_inst_in_mml s.
Proof.
* clear statement_kind_inst_defined.
  induction ssl ; intros ; first by done.
  simpl ; apply (introT andP).
  split.
  + apply (statement_kind_inst_defined_hfstmt h sc ckt_earlier).
    destruct h ; simpl ; simpl in H ; try done.
    - 1,2: destruct (~~ VM.find s ckt_earlier) ; by done.
    - 1,2: destruct (VM.find s ckt_earlier) ; first by done.
      destruct (module_type s0) ; by done.
    - destruct (type_of_expr (Some sc) ckt_earlier h) ; by done.
    - destruct (type_of_ref (Some sc) ckt_earlier h) ; last by done.
      destruct (type_of_expr (Some sc) ckt_earlier h0) ; last by done.
      destruct (isSome (ftype_mux f f0)) ; by done.
    - destruct (isSome (type_of_ref (Some sc) ckt_earlier h)) ; by done.
    - destruct (type_of_expr (Some sc) ckt_earlier h) as [[[| | | | | | |]| |]|] ; try by done.
      destruct (statement_kind sc ckt_earlier h0) as [[_ ckt_true]|] ; last by done.
      destruct (statement_kind sc ckt_true h1) as [[_ ckt_false]|] ; by done.
  + destruct h ; simpl in H ; try done.
    - by apply (IHssl _ _ H).
    - 1,2: destruct (~~ VM.find s ckt_earlier) ; last (by done) ;
              first (by apply (IHssl _ _ H)).
    - 1,2: destruct (VM.find s ckt_earlier) ; first by done.
      destruct (module_type s0) ; last by done.
      by apply (IHssl _ _ H).
    - destruct (type_of_expr (Some sc) ckt_earlier h) ; last by done.
      by apply (IHssl _ _ H).
    - destruct (type_of_ref (Some sc) ckt_earlier h) ; last by done.
      destruct (type_of_expr (Some sc) ckt_earlier h0) ; last by done.
      destruct (isSome (ftype_mux f f0)) ; last by done.
      by apply (IHssl _ _ H).
    - destruct (isSome (type_of_ref (Some sc) ckt_earlier h)) ; last by done.
      by apply (IHssl _ _ H).
    - destruct (type_of_expr (Some sc) ckt_earlier h) as [[[| | | | | | |]| |]|] ; try by done.
      destruct (statement_kind sc ckt_earlier h0) as [[_ ckt_true]|] ; last by done.
      destruct (statement_kind sc ckt_true h1) as [[_ ckt_false]|] ; last by done.
      by apply (IHssl _ _ H).
* clear statement_kind_inst_defined_hfstmt.
  intros ; destruct s ; simpl ; try done.
  - simpl in H.
    destruct (VM.find s ckt_earlier) ; first by done.
    rewrite /module_type in H.
    destruct (find_module s0) ; by done.
  - simpl in H.
    destruct (type_of_expr (Some sc) ckt_earlier h) as [[[| | | | | | |]| |]|] ; try by done.
    generalize (statement_kind_inst_defined h0 sc ckt_earlier) ; intro.
    destruct (statement_kind sc ckt_earlier h0) as [[sc_true ckt_true]|] ; last by done.
    apply (introT andP) ; split ; first (by apply H0 ; done).
    apply (statement_kind_inst_defined h1 sc ckt_true).
    destruct (statement_kind sc ckt_true h1) ; by done.
Qed.

Fixpoint module_kind (mml : seq HiF.hfmodule) : option (VM.t (VM.t (fcomponent * ftype))) :=
    match mml with
    | [::] => Some (VM.empty (VM.t (fcomponent * ftype)))
    | FInmod mi ppl ssl :: mml' => let pk := port_kind ppl
                                   in match statement_kind (MKS.key_set pk) pk ssl, module_kind mml' with
                                      | Some sk, Some ckt_earlier => Some (VM.add mi (snd sk) ckt_earlier)
                                      | _, _ => None
                                      end
    | _ => None
    end.

Lemma module_statement_kind :
    forall (mi : VarOrder.T) (ppl: seq HiF.hfport) (ssl : HiF.hfstmt_seq) (mml : seq HiF.hfmodule),
        module_kind mml <> None -> (FInmod mi ppl ssl) \in mml -> let pk := port_kind ppl
                                                                  in statement_kind (MKS.key_set pk) pk ssl <> None.
Proof.
clear mi_top.
induction mml0 ; intros ; first by rewrite in_nil // in H0.
simpl module_kind in H.
rewrite in_cons in H0 ; move /orP : H0 => [/eqP H0|H0].
* subst a pk.
  destruct (statement_kind (MKS.key_set (port_kind ppl))
        (port_kind ppl) ssl) ; done.
* destruct a as [mi_a ppl_a ssl_a|] ; last by done.
  destruct (statement_kind (MKS.key_set (port_kind ppl_a))
        (port_kind ppl_a) ssl_a) as [sk|] ; last by done.
  destruct (module_kind mml0) as [ckt_earlier|] ; last by done.
  apply IHmml0 ; done.
Qed.

Definition circuit_kind_uninferred : option (VM.t (VM.t (fcomponent * ftype))) :=
    module_kind mml.
...
Section Width_and_Reset_Inference.

Axiom uninferred_circuit_correct : circuit_kind_uninferred <> None.

Definition unify_width_gtyp (tw : fgtyp) (tr : fgtyp) : option (fgtyp * fgtyp) :=
    match tw, tr with
    | Fsint_implicit w, Fsint v
    | Fsint_implicit w, Fsint_implicit v => Some (Fsint_implicit (maxn w v), tr)
    | Fuint_implicit w, Fuint v
    | Fuint_implicit w, Fuint_implicit v => Some (Fuint_implicit (maxn w v), tr)
    | Fuint_implicit w, Fuint_implicit_eq1 => Some (Fuint_implicit (maxn w 1), tr)
    | Freset, Fuint 1
    | Freset, Fasyncreset
    | Freset, Freset => Some (tr, tr)
    | Fuint 1, Freset
    | Fasyncreset, Freset => Some (tw, tw)
    | Freset, Fuint_implicit 0
    | Freset, Fuint_implicit 1
    | Freset, Fuint_implicit_eq1 => Some (Fuint_implicit_eq1, tr)
    | Fuint_implicit 0, Freset
    | Fuint_implicit 1, Freset
    | Fuint_implicit_eq1, Freset => Some (Fuint_implicit_eq1, Fuint_implicit_eq1)
    | Freset, _
    | _, Freset => None
    | Fuint_implicit_eq1, Fuint 0
    | Fuint_implicit_eq1, Fuint 1
    | Fuint_implicit_eq1, Fuint_implicit 0
    | Fuint_implicit_eq1, Fuint_implicit 1
    | Fuint_implicit_eq1, Fuint_implicit_eq1 => Some (tw, tr)
    | Fuint_implicit_eq1, _ => None
    | _, _ => Some (tw, tr)
    end.

Fixpoint unify_width (fl : fflip) (tw : ftype) (tr : ftype) : option (ftype * ftype) :=
    match tw, tr, fl with
    | Gtyp gtw, Gtyp gtr, Nflip => match unify_width_gtyp gtw gtr with
                                   | Some (ugtw, ugtr) => Some (Gtyp ugtw, Gtyp ugtr)
                                   | None => None
                                   end
    | Gtyp gtw, Gtyp gtr, Flipped => match unify_width_gtyp gtr gtw with
                                     | Some (ugtr, ugtw) => Some (Gtyp ugtw, Gtyp ugtr)
                                     | None => None
                                     end
    | Atyp tw' nw, Atyp tr' nr, _ => if nw==nr then match unify_width fl tw' tr' with
                                                    | Some (utw', utr') => Some (Atyp utw' nw, Atyp utr' nr)
                                                    | None => None
                                                    end
                                               else None
    | Btyp ffw, Btyp ffr, _ => match unify_width_fields fl ffw ffr with
                               | Some (uffw, uffr) => Some (Btyp uffw, Btyp uffr)
                               | None => None
                               end
    | _, _, _ => None
    end
with unify_width_fields (fl : fflip) (ffw : ffield) (ffr : ffield) : option (ffield * ffield) :=
    match ffw, ffr with
    | Fnil, Fnil => Some (Fnil, Fnil)
    | Fflips vw fw tw ffw', Fflips vr fr tr ffr' => if (vw==vr) && (fw==fr) then match unify_width (if fl==fw then Nflip else Flipped) tw tr, unify_width_fields fl ffw' ffr' with
                                                                                 | Some (utw, utr), Some (uffw', uffr') => Some (Fflips vw fw utw uffw', Fflips vr fr utr uffr')
                                                                                 | _, _ => None
                                                                                 end
                                                                            else None
    | _, _ => None
...
(* Helper function for repl_type_part.
It finds field v in ff and returns a type with a hole, and also the old type of field v. *)
Fixpoint repl_ffield_part (v : VarOrder.T) (ff : ffield) : option ((ftype -> ffield) * ftype) :=
    match ff with
    | Fnil => None
    | Fflips v' fl t ff' => if v==v' then Some (fun (t' : ftype) => Fflips v' fl t' ff', t)
                                     else match repl_ffield_part v ff' with
                                          | Some (ffn, old_t) => Some (fun (t' : ftype) => Fflips v' fl t (ffn t'), old_t)
                                          | None => None
                                          end
    end.

(* replace the part indicated by r of old_comp_type by new_part_type
For example, if
r = Eid v then v has type old_comp_type and it should become new_part_type
r = Esubindex v 1 then v has type Atyp something n and it should become Atyp new_part_type n
r = Esubfield (Esubindex v 1) f then v has a type like Atyp (Btyp (Fflips f Nflip something Fnil)) n

So, how can we make this function handle the two recursions that go the other way round?

We do it like this:
the function returns a "type with a hole", that is a type where one has to insert the new_part_type later.
It also returns the original type that was in the hole.
*)
Fixpoint repl_type_part' (r : HiF.href) (old_comp_type : ftype) : option ((ftype -> ftype) * ftype) :=
    match r with
    | Eid _ => Some (fun (t : ftype) => t, old_comp_type)
    | Esubfield r' v => match repl_type_part' r' old_comp_type with
                        | Some (fn, Btyp ff) => match repl_ffield_part v ff with
                                                | Some (ffn, old_t) => Some (fun (t : ftype) => fn (Btyp (ffn t)), old_t)
                                                | None => None
                                                end
                        | _ => None
                        end
    | Esubindex r' _
    | Esubaccess r' _ => match repl_type_part' r' old_comp_type with
                         | Some (fn, Atyp old_t n) => Some (fun (t : ftype) => fn (Atyp t n), old_t)
                         | _ => None
                         end
    end.

Definition repl_type_part (r : HiF.href) (old_comp_type : ftype) (new_part_type : ftype) : option ftype :=
    match repl_type_part' r old_comp_type with
    | Some (fn, _) => Some (fn new_part_type)
    | None => None
    end.

Fixpoint base_ref (r : HiF.href) : VarOrder.T :=
    match r with
    | Eid v => v
    | Esubfield r' _
    | Esubindex r' _
    | Esubaccess r' _ => base_ref r'
    end.

Fixpoint width_iter (ckt : VM.t (fcomponent * ftype)) (ssl : HiF.hfstmt_seq) : option (VM.t (fcomponent * ftype)) :=
    match ssl with
    | Qnil => Some ckt
    | Qcons Sskip ssl'
    | Qcons (Swire _ _) ssl'
    | Qcons (Sreg _ _) ssl'
    | Qcons (Sinst _ _) ssl' => width_iter ckt ssl'
    | Qcons (Snode n e) ssl' => match type_of_expr None ckt e with
                                | Some t => width_iter (VM.add n (Node, t) ckt) ssl'
                                | None => None
                                end
    | Qcons (Sfcnct r (Eref r')) ssl' => match type_of_ref None ckt r, type_of_ref None ckt r' with
                                         | Some tr, Some tr' => match unify_width Nflip tr tr', VM.find (base_ref r) ckt with
                                                                | Some (ut, ut'), Some (bc, bt) => match repl_type_part r bt ut with
                                                                                                   | Some nt => let ckt_r := VM.add (base_ref r) (bc, nt) ckt
                                                                                                                in match VM.find (base_ref r') ckt_r with
                                                                                                                   | Some (bc', bt') => match repl_type_part r' bt' ut' with
                                                                                                                                        | Some nt' => width_iter (VM.add (base_ref r') (bc', nt') ckt_r) ssl'
                                                                                                                                        | None => None
                                                                                                                                        end
                                                                                                                   | None => None
                                                                                                                   end
    ...
    | Qcons (Sfcnct r e) ssl' => match type_of_ref None ckt r, type_of_expr None ckt e with
                                 | Some tr, Some te => match unify_width Nflip tr te, VM.find (base_ref r) ckt with
                                                       | Some (ut, _), Some (bc, bt) => match repl_type_part r bt ut with
                                                                                        | Some nt => width_iter (VM.add (base_ref r) (bc, nt) ckt) ssl'
                                                                                        | None => None
                                                                                        end
                                                       | _, _ => None
                                                       end
    ...
    | Qcons (Sinvalid r) ssl' => width_iter ckt ssl'
    | Qcons (Swhen c ssl_true ssl_false) ssl' => match width_iter ckt ssl_true with
                                                 | Some ckt_true => match width_iter ckt_true ssl_false with
                                                                    | Some ckt_false => width_iter ckt_false ssl'
                                                                    | None => None
                                                                    end
                                                 | None => None
                                                 end
...
Fixpoint port_type_with_widths_from_ckt (ckt_mi' : VM.t (fcomponent * ftype)) (ppl : seq HiF.hfport) : option ffield :=
    match ppl with
    | [::] => Some Fnil
    | Finput p _ :: ppl' => match VM.find p ckt_mi', port_type_with_widths_from_ckt ckt_mi' ppl' with
                            | Some (In_port, t), Some ff => Some (Fflips p Flipped t ff)
                            | _, _ => None
                            end
    | Foutput p _ :: ppl' => match VM.find p ckt_mi', port_type_with_widths_from_ckt ckt_mi' ppl' with
                             | Some (Out_port, t), Some ff => Some (Fflips p Nflip t ff)
                             | _, _ => None
                             end
    end.

(* unify widths down, that is, copy width information from module definitions to their instances in module mi.
   ckt is the complete map of all components (read-only).
   ckt_mi is the map for the current module (this map is being changed) = (VM.find mi ckt).
          We provide two separate maps because changing ckt may be less efficient than only changing ckt_mi.
   ssl is the list of statements in module mi. *)
Fixpoint unify_width_down (ckt : VM.t (VM.t (fcomponent * ftype))) (ckt_mi : VM.t (fcomponent * ftype)) (ssl : HiF.hfstmt_seq) : option (VM.t (fcomponent * ftype)) :=
    (* go through all statements in module mi, which has statements ssl *)
    match ssl with
    | Qnil => Some ckt_mi
    | Qcons (Sinst ii mi') ssl' => match find_module mi', VM.find mi' ckt with
                                   | Some (FInmod _ ppl _), Some ckt_mi' => match port_type_with_widths_from_ckt ckt_mi' ppl with
                                                                            | Some t => unify_width_down ckt (VM.add ii (Instanceof, Btyp t) ckt_mi) ssl'
                                                                            | None => None
                                                                            end
                                   | _, _ => None
                                   end
    | Qcons _ ssl' => unify_width_down ckt ckt_mi ssl'
    end.

Fixpoint unify_ports_up (ckt_mi' : VM.t (fcomponent * ftype)) (ff : ffield) : VM.t (fcomponent * ftype) :=
    match ff with
    | Fnil => ckt_mi'
    | Fflips p Flipped t ff' => unify_ports_up (VM.add p (In_port , t) ckt_mi') ff'
    | Fflips p Nflip   t ff' => unify_ports_up (VM.add p (Out_port, t) ckt_mi') ff'
    end.

(* unify widths up, that is, copy width information from instances in module mi to their definitions.
   ckt is the complete map of all components (this map is being changed).
   ckt_mi is the map for the current module (read-only) = (VM.find mi ckt).
          We provide two separate maps because calling VM.find mi ckt every time may be less efficien.t
   ssl is the list of statements in module mi. *)
Fixpoint unify_width_up (ckt : VM.t (VM.t (fcomponent * ftype))) (ckt_mi : VM.t (fcomponent * ftype)) (ssl : HiF.hfstmt_seq) : option (VM.t (VM.t (fcomponent * ftype))) :=
    match ssl with
    | Qnil => Some ckt
    | Qcons (Sinst ii mi') ssl' => match VM.find mi' ckt, VM.find ii ckt_mi with
                                   | Some ckt_mi', Some (Instanceof, Btyp ff) => unify_width_up (VM.add mi' (unify_ports_up ckt_mi' ff) ckt) ckt_mi ssl'
                                   | _, _ => None
                                   end
    | Qcons _ ssl' => unify_width_up ckt ckt_mi ssl'
    end.

Fixpoint circuit_width_iter' (mml : seq HiF.hfmodule) (ckt : VM.t (VM.t (fcomponent * ftype))) : option (VM.t (VM.t (fcomponent * ftype))) :=
    match mml with
    | [::] => Some ckt
    | FInmod mi _ ssl_i :: mml' => match VM.find mi ckt with
                                   | Some ckt_mi => match unify_width_down ckt ckt_mi ssl_i with
                                                    | Some ckt_mi_down => match width_iter ckt_mi_down ssl_i with
                                                                          | Some ckt_mi_unif => match unify_width_up (VM.add mi ckt_mi_unif ckt) ckt_mi_unif ssl_i with
                                                                                                | Some ckt_mi_up => circuit_width_iter' mml' ckt_mi_up
                                                                                                | None => None
                                                                                                end
                                                                          | None => None
                                                                          end
...
Definition circuit_width_iter (ckt : VM.t (VM.t (fcomponent * ftype))) : option (VM.t (VM.t (fcomponent * ftype))) :=
    circuit_width_iter' mml ckt.

(* count the number of ground type components in type t for width inference.
   Because arrays have only one type, an array does not need to be counted multiple times. *)
Fixpoint Gtyp_comp_count (t : ftype) : nat :=
    match t with
    | Gtyp _ => 1
    | Atyp t' _ => Gtyp_comp_count t'
    | Btyp ff => Gtyp_comp_count_fields ff
    end
with Gtyp_comp_count_fields (ff : ffield) : nat :=
    match ff with
    | Fnil => 0
    | Fflips _ _ t ff' => Gtyp_comp_count t + Gtyp_comp_count_fields ff'
    end.

(* counts the number of iterations that is needed at most, based on the the connect statements in ssl *)
Fixpoint max_iter_count (ckt_mi : VM.t (fcomponent * ftype)) (ssl : HiF.hfstmt_seq) : option nat :=
    match ssl with
    | Qnil => Some 0
    | Qcons (Sfcnct r e) ssl' => match type_of_ref None ckt_mi r, max_iter_count ckt_mi ssl' with
                                 | Some tr, Some it => Some (Gtyp_comp_count tr + it)
                                 | _, _ => None
                                 end
    | Qcons _ ssl' => max_iter_count ckt_mi ssl'
    end.

Fixpoint module_max_iter_count (mml : seq HiF.hfmodule) (ckt : VM.t (VM.t (fcomponent * ftype))) : option nat :=
    match mml with
    | [::] => Some 0
    | FInmod mi _ ssl :: mml' => match VM.find mi ckt with
                                 | Some ckt_mi => match max_iter_count ckt_mi ssl, module_max_iter_count mml' ckt with
...
(* maximum number of iterations needed to find the fixed point of width inference.
   ckt is not changed; it is used to find the type of references. *)
Definition circuit_max_iter_count (ckt : VM.t (VM.t (fcomponent * ftype))) : option nat :=
    module_max_iter_count mml ckt.

Fixpoint circuit_kind_iterator (n : nat) (ckt : VM.t (VM.t (fcomponent * ftype))) : option (VM.t (VM.t (fcomponent * ftype))) :=
    match n with
    | 0 => Some ckt
    | n'.+1 => match circuit_width_iter ckt with
               | Some ckt' => circuit_kind_iterator n' ckt'
               | None => None
               end
    end.

(* kinds and types of all components in the modules after width inference *)
Definition circuit_kind : option (VM.t (VM.t (fcomponent * ftype))) := 
    match circuit_kind_uninferred with
    | Some ckt_uninf => circuit_kind_iterator (circuit_max_iter_count ckt_uninf) ckt_uninf
    | None => None
    end.

(* Perhaps prove that this leads to a fixed point? *)

End Width_and_Reset_Inference.

Section Single_Cycle_Dynamics.

Axiom static_circuit_correct : circuit_kind <> None.

(* identifiers for instances *)
(* To identify an instance, we use a sequence of identifiers.
   The empty sequence identifies the top instance,
   and if the instance identified with ``iil'' contains a statement ``inst i of m'',
   then the instance created by such statement is identified by ``i :: iil''.
   (Note that we place the innermost level at the beginning to simplify handling,
   because sequences are constructed this way.) *)

(* inductive definition of instance *)

(* the statement list contains ``Sinst ii mi'' *)
Fixpoint statements_contain_instance (ssl : HiF.hfstmt_seq) (ii : VarOrder.T) (mi : VarOrder.T) : bool :=
    match ssl with
    | Qnil => false
    | Qcons (Sinst ii' mi') ssl' => ((ii==ii') && (mi==mi')) || statements_contain_instance ssl' ii mi
    | Qcons (Swhen _ ssl_true ssl_false) ssl' => statements_contain_instance ssl_true ii mi || statements_contain_instance ssl_false ii mi || statements_contain_instance ssl' ii mi
    | Qcons _ ssl' => statements_contain_instance ssl' ii mi
    end.

(* the module with identifier mi_outer contains an instance with identifier ii, which has module type mi_inner *)
Definition contains_instance (mi_outer : VarOrder.T) (ii : VarOrder.T) (mi_inner : VarOrder.T) : bool :=
    match find_module mi_outer with
    | Some (FInmod _ _ ssl) => statements_contain_instance ssl ii mi_inner
    | _ => false
    end.

Inductive inst : seq VarOrder.T (* sequence of instance identifiers *) -> VarOrder.T (* module identifier *) -> Type :=
| inst_top : inst [::] mi_top
| inst_sub : forall (iil : seq VarOrder.T) (mi_outer : VarOrder.T) (ii : VarOrder.T) (mi_inner : VarOrder.T),
             inst iil mi_outer ->
             contains_instance mi_outer ii mi_inner -> inst (ii :: iil) mi_inner.

(* I am sure that there is a lemma proving this but I cannot find it. *)
Lemma in_all {T : eqType} : forall (a : pred T) (el : T) (s : seq T), el \in s -> all a s -> a el.
Proof.
intros.
induction s ; first by (rewrite in_nil // in H).
simpl all in H0 ; move /andP : H0 => H0.
rewrite in_cons in H ; move /orP : H => [/eqP H|H].
* subst a0.
  apply H0.
* apply (IHs H), H0.
Qed.

(* Now an element of inst is an instance identifier, which also carries the module identifier related to it. *)
Lemma inst_exists : forall (ii : seq VarOrder.T) (mi : VarOrder.T) (im : inst ii mi),
    match find_module mi with
    | Some (FInmod _ _ _) => True
    | _ => False
    end.
Proof.
intros.
destruct im.
* specialize No_external_modules ; specialize Top_module_present ; specialize (find_module_in (mi := mi_top)) ; intros.
  destruct (find_module mi_top) ; last by done.
  clear H0.
  specialize (H h Logic.eq_refl).
  specialize (in_all H H1) ; intro ; simpl in H0.
  destruct h ; done.
* specialize uninferred_circuit_correct ; specialize No_external_modules ; specialize (find_module_in (mi := mi_outer)) ; intros.
  rewrite /circuit_kind_uninferred in H1.
  rewrite /contains_instance in i.
  destruct (find_module mi_outer) as [[mi_outer' ppl_outer ssl_outer|]|] ; try done.
  specialize (H _ Logic.eq_refl).
  generalize (module_statement_kind H1 H) ; intro.
  (* Now use that statement_kind ... ssl_outer <> None to prove that Sinst statements in ssl_outer
     refer to a module that is actually in mml. So, find_module mi_inner is Some (an element of mml). *)
  (* by induction over ssl_outer? (needs also induction into Swhen...) *)
  apply statement_kind_inst_defined in H2.
  clear H H0 H1 iil mi_outer im mi_outer' ppl_outer.
  (* We now need a combined induction over ssl_outer and a statement in a similar position. *)

  revert i H2.
  induction ssl_outer ; intros ; first by simpl in i ; done.
  simpl in i ; simpl in H2.

At this point I can see that one would run into trouble with Swhen statements;
the induction hypothesis is insufficient to handle them. Therefore we really need
the combined induction.

(* values for one instance *)

End Single_Cycle_Dynamics.
...
(* evaluates statement st.
   rs = updated values of registers
   ns = updated values of other variables
   s = old values of variables (values to be read)
   tmap = data type and component type for every defined component
   Returns a pair (rs possibly with further updates, ns possibly with further updates) *)
...
(* The lemma states that iterate achieves the least fixed point.
   It should have an additional premissa on n, basically the requirement that n is larger than the longest connect-path in the structure.
   A safe bound could be: n is larger than the number of basic components in the statements ss. 
   Perhaps a possible inductive statement is: after n iterations, at least n basic components have reached their fixpoint value,
   or all basic components have reached their fixpoint value.
   Proof idea: if not every basic component has reached its fixpoint value, and there are no combinatorial cycles,
   then there is some additional basic component whose connections only depend on those who have already reached their fixpoint value.
   Requires to register which components have already reached their fixpoint value---a (finite) set of basic components! 

   A helper lemma could then be: given a dependency-downward-closed set of basic components,
   there is at least one basic component that only depends on components in the set (but is not in the set).
   Adding that component to the set will create a larger dependency-downward-closed set. *)
...
Lemma eval_hfstmts_find_none_cases_helper ss init_s tmap rs s temp_rs temp_s v : 
  match PVM.find v tmap with
  | Some (_, In_port) => PVM.find v temp_s = None /\ PVM.find v temp_rs = None
  | Some (_, Instanceof) => PVM.find v temp_s = None /\ PVM.find v temp_rs = None
  | Some (_, Memory) => PVM.find v temp_s = None /\ PVM.find v temp_rs = None
  | Some (_, Fmodule) => PVM.find v temp_s = None /\ PVM.find v temp_rs = None
  | Some (_, Register) => PVM.find v temp_s = None
  | None => PVM.find v temp_s = None /\ PVM.find v temp_rs = None
...
with eval_hfstmt_find_rs_helper st init_s tmap rs s temp_rs temp_s v : 
  match PVM.find v tmap with
  | Some (_, In_port) => PVM.find v temp_s = None /\ PVM.find v temp_rs = None
  | Some (_, Instanceof) => PVM.find v temp_s = None /\ PVM.find v temp_rs = None
  | Some (_, Memory) => PVM.find v temp_s = None /\ PVM.find v temp_rs = None
  ...
  match PVM.find v tmap with
  | Some (_, In_port) => (forall r, ~ hfstmt_eqn (Sreg v r) st) /\ (forall e, ~ hfstmt_eqn (Snode v e) st)
      /\ (forall e, ~ hfstmt_eqn (Sfcnct (Eid v) e) st) /\ (~ hfstmt_eqn (Sinvalid (Eid v)) st)
  | Some (_, Instanceof) => (forall r, ~ hfstmt_eqn (Sreg v r) st) /\ (forall e, ~ hfstmt_eqn (Snode v e) st)
      /\ (forall e, ~ hfstmt_eqn (Sfcnct (Eid v) e) st) /\ (~ hfstmt_eqn (Sinvalid (Eid v)) st)
  | Some (_, Memory) => (forall r, ~ hfstmt_eqn (Sreg v r) st) /\ (forall e, ~ hfstmt_eqn (Snode v e) st)
      /\ (forall e, ~ hfstmt_eqn (Sfcnct (Eid v) e) st) /\ (~ hfstmt_eqn (Sinvalid (Eid v)) st)
  | Some (_, Node) => forall r, ~ hfstmt_eqn (Sreg v r) st
  | Some (_, Out_port) => forall r, ~ hfstmt_eqn (Sreg v r) st
  | Some (_, Wire) => forall r, ~ hfstmt_eqn (Sreg v r) st
  | Some (_, Fmodule) => (forall r, ~ hfstmt_eqn (Sreg v r) st) /\ (forall e, ~ hfstmt_eqn (Snode v e) st)
      /\ (forall e, ~ hfstmt_eqn (Sfcnct (Eid v) e) st) /\ (~ hfstmt_eqn (Sinvalid (Eid v)) st)
  | Some (_, Register) => (forall e, ~ hfstmt_eqn (Snode v e) st)
  | None => (forall r, ~ hfstmt_eqn (Sreg v r) st) /\ (forall e, ~ hfstmt_eqn (Snode v e) st)
  end ->
  Sem_HiFP.eval_hfstmt st temp_rs temp_s init_s tmap = Some (rs, s) ->
  match PVM.find v tmap with
  | Some (_, In_port) => PVM.find v s = None /\ PVM.find v rs = None
...
Proof. 
  move : ss temp_rs temp_s rs s. elim. 
  simpl; intros. inversion H0; subst rs s. apply H. 
  simpl; intros hd tl IH; intros. clear eval_hfstmts_find_none_cases_helper.
  destruct (Sem_HiFP.eval_hfstmt hd temp_rs temp_s init_s tmap) as [[rs0 ns0]|] eqn : Heval; try discriminate.
  move : H0; apply IH; clear IH. move : Heval; apply (eval_hfstmt_find_rs_helper _ _ _ _ _ _ _ _ H). admit.

  intros Hinit Hunique Heval. destruct st as [|v0 t|v0 r|v0 m|v0 v1|v0 e0|v0 e0|v0|c s1 s2] eqn : Hst; simpl in *.
  (* skip, wire *) 1,2,4,5 : inversion Heval; subst rs s; apply Hinit.
  (* reg *) destruct (PVM.find v0 init_s) as [val|]; try discriminate. inversion Heval; subst rs s; clear Heval.
    destruct (PVM.find v tmap) as [[gt cmpnt]|] eqn : Hfind. destruct cmpnt; try done.
    - (* inport *)
      move : Hinit => [Htemp_s Htemp_rs]. split; try done. rewrite PVM.Lemmas.find_add_neq //. move : Hunique => [Hunique _].
      specialize (Hunique r). move : Hunique; apply contra_not; intro. unfold PVM.M.SE.eq in H. 
      rewrite H eq_refl //.
    - (* inst *)
      move : Hinit => [Htemp_s Htemp_rs]. split; try done. rewrite PVM.Lemmas.find_add_neq //. move : Hunique => [Hunique _].
      specialize (Hunique r). move : Hunique; apply contra_not; intro. unfold PVM.M.SE.eq in H. 
    ...
    - (* memory *)
      ...
      rewrite H eq_refl //.
    - (* node *)
      rewrite PVM.Lemmas.find_add_neq //. 
      specialize (Hunique r). move : Hunique; apply contra_not; intro. unfold PVM.M.SE.eq in H. 
      rewrite H eq_refl //.
    - (* outport *)
      rewrite PVM.Lemmas.find_add_neq //. 
    ...
    - (* none *)
      ...
      specialize (Hunique r). move : Hunique; apply contra_not; intro. unfold PVM.M.SE.eq in H. 
      rewrite H eq_refl //.
  (* node *) destruct (Sem_HiFP.eval_hfexpr e0 init_s tmap) as [val|]; try discriminate. inversion Heval; subst rs s; clear Heval.
    destruct (PVM.find v tmap) as [[gt cmpnt]|] eqn : Hfind. destruct cmpnt; try done.
    - (* inport *)
      move : Hinit => [Htemp_s Htemp_rs]. split; try done. rewrite PVM.Lemmas.find_add_neq //. move : Hunique => [_ [Hunique _]].
      specialize (Hunique e0). move : Hunique; apply contra_not; intro. unfold PVM.M.SE.eq in H. 
      rewrite H eq_refl //.
    - (* inst *)
      move : Hinit => [Htemp_s Htemp_rs]. split; try done. rewrite PVM.Lemmas.find_add_neq //. move : Hunique => [_ [Hunique _]].
      specialize (Hunique e0). move : Hunique; apply contra_not; intro. unfold PVM.M.SE.eq in H. 
    ...
    - (* module *)
      ...
      rewrite H eq_refl //.
    - (* none *)
      move : Hinit => [Htemp_s Htemp_rs]. split; try done. rewrite PVM.Lemmas.find_add_neq //. move : Hunique => [_ Hunique].
      specialize (Hunique e0). move : Hunique; apply contra_not; intro. unfold PVM.M.SE.eq in H. 
      rewrite H eq_refl //.
  (* cnct *) destruct v0 as [ref|a b|a b|a b]; try (inversion Heval; subst rs s; try done). 
    destruct (PVM.find ref tmap) as [[gt_ref cmpnt_ref]|] eqn : Hgt; try discriminate. destruct (v == ref) eqn : Heq. 
    (* v == ref *) move /eqP : Heq => Heq; subst ref. rewrite Hgt in Hunique Hinit; rewrite Hgt.
      destruct cmpnt_ref; destruct (Sem_HiFP.eval_hfexpr e0 init_s tmap); try discriminate; inversion Heval; subst rs s; clear Heval; try done.
      (* inport, inst, memory, module *)
      1-4 : move : Hunique => [_ [_ [Hunique _]]]; specialize (Hunique e0); rewrite eq_refl in Hunique; simpl in Hunique;
      rewrite eq_refl in Hunique; simpl in Hunique; done.
    (* v <> ref *) destruct cmpnt_ref; destruct (Sem_HiFP.eval_hfexpr e0 init_s tmap); try discriminate; 
      inversion Heval; subst rs s; clear Heval; try done; try rewrite PVM.Lemmas.find_add_neq; try done.
      1-8 : unfold PVM.M.SE.eq; rewrite Heq //.
  (* invalid *) destruct v0 as [ref|a b|a b|a b]; try (inversion Heval; subst rs s; try done). 
    destruct (PVM.find ref tmap) as [[gt_ref cmpnt_ref]|] eqn : Hgt; try discriminate. destruct (v == ref) eqn : Heq. 
    (* v == ref *) move /eqP : Heq => Heq; subst ref. rewrite Hgt in Hunique Hinit; rewrite Hgt.
      destruct cmpnt_ref; destruct (sizeof_fgtyp gt_ref < length indeterminate_val); inversion Heval; subst rs s; clear Heval; try done.
      (* inport, inst, memory, module *)
      1-8 : move : Hunique => [_ [_ [_ Hunique]]]; rewrite eq_refl in Hunique; simpl in Hunique; done.
    (* v <> ref *) destruct cmpnt_ref; destruct (sizeof_fgtyp gt_ref < length indeterminate_val); 
      inversion Heval; subst rs s; clear Heval; try done; try rewrite PVM.Lemmas.find_add_neq; try done.
      1-16 : unfold PVM.M.SE.eq; rewrite Heq //.
  (* when *) destruct (Sem_HiFP.eval_hfexpr c init_s tmap) as [valc|]; try discriminate. destruct (~~ is_zero valc).
    move : Heval; apply eval_hfstmts_find_none_cases_helper; try done.
    move : Heval; apply eval_hfstmts_find_none_cases_helper; try done.
Admitted.

Lemma eval_hfstmts_find_none_cases ss init_s tmap rs s v : Sem_HiFP.eval_hfstmts ss (PVM.empty bits) (PVM.empty bits) init_s tmap = Some (rs, s) ->
  match PVM.find v tmap with
  | Some (_, In_port) => PVM.find v s = None /\ PVM.find v rs = None
  | Some (_, Instanceof) => PVM.find v s = None /\ PVM.find v rs = None
  | Some (_, Memory) => PVM.find v s = None /\ PVM.find v rs = None
