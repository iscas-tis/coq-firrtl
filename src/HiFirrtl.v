...
Section HiFirrtl.
  ...
  Proof. clear hfexpr_eq_dec.
    ...
    1-6, 12, 18, 24, 30: destruct (b0 == b1) eqn: H ; move /eqP : H => H ;
      first (by left  ; exact H) ;
      last  (by right ; exact H).
    1-25: destruct (b == b0) eqn: H ; move /eqP : H => H ;
      first (by left  ; exact H) ;
      last  (by right ; exact H).
  * clear href_eq_dec.
    ...
    + destruct (s1 == s2) eqn: H ; move /eqP : H => H ;
      first (by left  ; exact H) ;
      last  (by right ; exact H).
    1,2,3,8-11: destruct (s1 == s2) eqn: H ; move /eqP : H => H ;
      first (by left  ; exact H) ;
      last  (by right ; exact H).
    1-12,17-20: destruct (s0 == s1) eqn: H ; move /eqP : H => H ;
      first (by left  ; exact H) ;
      last  (by right ; exact H).
    1-8: destruct (s == s0) eqn: H ; move /eqP : H => H ;
      first (by left  ; exact H) ;
      last  (by right ; exact H).
  Qed.
...
Lemma Qremove_when_Qin_when : forall s s0 ss, Qin_when s0 (Qremove_when s ss) -> Qin_when s0 ss.
Proof.
  (*fix F 3.
  intros s s0 ss.
  destruct ss as [|hd tl].
  - simpl. done.
  - destruct hd as [ | v f | v r | v m | v w | v e | r e | r | c s1 s2 ]; simpl in *.
    (* ---------- Sskip ---------- *)
    + destruct (hfstmt_eqn (Sskip) s) eqn:Heq.
      * intros H. apply orb_true_iff; right. apply F with (s:=s) (s0:=s0) (ss:=tl). exact H.
      * intros H. destruct (hfstmt_eqn (Sskip) s0) eqn:Heq0.
        -- apply orb_true_iff; left. exact Heq0.
        -- apply orb_true_iff; right. apply F with (s:=s) (s0:=s0) (ss:=tl) in H. exact H.
    (* ---------- Swire ---------- *)
    + destruct (hfstmt_eqn (Swire v f) s) eqn:Heq.
      * intros H. apply orb_true_iff; right. apply F with (s:=s) (s0:=s0) (ss:=tl). exact H.
      * intros H. destruct (hfstmt_eqn (Swire v f) s0) eqn:Heq0.
        -- apply orb_true_iff; left. exact Heq0.
        -- apply orb_true_iff; right. apply F with (s:=s) (s0:=s0) (ss:=tl) in H. exact H.
    (* ---------- Sreg ---------- *)
    + destruct (hfstmt_eqn (Sreg v r) s) eqn:Heq.
      * intros H. apply orb_true_iff; right. apply F with (s:=s) (s0:=s0) (ss:=tl). exact H.
      * intros H. destruct (hfstmt_eqn (Sreg v r) s0) eqn:Heq0.
        -- apply orb_true_iff; left. exact Heq0.
        -- apply orb_true_iff; right. apply F with (s:=s) (s0:=s0) (ss:=tl) in H. exact H.
    (* ---------- Smem ---------- *)
    + destruct (hfstmt_eqn (Smem v m) s) eqn:Heq.
      * intros H. apply orb_true_iff; right. apply F with (s:=s) (s0:=s0) (ss:=tl). exact H.
      * intros H. destruct (hfstmt_eqn (Smem v m) s0) eqn:Heq0.
        -- apply orb_true_iff; left. exact Heq0.
        -- apply orb_true_iff; right. apply F with (s:=s) (s0:=s0) (ss:=tl) in H. exact H.
    (* ---------- Sinst ---------- *)
    + destruct (hfstmt_eqn (Sinst v w) s) eqn:Heq.
      * intros H. apply orb_true_iff; right. apply F with (s:=s) (s0:=s0) (ss:=tl). exact H.
      * intros H. destruct (hfstmt_eqn (Sinst v w) s0) eqn:Heq0.
        -- apply orb_true_iff; left. exact Heq0.
        -- apply orb_true_iff; right. apply F with (s:=s) (s0:=s0) (ss:=tl) in H. exact H.
    (* ---------- Snode ---------- *)
    + destruct (hfstmt_eqn (Snode v e) s) eqn:Heq.
      * intros H. apply orb_true_iff; right. apply F with (s:=s) (s0:=s0) (ss:=tl). exact H.
      * intros H. destruct (hfstmt_eqn (Snode v e) s0) eqn:Heq0.
        -- apply orb_true_iff; left. exact Heq0.
        -- apply orb_true_iff; right. apply F with (s:=s) (s0:=s0) (ss:=tl) in H. exact H.
    (* ---------- Sfcnct ---------- *)
    + destruct (hfstmt_eqn (Sfcnct r e) s) eqn:Heq.
      * intros H. apply orb_true_iff; right. apply F with (s:=s) (s0:=s0) (ss:=tl). exact H.
      * intros H. destruct (hfstmt_eqn (Sfcnct r e) s0) eqn:Heq0.
        -- apply orb_true_iff; left. exact Heq0.
        -- apply orb_true_iff; right. apply F with (s:=s) (s0:=s0) (ss:=tl) in H. exact H.
    (* ---------- Sinvalid ---------- *)
    + destruct (hfstmt_eqn (Sinvalid r) s) eqn:Heq.
      * intros H. apply orb_true_iff; right. apply F with (s:=s) (s0:=s0) (ss:=tl). exact H.
      * intros H. destruct (hfstmt_eqn (Sinvalid r) s0) eqn:Heq0.
        -- apply orb_true_iff; left. exact Heq0.
        -- apply orb_true_iff; right. apply F with (s:=s) (s0:=s0) (ss:=tl) in H. exact H.
    (* ---------- Swhen ---------- *)
    + remember (Qin_when s s1) as b1.
      remember (Qin_when s s2) as b2.
      destruct b1 eqn:Hb1, b2 eqn:Hb2.
      * (* b1 = true, b2 = true *)
        intros H.
        apply orb_true_iff in H; destruct H as [H|H].
        -- apply F with (s:=s) (s0:=s0) (ss:=s1) in H.
           apply orb_true_iff; left; apply orb_true_iff; left; exact H.
        -- apply orb_true_iff in H; destruct H as [H|H].
           ++ apply orb_true_iff; left; apply orb_true_iff; right; exact H.
           ++ apply orb_true_iff; right. apply F with (s:=s) (s0:=s0) (ss:=tl) in H. exact H.
      * (* b1 = true, b2 = false *)
        intros H.
        apply orb_true_iff in H; destruct H as [H|H].
      ...
      * (* b1 = false, b2 = true *)
        ...
        -- apply orb_true_iff; left; apply orb_true_iff; left; exact H.
        -- apply orb_true_iff in H; destruct H as [H|H].
           ++ apply F with (s:=s) (s0:=s0) (ss:=s2) in H.
              apply orb_true_iff; left; apply orb_true_iff; right; exact H.
           ++ apply orb_true_iff; right. apply F with (s:=s) (s0:=s0) (ss:=tl) in H. exact H.
      * (* b1 = false, b2 = false *)
        intros H.
