...
\subsection{``Infer Width''}
Implicit Width is allowed to be defined in HiFIRRTL programs.
Such unspecified widths are infered by the Infer Width pass. 
The general rules are,
\begin{itemize}
\item The only allowed width for explicit defined width is itself.
\item The implicit width will be assigned by the largest one that can
  be found through signal connections.
\item The last connection semantic is not considered at this stage,
...
\item[The output] of InferWidth give a new component environment $ce'$
  ...
  that afer this pass, signals are infered to zero width.  The overall
  compilation will use other passed to deal with zero width, which is
  not concerned here.
\end{description}

To state the correctness of InferWidth that implement in Coq, we first
state the semantics of width infering. According to each statement,


\subsection{``Expand Connects''}
...
\begin{description}
\item[The input] of ExpandConnects 
