...
To this end, $\madeexplicit$ calls a function $\sizeofftype$ to calculate the number of ground types included in a type (see Fig.~\ref{fig:size-list-ftype-fields} for its definition) \tl{to see how to improve this}. 
...
We also define a relation $\typeeq$ to specify the equivalence between two ground types, one from $M$ and the other one from $tm$. 
%
%For instance, in $\typeeq$, two types $(\uintimp, n_1)$ and $(\uintimp, n_2)$ are equivalent regardless of $n_1$ and  $n_2$, and the (abstract) $\reset$ type from $M$ is equivalent to both $(\uint, 1)$ and $\asyncreset$ from $tm$. (Note that according to the syntax of {\lofirrtl} in Fig.~\ref{fig-lofirrtl-synt}, the $\reset$ type does not appear in {\lofirrtl}. )
%
\typeeq:=
...
%    }
  ...
  \end{gather*}
  \caption{\madeexplicit, \typeeq, and \madeexplicitfields}
