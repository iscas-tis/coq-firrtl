...
To this end, $\madeexplicit$ uses a function $\sizeofftype$ to calculate the number of ground types included in a type %(see Fig.~\ref{fig:size-list-ftype-fields} for its definition) 
...
%
\typeeq:=
\[\left\{ ((\uintimp, \_), (\uintimp, \_)),  
%		
((\sintimp, \_) , (\sintimp, \_)),  
%		
(\reset, (\uint, 1)), 
(\reset,\asyncreset)\right\}
\]   
Intuitively, two types $(\uintimp, n_1)$ and $(\uintimp, n_2)$ are equivalent regardless of $n_1$ and  $n_2$, and the (abstract) $\reset$ type from $M$ is equivalent to both $(\uint, 1)$ and $\asyncreset$ from $tm$. (Note that according to the syntax of {\lofirrtl} in Fig.~\ref{fig-lofirrtl-synt}, the $\reset$ type does not appear in {\lofirrtl}. )
