...
There are three types of orientations, \duplex, \source, and \sink. The orientations are related to the connect statements. 
...
To tackle the mismatch between $tm$ and $tmap$, we introduce a function $\madeexplicit$ (see Fig.~\ref{fig:made-explicit}). The arguments of $\madeexplicit$ are $(ft, id, tm)$, where $ft \in \ftypes$, $id \in \gidset_M$, and $tm$ is a type mapping. Intuitively, $\madeexplicit$ utilizes $id$ and $tm$ to determine the missing bit widths in $ft$ (i.e. the widths for the ground types $(\uintimp, 0)$ and $(\sintimp, 0)$), resulting into a new type $newt$ where all the widths are explicit (i.e. non-zero). The function $\madeexplicit$ calls a function $\sizeofftype$ to calculate the number of ground types included in a type (see Fig.~\ref{fig:size-list-ftype-fields} for its definition). Moreover, it also calls a function $\typeeq$ to check the equivalence between two ground types, one from $M$ and the other one from $tm$. For instance, in $\typeeq$, two types $(\uintimp, n_1)$ and $(\uintimp, n_2)$ are seen as equivalent, no matter whether $n_1 = n_2$ or not, and the (abstract) $\reset$ type from $M$ is seen as equivalent to both $(\uint, 1)$ and $\asyncreset$ from $tm$. (Note that according to the syntax of {\lofirrtl} in Fig.~\ref{fig-lofirrtl-synt}, the $\reset$ type does not appear in {\lofirrtl}. )
