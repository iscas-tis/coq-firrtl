...
\smallskip
\noindent\emph{Formalization.}
We formalize the InferWidths pass as a function $\inferwidths$ such that for each module $M$, $\inferwidths(M)$ generates a new module $M'$ where all bit-widths are explicit. The $\inferwidths$ function is defined by the rule below. 
  \begin{gather*}
...
\begin{enumerate}
\item We first call $\tycnmap(M)$ to compute a triple $(atm, gtm, gcm)$ where $atm$ is a type mapping from $\range{id}$ to $\ftypes$, $gtm$ is a ground-type mapping from $\gidset_M$ to $\gtypes$,  and $gcm$ is a ground-type connection mapping which assigns to each $(v, m) \in \gidset_M$ a list of ground-type expressions that are connected to $(v, m)$. 
%
...
\item We call $\depgraph(gcm)$ to compute a dependency graph $G = (V, E)$ from $gcm$. As we choose to focus on the components of implicit widths, $V$ is the set of $(v, m) \in \gidset_M$ such that $gtm(v, m) \in \{(\uintimp, n), (\sintimp, n) \mid n \in \Nat\}$.  
Moreover, $E$ is defined as follows: if $(v, m) \in V$ and a component of identifier $(v', m') \in V$ occurs in $cm(v, m)$, then there is an edge from $(v',m')$ to $(v, m)$. 
%
\item We assume that \emph{$G$ is acyclic} and compute a topological sorting $ts$ of $G$. For convenience, we represent $ts$ as a vertex sequence. 
%
\item We define a recursive function $gtm_1 \xrightarrow[cm]{vs} gtm_2$ where $gtm_1, gtm_2: \range{id} \rightarrow \ftypes$ and $vs$ is a sequence of vertices in $G$. Let $gtm'$ be the ground-type mapping such that $gtm \xrightarrow[cm]{ts} gtm'$. 
%
\item We construct $atm':\range{id} \rightarrow \ftypes$ by calling $\detwidths(atm, gtm')$ to determine the implicit widths in $atm$ according to $gtm'$. 
%
\item Finally, we call  $\replacewidths(M, atm')$ to replace the types in $M$ by those in $atm'$ and generate $M'$. 
\end{enumerate}

Note here currently we require the dependency graph $G$ to be acyclic. The main reason is that %. This technical choice is justified by that 
inferring widths for cyclic dependency graphs is error-prone\footnote{See e.g. \url{https://github.com/llvm/circt/issues/1271} and \url{https://github.com/llvm/circt/issues/6742} for the the bugs in Firtool related to this.} and the semantics is unclear %how the semantics should be defined 
for the general case.  
 
We briefly explain how the procedures $\tycnmap(M)$ and $\topsort(G)$ work. 
\begin{itemize}
