...
The InferWidth pass infer the types of components of $M$ and additionally infers the unknown widths in these types. The InferWidths pass only modifies the declaration statements in the original program, and does not modify the other statements. 

\paragraph*{Formalization}
We shall formalize the InferWidths pass as a function $\inferwidths$ such that for each module $M$, $\inferwidths(M)$ generates a new module $M'$ where all widths are explicit. The $\inferwidths$ function is defined by the rule below. 
  \begin{gather*}
  \def\defaultHypSeparation{\hskip 10pt}
    \infer[xx]
     {\seqq{\inferwidths(M) = M'}}
     {
     \begin{array}{c}
      \tycnmap(M) = (atm, gtm, gcm) \hspace{15pt} G = \depgraph(gcm) \hspace{15pt} \topsort(G) = ts\\
    gtm \xrightarrow[gcm]{ts} gtm'  \hspace{15pt}  atm' = \detwidths(itm, gtm') \hspace{15pt} M' = \replacewidths(M, atm')
    \end{array}
    }
  \end{gather*}
The main idea is to focus on the ground-type components that are flattened from the components in $M$, infer the widths of the ground types, assemble the ground types into aggregate types where all the widths are explicit, and finally replace the aggregate types in $M$ by the inferred aggregate types and obtain the new module $M'$. Specifically, the computation of $\inferwidths(M)$ comprises the following steps. 
\begin{enumerate}
\item We first call the procedure $\tycnmap(M)$ to compute a triple $(atm, gtm, gcm)$ such that $atm$ is a type mapping from $\range{id}$ to $\ftypes$, and $gtm$ is a ground-type mapping, i.e. a function from $\gidset_M$ to $\gtypes$,  and $gcm$ is a ground-type connection mapping, i.e. a function that assigns to each $(v, m) \in \gidset_M$ a list of ground-type expressions that are connected to $(v, m)$. 
%
...
\item We call the procedure $\depgraph(gcm)$ to compute a dependency graph $G = (V, E)$ from $gcm$. We choose to focus on the components of implicit widths. Therefore, we set $V$ to be the set of $(v, m) \in \gidset_M$ such that $gtm(v, m) \in \{(\uintimp, n), (\sintimp, n) \mid n \in \Nat\}$.  
...
%
\item Then we define a recursive function $gtm_1 \xrightarrow[cm]{vs} gtm_2$ where $gtm_1, gtm_2 \in \range{id} \rightarrow \ftypes$ and $vs$ is a sequence of vertices in $G$. Let $gtm'$ be the ground-type mapping such that $gtm \xrightarrow[cm]{ts} gtm'$. 
%
\item We construct $atm' \in \range{id} \rightarrow \ftypes$ by calling the procedure $\detwidths(atm, gtm')$ to determine the implicit widths in $atm$ according to $gtm'$. 
%
\item Finally, we call the procedure $\replacewidths(M, atm')$ to replace the types in $M$ by those in $atm'$ and generate $M'$. 
\end{enumerate}
...
Let us briefly explain how the procedures $\tycnmap(M)$ and $\topsort(G)$ work. 
\begin{itemize}
\item The definition of the function $\tycnmap(M)$, that is, the computation of $(atm, gtm, gcm)$, is roughly the combination of the function $\portsstmtstmap$ and the relation $cm_1 \xrightarrow[tm, tmap]{ss} cm_2$ in Section~\ref{sec-form-firrtl}. 
%
\item The construction of a dependency graph from $cm$, i.e.  the procedure $\depgraph(cm)$, is easy to define. 
% 
\item The procedure $\topsort(G)$ is adapted from a depth-first search (DFS) algorithm in the graph library of the mathematical components repository~\cite{DP20,CoqGraph}. Given a directed graph $G$, $\topsort(G)$ either reports that the input graph is acyclic and outputs a topological sorting of the acyclic graph, or reports that the input graph is cyclic and outputs a cycle. Moreover, accordingly, we prove the correctness of the topological sorting algorithm by adapting the correctness proof of the DFS algorithm. 
%
%\item The procedure $\detwidths(tm, gtm')$ can also be easily defined.  
\end{itemize}
Moreover, the readers can also intuitively figure out the definitions of the procedures $\depgraph(gcm)$, $\detwidths(atm, gtm')$, and $\replacewidths(M, atm')$. 
Therefore, to avoid tediousness, we omit the detailed definitions of these procedures. 
