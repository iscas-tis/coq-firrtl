...
Since the InferWidth transformation only modifies the declaration statements, we shall formalize it as a function $\inferwidth_M$ from $\range{id} \rightarrow \ftypes$ to $\range{id} \rightarrow \ftypes$. 
To define $\inferwidth_M$, we first compute from $M$ a topological sorting of the dependency graph of its ground-type components, denoted by $tpSort$. Moreover, we compute from $M$ a mapping $connExps$ such that  $connExps(v, m)$ is a list of expressions that are connected to $(v, m)$. Finally, $\inferwidth_M$ is defined as $infer$
  ...
  \begin{gather*}
   ...
   \\[1ex]
    ...
    \begin{array}{c}
      order = v\ tl \\
      el = var2exprs(v) \\
      tmap'' = {\sf inferWidthFun}(v, el, tmap) \\
