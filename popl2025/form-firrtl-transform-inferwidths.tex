...
$gtm \xrightarrow[cm]{ts} gtm'$ is defined by two rules,  for the cases $ts = \nil$ and $ts \neq \nil$ respectively. The first rule is trivial, that is, $gtm \xrightarrow[gcm]{\nil} gtm$. For the case $ts \neq \nil$, let $ts = (v, m)\ ts'$,   
\begin{itemize}
\item it obtains the ground type $gt = gtm(v, m)$ and the list of ground-type expressions $cgels = gcm(v, m)$ connected to $(v, m)$, and computes the corresponding list $cgtls$ of ground types for $cgels$, 
...
$\maxgtlist(gt, cgtls)$ is defined by the rules in Fig.~\ref{fig:maxgtlist}, where the maximum is chosen among the widths for the implicit types. 
