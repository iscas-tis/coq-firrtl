...
\begin{itemize}
\item The definition of the function $\tycnmap(M)$, that is, the computation of $(tm, gtm, gcm)$, is roughly the combination of the function $\portsstmtstmap$ and the relation $cm_1 \xrightarrow[tm, tmap]{ss} cm_2$ in Section~\ref{sec-form-firrtl}. 
%
\item The construction of a dependency graph from $cm$, i.e.  the procedure $\depgraph(cm)$, is easy to understand. 
% 
...
%
\item The procedure $\instantiate(tm, gtm')$ can also be easily defined.  
\end{itemize}
Moreover, the readers can also intuitively figure out the definitions of the procedures $\depgraph(gcm)$ and $\instantiate(tm, gtm')$. 
Therefore, to avoid tediousness, we omit the detailed definitions of these five procedures. 
