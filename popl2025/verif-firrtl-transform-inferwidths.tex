...
Here {\sf madeImplicit} is a function that makes bit-width implicit, given a type mapping with only implicit widths components $tm$ and the original module $M$, it return a type mapping that makes some components implicit according to the definitions in $M$.
...
For the second formula, A typing mapping $tm' \preceq tm$ means that they involve same components and regardless of implicit or not, the bit width of a component in $tm'$ is less than or equal to the width of it in $tm$.
