...
which is also formalized in Coq.
We present the operational semantics for LoFIRRTL programs that are free of \prettyll{inst of} statements.
...
We denote by $\expgtypes$ the set of explicit ground types $(\{\mbox{\prettyll{UInt}}, \mbox{\prettyll{SInt}}\} \times \Nat) \cup \{\mbox{\prettyll{Clock}},
...
This is checked by the formally verified topological sorting algorithm mentioned in Section~\ref{sec-form-verif-transform}.
...
is violated. 
%Furthermore, the functional correctness of the ``ExpandWhens'' compilation pass is also proved in Coq.
...
\end{subfigure}
\caption{Examples for pseudo-SSA form}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
A \emph{configuration} of the module $A$ is a triple $(\tEnv, \sEnv, \rEnv)$, where $\tEnv:\cpnts(A)\rightarrow \expgtypes$ is a type environment,
...
The type environment $te$ and state $s$ are lift to expressions $e$ as usual, whose definitions are given in the sequel. % Appendix~\ref{app-lofirrtl}.
...
\end{itemize}
The function $te$ and $s$ from $\cpnts(A)$ to $\expgtypes$ are extended to expressions as follows.
\begin{itemize}
...
Let $\expgtypes$ denote the set of ground types, that is, $(\{UInt, SInt\} \times \Nat) \cup \{Clock, Reset, AsyncReset\}$. Let $\bv$ denote the set of bit vectors, that is, $\{0,1\}^+$.
...
\begin{itemize}
\item a \emph{type environment} $te$ of $A$ is a function from $\cpnts(A)$ to $\expgtypes$,
%
...
%A \emph{type environment} of $M$ is a function from $\varset(M)$ to $\expgtypes$.
%A \emph{state} of $M$ is a function from $\varset(M)$ to $\bv \cup \{\bot\}$. A \emph{register state} $rs$ of $M$ is a function from $\varset(M)$ to $\bv \cup \{\bot\}$ such that $rs(v) = \bot$ for every $v \in \expgtypes \setminus \registers(M)$. Note that $\bot$ is a special symbol denoting the fact that the value of a variable is not determined yet.
