...
\begin{table}[htbp]
  ...
  \begin{gather*}
   ...
   \inferiii[size\_of\_fields: $\neq$ nil]
    ...
    {\seqq{
    \begin{array}{c}
    \forall n.\ 0 \le n < size\_of\_ftype(ft) \Rightarrow vm(v, n) = none \wedge vm'(v,n) = list\_gtype(ft)[n] \\
    ...
    \\[1ex]    
    \infer[ct\_match]
    {\seqq{ct\_match(v, ft, vm, vm')}}
    {\seqq{
    \begin{array}{c}
    \forall n.\ 0 \le n < size\_of\_ftype(ft) \Rightarrow ct(v, n) = none \wedge ct'(v,n) = undefined \\
    \forall n.\ n \ge size\_of\_ftype(ft) \Rightarrow ct(v, n) =  ct'(v,n)
    \end{array}
    }}
    \quad
  \end{gather*}
%  \vspace{-4mm}
  \caption{$vm\_match(v, ft, vm, vm')$ and $ct\_match(v, ft, vm, vm')$, where $++$ means the list concatenation}
  \label{tab:vm-match}
\end{table}
...
\begin{table}[t]
  ...
  \begin{gather*}
   ...
   {tmap(id(x))= newft} {vm\_match(id(x), newft, vm, vm')} {ct\_match(id(x), newft, ct, ct')}
    \quad
    \\[1ex]
    \inferiii[node]
    {\seqq{(vm, ct) \xrightarrow[tmap]{\mbox{node } x = e} (vm', ct')}}
   {tmap(e)= newft} {vm\_match(id(x), newft, vm, vm')} {ct\_match(id(x), newft, ct, ct')}
    \quad
    \\[1ex]
    \inferii[reg: $\neq\bot$]
