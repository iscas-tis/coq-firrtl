...
In response to (1), we define a denotation semantics for \hifirrtl. To this end, we represent a \lofirrtl program by a pair $(tm, cm)$ of type and connection mappings, and define a conformance relation between a low FIRRTL module and a HiFIRRTL program $M$, namely, $(tm, cm)\models M$. As such, the semantics $M$, i.e., $\llbracket M\rrbracket$  is interpreted as all $(tm,cm)$ such that $(tm, cm)\models M$.  


%VeFir provides a mechanized formal semantics for low FIRRTL,
%and formalizes the syntax of FIRRTL and 
In response to (2) We formalize the three key lowering transformations, i.e., InferWidths, ExpandConnect and ExpandWhens and their correctness based on the semantics, and verify formally that these transformations preserve the semantics of a HiFIRRTL program. All of them are mechanized in
