...
We represent a {\lofirrtl} program by a pair $(tm, cm)$ of type and connection mappings, and define a conformance relation between a {\lofirrtl} module and a {\hifirrtl} program $M$, i.e., $(tm, cm)\models M$.
As such, the semantics of $M$ is interpreted as the set of {\lofirrtl} modules $(tm,cm)$ such that $(tm, cm)\models M$.
The challenge is that there is an abstraction-level gap between {\hifirrtl} and {\lofirrtl} programs:
the former embraces high-level constructs such as aggregate types (i.e., bundle and vector types), implicit bit-widths for their components, and conditional statements (when),
whereas the latter is close to circuits where all components are of ground type and all bit-widths must be explicitly given.


%VeFir provides a mechanized formal semantics for low FIRRTL,
%and formalizes the syntax of FIRRTL and
For (2), we formalize the three key lowering transformations, i.e., InferWidths, ExpandConnects and ExpandWhens,
...
For (3), we verify that these transformations preserve the semantics.
All the semantic definitions, correctness specification and proofs are mechanized in
...
In total, 22,703 lines of Coq code are used for specifying the syntax and semantics of FIRRTL programs, and 14,450 lines of Coq code are used for the formalization of the three lowering transformations and their correctness proofs.
The Coq source code and benchmarks are available at \url{https://anonymous.4open.science/r/VeFir-D081}.

%% 22,703 loc : 7,333 for {\hifirrtl} and 15370 for {\lofirrtl}
%
%
