...
%
%
%Following this promising research direction,

%FIRRTL originated from the high-level hardware construction language Chisel~\cite{BachrachVRLWAWA12}.

Chisel (Constructing Hardware in a Scala Embedded Language~\cite{BachrachVRLWAWA12}) is an open-source hardware description language used to describe digital electornics and circuits and the register-transfer level. 
%
...
XiangShan~\cite{xiangshan,Xiangshan-github}.
...
99 have been labeled as bugs~\cite{FIRRTLissues}. This motivates us to investigate the formalization and verification of FIRRTL and its lowering transformations, along the similar line for formally verifying compilers of mainstream programming languages such as C/C++ (a review of which is given in Section~\ref{sec-related-work}).
%
...
%
%Nevertheless, 
Partially because Chisel and FIRRTL are relatively new and the ecosystem around them is still evolving, formalization and verification of FIRRTL and its lowering transformations are largely missing. To the best our knowledge,  the only  exception is \cite{HarrisonHA21} where a coinductive denotational semantics for an idealized subset of LoFIRRTL was proposed and formalized in the Agda proof assistant, based on a formalism called device calculus. %Nevertheless, while \cite{HarrisonHA21} made some progress in the formalization of LoFIRRTL, 

In general, a framework where programs in HiFIRRTL and its lowering transformations can be formally reasoned about 
%is still elusive. %The establishment of 
%Such a framework would require
requires at least (1) a mechanized %operational 
semantics of both HiFIRRTL and LoFIRRTL,
...
\smallskip
\noindent
{\bf Contributions.} In this paper, we present VeFir, a framework to formally reason about programs in FIRRTL and its lowering transformations.
...
In response to (1), we define a denotation semantics for \hifirrtl. To this end, we represent a \lofirrtl program by a pair $(tm, cm)$ of type and connection mappings, and define a conformance relation between a low FIRRTL module and a HiFIRRTL program $M$, namely, $(tm, cm)\models M$. As such, the semantics $M$, i.e., $\llbracket M\rrbracket$  is interpreted as all $(tm,cm)$ such that $(tm, cm)\models M$. The main challenge is that there is a semantics gap between {\hifirrtl} and {\lofirrtl} programs: the former embraces high-level constructs such as aggregate types (e.g., bundle and vector types), implicit bit-widths for their components, partial connections, and conditional statements (when), etc, whereas the latter is very close to circuits where all components are of ground type and all bit-widths must be explicitly given. 


%VeFir provides a mechanized formal semantics for low FIRRTL,
%and formalizes the syntax of FIRRTL and 
In response to (2), we formalize the three key lowering transformations, i.e., InferWidths, ExpandConnect and ExpandWhens and their functional correctness based on the semantics. Based on the denotational semantics, a {\hifirrtl} program is an underspecification   which may be satisfied by many {\lofirrtl} programs and the transformations can be viewed as a refinement process, and eventually induce an ``optimal" {\lofirrtl} program which should respect all the constraints of the  {\hifirrtl} program but also admits the lowest bit-widths. 


In response to (3), we verify formally that these transformations preserve the semantics. All of semantics definitions, the correctness specification and the proofs are mechanized in
