...
FIRRTL~\cite{IzraelevitzKLLW17} was proposed  as an intermediate representation (IR) for the compilation of Chisel programs, analogous to the role of LLVM IR~\cite{LLVM} for compiling C programs.
...
In general, a framework where programs in HiFIRRTL and its lowering transformations can be formally reasoned about is still elusive. %The establishment of 
Such a framework would require
a mechanized %operational 
...
formalization of %HiFIRRTL and its 
the lowering transformations from HiFIRRTL to LoFIRRTL, together with the %as well as 
specification and verification of the correctness of its lowering transformations.
...
In this work, we establish for the first time a framework that formally reasons about programs in FIRRTL~\cite{firrtl-spec} (Flexible Intermediate Representation for RTL) and its lowering transformations (aka compilation passes).
