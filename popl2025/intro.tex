...
This motivated us to investigate the formalization and verification of FIRRTL and its lowering transformations,
along a similar line as formally verifying compilers of mainstream programming languages such as C/C++
...
%
%Nevertheless, 
Partially because Chisel and FIRRTL are relatively new and the ecosystem around them is still evolving, formalization and verification of FIRRTL and its lowering transformations are largely missing.
...
We represent a \lofirrtl program by a pair $(tm, cm)$ of type and connection mappings, and define a conformance relation between a low FIRRTL module and a HiFIRRTL program $M$, namely, $(tm, cm)\models M$.
As such, the semantics $M$, i.e., $\llbracket M\rrbracket$  is interpreted as all $(tm,cm)$ such that $(tm, cm)\models M$.
The main challenge is that there is a semantics gap between {\hifirrtl} and {\lofirrtl} programs:
...
The transformations can be viewed as a refinement process, and eventually produce an ``optimal" {\lofirrtl} program,
which should respect all the constraints of the {\hifirrtl} program but also admits the lowest bit-widths.


For (3), we verify formally that these transformations preserve the semantics.
All semantic definitions, the correctness specification and the proofs are mechanized in
the interactive theorem prover Coq. In total, \zhilin{xxx} lines of Coq code are used for specifying the syntax and semantics of {\hifirrtl} programs, and \zhilin{xxx} lines of Coq code are used for the formalization of the three lowering transformations and their correctness proofs. 
