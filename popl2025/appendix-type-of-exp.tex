...
\item $\typeofexp(x, tmap) =tmap(id(x))$, where $x$ is a name in $M$.
  If $x$ is defined as an implicit ground type in $tmap$, the {\typeofexp} will choose a possible explict width for $x$. If $x$ is explicit in $tmap$, then the same explicit type is returned. If $x$ is an aggregate type in $tmap$, {\typeofexp} inductively make a choice on width for all the implicit subfields. Since expressions are used on the right-hand side of connections, all the chosen widths have to satisfy the {\typecompatible} constraints. 
%
