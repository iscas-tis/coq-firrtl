...
%
\item $\typeofexp(x.f, tmap)$ is inductively defined as follows: Suppose $\typeofexp(x, tmap) = (\btype, ft)$. If $f$ occurs in $ft$, that is, $ft$ contains a triple $(f, b, t)$, then $\typeofexp(x.f, tmap)$ is defined as $t$, otherwise, $\typeofexp(x.f, tmap) = \svnone$. 
%
\item $\typeofexp(r[n], tmap)$ is inductively defined as follows: Suppose $\typeofexp(r, tmap) = (\vtype, t[m])$ for some $m \in \Nat$. If $n < m$, then $\typeofexp(r[e], tmap) = t$, otherwise, $\typeofexp(r[n], tmap) = \svnone$. 
%
