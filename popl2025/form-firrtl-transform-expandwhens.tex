...
\begin{gather*}
    ...
    \infer[]
    {\seqq{\expandbranch(ss ; \mathit{declare}\ x, ecm_0) = (ss_\mathrm{dcl} ; \mathit{declare}\ x, ecm[\svundefined/x])}}
    {\seqq{\expandbranch(ss, ecm_0) = (ss_\mathrm{dcl}, ecm)}}
\\[1ex]
    \label{rule-expandwhen-connect}
    \infer[]
    {\seqq{\expandbranch(ss ; x <= e, ecm_0) = (ss_\mathrm{dcl}, ecm [e/x])}}
    {\seqq{\expandbranch(ss, ecm_0) = (ss_\mathrm{dcl}, ecm)}}
\\[1ex]
    \label{rule-expandwhen-invalid}
    \infer[]
    {\seqq{\expandbranch(ss ; x \text{ \prettyll{is invalid}}, ecm_0) = (ss_\mathrm{dcl}, ecm [{\tt invalid}/x])}}
    {\seqq{\expandbranch(ss, ecm_0) = (ss_\mathrm{dcl}, ecm)}}
    \\[1ex]
    \label{rule-expandwhen-when}
    \inferii[]
    {\seqq{\expandbranch(ss ; \text{\prettyll{when c then} } ss^\mathsf{T} \text{ \prettyll{else} } ss^\mathsf{F}, ecm_0) =
        (ss_\mathrm{dcl} ; ss_\mathrm{dcl}^\mathsf{T} ; ss_\mathrm{dcl}^\mathsf{F}, \combinebranches(c, ecm^\mathsf{T}, ecm^\mathsf{F}))}}
    {\seqq{\expandbranch(ss, cm_0) = (ss_\mathrm{dcl}, ecm)}}
    {\begin{gathered}[b] \seqq{\expandbranch(ss^\mathsf{T}, ecm) = (ss_\mathrm{dcl}^\mathsf{T}, ecm^\mathsf{T})} \\
                      \seqq{\expandbranch(ss^\mathsf{F}, ecm) = (ss_\mathrm{dcl}^\mathsf{F}, ecm^\mathsf{F})} \end{gathered}}
\end{gather*}
%
Note that for a statement \prettyll{when c then} $ss^\mathsf{T}$ \prettyll{else} $ss^\mathsf{F}$,
 the function {\expandbranch}  first processes the statements preceding this statement and obtains a connection mapping $ecm$, then uses $ecm$ to process $ss^\mathsf{T}$ and $ss^\mathsf{F}$ respectively,
 resulting in connect mappings $ecm^\mathsf{T}$ and $ecm^\mathsf{F}$ respectively.
 Finally, $ecm^\mathsf{T}$ and $ecm^\mathsf{F}$ are combined into one mapping by {\combinebranches}.
...
\\[1ex]
 \label{rule-expandwhen-combine-2}
    \infer[]
    {\combinebranches(c, ecm^\mathsf{T},ecm^\mathsf{F})(x) = {\tt invalid}}
    {ecm^\mathsf{T}(x) = ecm^\mathsf{F}(x) = {\tt invalid}}
\end{gather*}
...
\begin{gather*}
 \label{rule-expandwhen-combine-3}
    ...
    {e^\mathsf{T} \not= e^\mathsf{F}}
\end{gather*}
To make this rule work properly, $ecm$ needs to contain all connections that are in scope.
...
Note that if in the end some component $x$ is still assigned the value $\svnone$ by $ecm$,
...
{\begin{gather*}
    ...
    {\expandbranch(ss, {\sf emp}) = (ss_{\sf dcl}, ecm)}
    {\seqq{\forall x \in \gidset_M.\ ecm(x) \neq \svundefined}}
