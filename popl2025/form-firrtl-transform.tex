...
In this section, we formalize the lowering transformations of {\hifirrtl} described informally in Section~\ref{subsec-lowering-transformations} in Coq. Specifically, we consider three key lowering transformations, namely, %``InferTypes'',
...
For simplicity, we focus on a {\hifirrtl} circuit that contains no \prettyll{inst of} statements, that is, a circuit that contains exactly one top-level module $M$. We still assume an identifier function $id$ that assigns to each component name in $M$ a unique natural number. 
