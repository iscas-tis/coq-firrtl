...
In this section, we show how the lowering transformations of HiFIRRTL, which were described informally in Section~\ref{subsec-lowering-transformations}, are formalized in Coq. Specifically, we consider the formalization of the three lowering transformations, namely, %``InferTypes'',
``InferWidths'', %``InferResets'',
``ExpandConnects'', and ``ExpandWhens''. %, and ``LowerTypes''.
These three transformations are the key transformations from {\hifirrtl} programs to {\lofirrtl} ones. 
Moreover, we shall show how to prove the semantics preservation property of these three transformations. 

For simplicity, let us focus on a {\hifirrtl} circuit that contains no \prettyll{inst of} statements, that is, a circuit that contains exactly one top-level module $M$. We still assume an identifier function $id$ that assigns to each component name in $M$ an unique natural number. 

\subsection{Formalization of Transformations}\label{sec:form-transform}

In the sequel, we show how to formalize the three transformations. 
...
\input{form-firrtl-transform-inferwidths}


%%%% the original texts for InferWidths
\hide{
...
\input{transform-expandconnects}
...
The function \expandbranch scans through the statement sequence $ss$, copies the declaration statements, and updates the component environment $cm$ when processing connect, \prettyll{is invalid}, and \prettyll{when} statements.  The function \combinebranches is used to combine the connect mappings for branches of \prettyll{when} statements.
...
The  \expandbranch function is defined by the following four rules, % (\ref{rule-expandwhen-declare})-(\ref{rule-expandwhen-when}),
where $\mathit{declare}\ x$ represents a declaration statement for $x$, e.g. \prettyll{wire x: t}.
\begin{gather*}
    \label{rule-expandwhen-declare}
    \infer[]
    {\seqq{\expandbranch(ss ; \mathit{declare}\ x, cm_0) = (ss_\mathrm{dcl} ; \mathit{declare}\ x, cm[\bot/x])}}
...
\\[1ex]
    ...
    \infer[]
    {\seqq{\expandbranch(ss ; v \text{ \prettyll{is invalid}}, cm_0) = (ss_\mathrm{dcl}, cm [{\tt invalid}/v])}}
    {\seqq{\expandbranch(ss, cm_0) = (ss_\mathrm{dcl}, cm)}}
\end{gather*}
%
\begin{gather*}
...
Note that for a statement \prettyll{when c then} $ss^\mathsf{T}$ \prettyll{else} $ss^\mathsf{F}$,
 the function \expandbranch  first processes the statements preceding this statement and obtains a connect mapping $cm$, then uses $cm$ to process $ss^\mathsf{T}$ and $ss^\mathsf{F}$ respectively, resulting into the connect mappings $cm^\mathsf{T}$ and $cm^\mathsf{F}$ respectively. Finally, $cm^\mathsf{T}$ and $cm^\mathsf{F}$ are combined into one mapping by \combinebranches.
...
The function \combinebranches combines two connect mappings into one by the following fix rules %(\ref{rule-expandwhen-combine-1})-(\ref{rule-expandwhen-combine-6}),
...
\\[1ex]
 \label{rule-expandwhen-combine-6}
    \infer[]
    {\combinebranches(c, cm^\mathsf{T},cm^\mathsf{F})(x) =\bot}
    {cm^\mathsf{F}(x) = \bot}
\end{gather*}
...
Finally, the statement sequence is updated by concatenating the statement sequence generated by the function  \expandbranch and the sequence of connect and \prettyll{is invalid} statements corresponding to the connect mapping generated by the function \expandbranch.
