...
expanding the aggregate type using \listgtyperef, and also expanding the reset value with
\listgexp. 
...
\begin{gather*}
   ...
   \infer[]
         ...
         {\begin{array}{c}
            ...
            \seqq{\typeofref(r') = (t,ori')} \wedge
            \seqq{\listgtyperef(t, \lfalse) = tl} \\
             \seqq{ ss = if\;{\sf isOriFit}\; then\; {\tt map3}\; (fun\; (v_l,v_r,(\_,flipped)) \Rightarrow if\;flipped\; then\;v_l\, \mbox{\prettyll{ <= }}\, v_r\; else\;v_r\, \mbox{\prettyll{ <= }}\,v_l)\,(rl,rl',tl)}
           \end{array}
         }
\end{gather*}
%
In the case above, the connection is set between two references of
elements $r$ and $r'$. The function {\sf isOriFit} checks whether the
orientations can fit together.  When the connection takes an
