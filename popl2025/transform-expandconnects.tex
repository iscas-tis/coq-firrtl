...
versa. %As a result, their orientations are changed between \source \, and \sink.
 \begin{gather*}
   ...
   \infer[]
         ...
         {\begin{array}{c}
             \seqq{{\expandref(x)} = rl \wedge \listgtype(t, \lfalse) = tl }\\
...
lists. It applies the generation function of input/output port statement to the
lists $(rl,tl)$ produced by expanding references and expanding types.
...
\begin{gather*}
  ...
  \infer[]
        ...
        {\begin{array}{c}
            ...
            \seqq{\listgtype(t, \lfalse) = tl} \wedge            
...
as input, which defines 
a register {\tt x}. Similar to expanding a port, expanding a register
involves expanding the reference to its subfield representations using \expandref,
expanding the aggregate type using \listgtype, and also expanding the reset value with
\listgexp. 

Nodes are also \source \, elements. They are only allowed to be defined
with passive types, which means no flip fields. During
...
\begin{gather*}
   ...
   \infer[]
         ...
         {\begin{array}{c}
            ...
            \seqq{\listgtype(t, \lfalse) = tl} \\
             \seqq{ ss = {\tt map3}\; (fun\; (v_l,v_r,(\_,flipped)) \Rightarrow if\;flipped\; then\;v_l\, \mbox{\prettyll{ <= }}\, v_r\; else\;v_r\, \mbox{\prettyll{ <= }}\,v_l)\,(rl,rl',tl)}
           \end{array}
         }
\end{gather*}
In the case above, the connection is set between two references of
elements $r$ and $r'$. When the connection takes an expression other than a
...
\begin{gather*}
   ...
   \infer[]
         ...
         {\begin{array}{c}
            ...
            \seqq{\typeofref(r) = (t,\_)} \wedge
             \seqq{\listgexp(e, t) = el}\\
             \seqq{ ss = {\tt map2}\; (fun\; (v,e) \Rightarrow v\, \mbox{\prettyll{ <= }}\, e)\,(rl,el)}
           \end{array}
         }
\end{gather*}
