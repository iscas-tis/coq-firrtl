...
and ``sink''.
 \begin{gather*}
   \label{expand-input}
   \infer[]
         {\seqq{{\tt expand\_connect} (\mbox{\prettyll{input v t}}) = ss}}
         {\begin{array}{c}
...
and ``ss'' is the sequence of output statements.  The ``ss'' is
produced by expanding the port {\tt v} to its atomic subfields with
its type {\tt t}, and flipping the port orientation by the judging
whether the subfields are marked with ``flip''({\tt is\_flipped}).
The {\tt map} is the genernal operation for lists that applies a
function over lists. In the {\tt expand\_connect} function for
``\mbox{\prettyll{input v}}'', it applies the judging {\tt
  is\_flipped} over all the expanded references of {\tt v} then gives
...
sequencially. Then the module in return contains only the elements and
