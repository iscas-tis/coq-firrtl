 ...
 \begin{gather*}
   ...
   \infer[]
         ...
         {\begin{array}{c}
             ...
             \seqq{{\tt flatten\_ftype \;t} = [t_0,\dots t_n]}\\
             \seqq{ pp= {\tt map2}\; (fun\; v_x \Rightarrow if\;{\tt is\_flipped}(v_x)\;then\; \mbox{\prettyll{output}}\; {\tt v_x}\; {\tt t_x}\;else\; \mbox{\prettyll{input}}\; {\tt v_x}\; {\tt t_x})\, [v_0,\dots v_n][t_0,\dots t_n]}
           \end{array}
         }
 \end{gather*}
...
input statement is the port declaration ``\mbox{\prettyll{input v t}}'',
and ``$pp$'' is the sequence of output. The ``$pp$'' is produced by
expanding the port to its atomic subfields using {\tt expand\_ref v}
with its type expanded by {\tt flatten\_ftype t}.
Then it flips the port orientation by judging whether the
subfields are marked with ``flip''. Here, the {\tt map2}
is the general operation for lists that applies a function over two
lists. In the {\tt expand\_connect} function for
``\mbox{\prettyll{input v t}}'', it applies the the function 
{\tt is\_flipped} over the list of expanded references $[v_0,\dots v_n]$
and the list of flattened types $[t_0,\dots t_n]$.
The result $pp$ is composed by 
...
\begin{gather*}
  ...
  \infer[]
        ...
        {\begin{array}{c}
            ...
            \seqq{ s s= {\tt map3}\; (fun\; v_x\;t_x\; rv_x\; \Rightarrow \mbox{\prettyll{reg}}\; v_x:\,\mbox{\prettyll{t_x clk with:(reset=> rst,}}\,rv_x))}
            %% \rightline{
              \seqq{[v_0,\dots v_n]\,[t_0,\dots t_n]\,[rv_0,\dots rv_n]}
          \end{array}
        }
\end{gather*}
...
\begin{gather*}
  ...
  \infer[]
        ...
        {\begin{array}{c}
            ...
            \seqq{ s s= if\;{\tt is\_passive}(v_x)\; then\; {\tt map2}\; (fun\; v_x\;e_x \Rightarrow \; \mbox{\prettyll{node}}\; {\tt v_x}\;{\tt e_x})\,[v_0, v_1,\dots v_n]\,[e_0, e_1,\dots e_n] else\; [::]}
          \end{array}
        } 
\end{gather*}
...
\begin{gather*}
   ...
   \infer[]
         ...
         {\begin{array}{c}
             ...
             \seqq{ s s = {\tt map2}\; (fun\; l_x\;r_x \Rightarrow if\;{\tt is\_flipped}(l_x)\; then\;\ {\tt r_x}\, \mbox{\prettyll{ <= }}\, {\tt l_x}\; else\;{\tt l_x}\, \mbox{\prettyll{ <= }}\, {\tt r_x})\,[l_0,\dots l_n]\,[r_0,\dots r_n]}
           \end{array}
         }
\end{gather*}
...
\begin{gather*}
   ...
   \infer[]
         ...
         {\begin{array}{c}
             ...
             \seqq{ s s = {\tt map2}\; (fun\; l_x\;e_x \Rightarrow {\tt l_x}\, \mbox{\prettyll{ <= }}\, {\tt e_x})\,[l_0,\dots l_n]\,[e_0,\dots e_n]}
           \end{array}
         }
\end{gather*}
