...
In this section, we discuss some related work about compiler verification for software and hardware design languages. 
...
In this work, we focus on the formalization of FIRRTL, an intermediate representation for the Chisel hardware description language.
...
As mentioned in the introduction, the closest work to ours is~\cite{HarrisonHA21}, which formalized a coinductive denotational semantics in Agda for an idealized subset of LoFIRRT, based on a formalism called device calculus. Our work makes substantial progress from \cite{HarrisonHA21} in the following aspects:  At first, the operational semantics  of LoFIRRTL is defined for the first time. Moreover, extensive experiments are done to validate the semantics against Treadle, the reference interpreter for LoFIRRTL,  by utilizing a benchmark suite comprising hundreds of programs that are collected from various sources. Furthermore, we go one step further by formalizing and verifying HiFIRRTL and its lowering transformations, thus laying down a solid foundation towards a fully verified FIRRTL compiler. 
