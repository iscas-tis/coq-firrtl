...
The definitions of $cm_1 \xrightarrow[tm, tmap]{s} cm_2$ and $cm_1 \xrightarrow[tm, tmap]{ss'} cm_2$ are given in Fig.~\ref{fig:firrtl-sem}. We first introduce some functions. 
...
\begin{itemize} 
	...
	%
	\item $\typecompatible(ft, ft', flipped)$ checks whether the two types $ft$ and $ft'$ are compatible and distinguish between whether $flipped = \ltrue$, 
	%
...
%
\item In the rule ``{\listgtyperef: vector}'', if $ft = (\vtype, ft'[m])$ and $\listgtyperef(ft', flipped) = l'$, then it returns $\underbrace{l' \ ++\ \cdots\ ++\ l'}_{m}$.   
%
...
\begin{align*}
	...
	\underbrace{\listgtyperef(ft', flipped) ++ \cdots ++ \listgtyperef(ft', flipped)}_{m}\\
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%     
    ...
    {\seqq{\listgtypereffields(\myvec{f}, flipped) = []}}
    {\seqq{\myvec{f} = \nil}}
    \quad
    \\[1ex]
