...
The ExpandConnects transformation expands the \prettyll{connect} statements between aggregate
types into those between ground types. Correspondingly, it also flattens an aggregate-type component to a collection of ground-type components, so that after the ExpandConnects transformation, all the components are of ground types. 
...
In the beginning of a {\firrtl} module, ports are declared first. When ports have flipped subfields, the expansion process changes those fields of input ports into output ports, and vice
versa. %As a result, their orientations are changed between \source \, and \sink.
 ...
 \end{gather*}
{\expandconnects} takes a statement as input and produces a sequence of statements as output. 

In the case above, the
input statement is the port declaration ``\prettyll{input x : t}'',
and ``$pp$'' is the sequence of output statements. The ``$pp$'' is produced by
expanding the port reference and its type.
According to the flip information carried by the types, the input port may be flipped to an output port.
Here, {\tt map2}
is a general operation for lists that applies a function over two
lists, which applies the generation function of the input/output port statement to the
lists $(rl,tl)$ produced by expanding references and types.

Elements such as wires and registers have duplex
orientation. Regardless of the flip subfields in the original aggregate types, the resulting orientations will remain duplex. To
expand registers of aggregate types, we also need to expand the reset
values of the registers. Expanding wires is relatively simple and is omitted here.
\begin{gather*}
  \label{expand-reg}
  \infer[]
...
``\prettyll{reg x: t clk with: reset=>(rsig, rval)}''
...
To expand the \prettyll{is invalid} statement, we
